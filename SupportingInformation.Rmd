...
cop_HB<-function(db,dp,year,rawplot,ploton){
  ...
  if(ploton==T){
    if(rawplot==T){
      plot(tab_dbp$avg.H,tab_dbp$avg.Biomass,xlab="avg.H",ylab="avg.Biomass",col=rgb(0,0,1,0.3),pch=19,cex.lab=1.5)
      mtext(paste0("Year=",year))
    }else{
      plot(cop_tab_dbp$avg.H,cop_tab_dbp$avg.Biomass,xlab="avg.H",ylab="avg.Biomass",col=rgb(1,0,0,0.3),pch=19,cex.lab=1.5)
      mtext(paste0("Year=",year))
    }
  }
