...
Call the result $b_i(t)$. This is the surrogate dataset.

\noindent Our code for this algorithm is `ncsurrog` in the `BIVAN` repository 
(\url{https://github.com/sghosh89/BIVAN}). Because, for each $i$, the final time series 
...
numbers of NAs are not too large and $T$ is not too small. The Kendall version of the
algorithm throws an error if there are ties in any of the time series. The algorithm
also throws an error in the event that
the matrix generated in the second step is not positive semidefinite. 
