...
We also defined a statistic $\P_{l_b,u_b}$, using the same bounds
...
$\P_{l_b,u_b}$, which are large when points satisfying $u_i+v_i>2l_b$ and 
$u_i+v_i<2u_b$ cluster close to the line $v=u$, $\D2_{l_b,u_b}$ is 
then small. 
Thus large values of $\cor_{l_b,u_b}$ and $\P_{l_b,u_b}$ indicate strong 
...
and $\cor_u=\cor_{0.5,1}$ ($u$ is for "upper"); likewise $\P_l=\P_{0,.5}$,
$\P_u=\P_{0.5,1}$, $\D2_l=\D2_{0,0.5}$, and $\D2_u=\D2_{0.5,1}$.

The difference $\cor_l-\cor_u$ is expected to be positive if 
dependence in the left halves of the distributions is stronger 
than dependence in their right halves; likewise for $\P_l-\P_u$.
This holds, for instance, for the Clayton copula (REF).
<!--Shya to cite the Nelson book-->
On the other hand, the difference $\D2_u-\D2_l$ is expected to be 
positive under the same conditions, so we use these three differences
to detect asymmetry of tail dependence.
