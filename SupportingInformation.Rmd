...
{
  frac<-NA*numeric(length(corstats_d))
...
pdf(file="CNdata_NonparamTailDep.pdf",width=totwd,height=totht)
  
#plot kendall results on the left panels, spearman on right,
#cor in top panels, then D2, then P on bottom panels
...
text(xlimits[1],ylimits_D2[1],labels='C',cex=1.5,adj=c(.5,0))
...
text(xlimits[1],ylimits_D2[1],labels='D',cex=1.5,adj=c(.5,0))
...
text(xlimits[1],ylimits_P[1],labels='E',cex=1.5,adj=c(.5,0))
...
text(xlimits[1],ylimits_P[1],labels='F',cex=1.5,adj=c(.5,0))
fracplot(Pstats_frac_S,ylimits_P)
vlineplot(x,ylimits_P)

dev.off()
#DAN: have to make the plot actually appear in the Rmd output, with a caption and everything!
```
