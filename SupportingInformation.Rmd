...
```{r stat_testing_save_result, message=F, warning=F, echo=F, cache=T, cache.extra=list(seed,mtime("TestStats_multivar_dataset.R"),mtime("CopupulaFunctions.R"),mtime("CopulaFunctions_flexible.R"))}
set.seed(seed)
...
<!--Shya, currently your plotter function displays a p-value for each point for the
middle two columns of panels, but only a p-value for the left-most point for the left 
column of panels and for the right-most column. Can you please make the left- and right-most
columns follow the same format at the middle two columns, i.e., they need to have a right axis label from 0 to 1 showing p-value and then show all the p-value points for all values
of Spearman's rho. -->
...
<!--Eventually we will need to add methods on how we actually use these stats, which is different for multivariate and bivariate data. For bivariate data, it involves surrogates, for multivariate data it involves resampling locations. Dan will have to insert descriptions of the surrogate and resampling methods.-->
