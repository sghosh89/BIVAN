...
# Transformations used in Fig. \ref{MT-fig_cop_pedag1} \label{pedagog1_details}

The data of Fig. \ref{MT-fig_cop_pedag1}A-B were generated to have standard normal marginals.
Fig. \ref{MT-fig_cop_pedag1}C can be obtained from Fig. \ref{MT-fig_cop_pedag1}A via transformation
of both variables by the cdf of the standard normal distribution, and then by the inverse of 
the cdf of the gamma distribution with shape and scale parameters 2. 
The function `qgamma` in R was used.
Fig. \ref{MT-fig_cop_pedag1}D can be obtained from Fig. \ref{MT-fig_cop_pedag1}B in the same way.
Fig. \ref{MT-fig_cop_pedag1}E can be obtained from Fig. \ref{MT-fig_cop_pedag1}A via transformation
of both variables by the cdf of the standard normal distribution.
Fig. \ref{MT-fig_cop_pedag1}F can be obtained from Fig. \ref{MT-fig_cop_pedag1}B in the same way. 
...
additional details???** on greenhouse gas flux methods for this site. We considered 13 locations that had at least 50 dates data.
...
```
<!--***DAN: These names differ from those used in Emma's 2016 Glob Change Biol paper.
Need to check with Chris Reid. Have to get this right.-->
...
We here define $\Ps_{l_b,u_b}$, using the bounds
...
in these families [@nelsen2006_copula]. C copulas are known to have stronger lower- than 
upper-tail dependence, and SC copulas, being rotations of C copulas by 
$180^\circ$, have stronger upper- than lower tail dependence [@nelsen2006_copula].
Using the `iRho` function from the `copula` package, for each of the four families N, 
F, C, and SC, we obtained parameters for which the expected value of 
...
statistics and their behavior on the small data sets which are common in some fields
...
C copulas, negative for SC copulas (except for the case of $\rho=0$, a 
...
\caption[Testing nonparametric statistics with 35 data points]{Results of tests of whether $\cor_l-\cor_u$, $\Ps_l-\Ps_u$, and $\Dsq_u-\Dsq_l$ reveal the known asymmetry of tail dependence of the C and SC copulas, and the known symmetry of tail dependence of the N and F copulas. See Appendix  \ref{Test_npa_stat} for details. $n=35$. \label{fig_stat_testing35}}
\end{center}
\end{figure}
...
\caption[Testing nonparametric statistics with 1000 data points]{Results of tests of whether $\cor_l-\cor_u$, $\Ps_l-\Ps_u$, and $\Dsq_u-\Dsq_l$ reveal the known asymmetry of tail dependence of the C and SC copulas, and the known symmetry of tail dependence of the N and F copulas. See Appendix  \ref{Test_npa_stat} for details. $n=1000$.\label{fig_stat_testing1000}}
\end{center}
\end{figure}
...
For bivariate data sets, surrogates were produced as follows.   
...
- The surrogate data set is $(x_{\sigma_x(i)},y_{\sigma_y(i)})$ for $i=1,\ldots,n$. 
...
Kendall correlation of the surrogate data set will be the same as that of the $a_i$ and $b_i$, and therefore 
...
N-copula null hypothesis, and answering question Q1 from the Introduction.
...
<!--***DAN: text all good up to here, except as marked (the data section). Also
all figs and tables relevant to the stuff this far have been edited and look pretty
good. This is as of 2018 01 10, 1255pm. So start editing from this point.-->
...
knitr::kable(h, #longtable = T, 
      ...
      caption = "Results of fitting our 16 copula families to the soil C and N dataset. Copula name abbreviations are given in the main text. AICw = AIC weight; BICw = BIC weight; LT = the lower-tail dependence statistic for the indicated copula family, with fitted parameters; UT = the same for upper-tail dependence.\\label{tab_soilCNfit}",
      booktabs = TRUE)
```
...
#tab_soilCN_asym[,1]<-c("corFS-corLS","PFS-PLS","D2LS-D2FS")
knitr::kable(tab_soilCN_asym, booktabs = T, escape=F,
             format = "latex",linesep = "",
      caption.short = "Nonparametric asymmetry tests against an N-copula null, soil C and N",
      caption = "Soil C and N dataset results for nonparametric tests of whether asymmetry of tail dependence was significant, compared to an N-copula null hypothesis. The values of the listed statistics (cor$_{FS}$-cor$_{LS}$ = $\\cor_{0,0.1}-\\cor_{0.9,1}$, P$_{FS}$-P$_{LS}$ = $\\Ps_{0,0.1}-\\Ps_{0.9,1}$, D$_{LS}^2$-D$_{FS}^2$ = $\\Dsq_{0.9,1}-\\Dsq_{0,0.1}$; here FS stands for `first slice' and LS for `last slice') for real data were compared to their values for 1000 Kendall- or Spearman-preserving N surrogates, in seperate comparisons. A table entry $<X$ indicates the value of the given statistic on the data was less than its value on $X$ of the surrogates, so entries of the form $<X$ for $X$ equal to $975$ or above indicate that upper-tail dependence was significantly stronger than lower-tail dependence.\\label{tab_soilCN_asym}")
```
...
knitr::kable(h,
             ...
             caption = "Results of fitting our 16 copula families to the bird body mass and BMR dataset. Analogous to Fig. \\ref{tab_soilCNfit}, see caption of that figure for details. \\label{tab_bmr_birds_fit}",
             booktabs=T)
```
...
\caption[Nonparametric tests against an N-copula null, bird body mass and BMR data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for bird body mass and BMR data. Same format as Fig. \ref{fig_soilCN_nonparam}, see caption of that figure. \label{fig_bmr_birds_nonparam}}
\end{center}
\end{figure}

```{r tab_bmr_birds_asym, echo=F, results="markup"}
tab_bmr_birds_asym<-make_table_stat_fn(data=stat_birds_ga)
#tab_bmr_birds_asym[,1]<-c("corFS-corLS","PFS-PLS","D2LS-D2FS")
knitr::kable(tab_bmr_birds_asym,
             format="latex",linesep = "",escape=F,
             caption.short = "Nonparametric asymmetry tests against an N-copula null, bird body mass and BMR",
             caption = "Bird body mass and BMR dataset results for nonparametric tests of whether asymmetry of tail dependence was significant, compared to an N-copula null hypothesis. Same format as Table \\ref{tab_soilCN_asym}, see caption of that figure.\\label{tab_bmr_birds_asym}",
             booktabs=T)
```
...
knitr::kable(h,
             ...
             caption = "Results of fitting our 16 copula families to the mammal body mass and BMR dataset. Analogous to Fig. \\ref{tab_soilCNfit}, see caption of that figure for details. \\label{tab_bmr_mammals_fit}",
             booktabs=T)
```
...
\caption[Nonparametric tests against an N-copula null, mammal body mass and BMR data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for mammal body mass and BMR data. Same format as Fig. \ref{fig_soilCN_nonparam}, see caption of that figure. \label{fig_bmr_mammals_nonparam}}
\end{center}
\end{figure}

```{r tab_bmr_mammals_asym, echo=F, results="markup"}
tab_bmr_mammals_asym<-make_table_stat_fn(data=stat_mammals_ga)
#tab_bmr_mammals_asym[,1]<-c("corFS-corLS","PFS-PLS","D2LS-D2FS")
knitr::kable(tab_bmr_mammals_asym,
             format="latex",linesep = "",escape=F,
             caption.short = "Nonparametric asymmetry tests against an N-copula null, mammal body mass and BMR",
             caption = "Mammal body mass and BMR dataset results for nonparametric tests of whether asymmetry of tail dependence was significant, compared to an N-copula null hypothesis. Same format as Table \\ref{tab_soilCN_asym}, see caption of that figure. \\label{tab_bmr_mammals_asym}",
             booktabs=T)
```
...
knitr::kable(h,
             ...
             caption = "Results of fitting our 16 copula families to the Cedar Creek dataset. Analogous to Fig. \\ref{tab_soilCNfit}, see caption of that figure for details. \\label{tab_cc2000fit}",
             booktabs=T)
```
...
\caption[Nonparametric tests against an N-copula null, Cedar Creek data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for Cedar Creek data. Same format as Fig. \ref{fig_soilCN_nonparam}, see caption of that figure.\label{fig_cc2000_nonparam}}
\end{center}
\end{figure}

```{r tab_cc_asym_2000, echo=F, results="markup"}
tab_cc_asym_2000<-make_table_stat_fn(data=stat_cc_ga$`2000`)
#tab_cc_asym_2000[,1]<-c("corFS-corLS","PFS-PLS","D2LS-D2FS")
knitr::kable(tab_cc_asym_2000,
             format="latex",linesep = "",escape=F,
             caption.short = "Nonparametric asymmetry tests against an N-copula null, Cedar Creek data",
             caption = "Cedar Creek dataset results for nonparametric tests of whether asymmetry of tail dependence was significant, compared to an N-copula null hypothesis. Same format as Table \\ref{tab_soilCN_asym}, see caption of that figure.\\label{tab_cc_asym_2000}",
             booktabs=T)
```
...
knitr::kable(RES_aphid_count$info_ord_copname[,,1], #longtable = T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for green spruce aphid abundance data. Copulas 
considered were the normal, Clayton, Gumbel, Frank, Joe, BB1, BB6, BB7, BB8, survival Clayton, survival Gumbel, 
survival Joe, survival BB1, survival BB6, survival BB7, and survival BB8 copulas, with abbreviations in the methods for Q1. NA occurs along the diagonal of the matrix because the diagonal represents a sampling location compared to itself.\\label{tab_count_bestcop}",
             booktabs=TRUE
...
knitr::kable(normal_cop_AICw_matrix(r=RES_aphid_count),
             format='latex',linesep = "",digits=3,
             caption.short = "AIC weights, N copulas, green spruce aphid abundance data",
             caption = "$\\text{AIC}_{\\text{w}}$ for the N copula family for each location pair for green spruce aphid abundance data, for comparison with Table \\ref{tab_count_AICw}. \\label{tab_count_AICw_normal}",
             booktabs=TRUE
             )
```
...
<!--Do the main computations for nonparametric analysis of aphid count data-->
...
knitr::kable(RES_aphid_ff$info_ord_copname[,,1], #longtable = T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for leaf-curling plum aphid first flight data. Copulas considered were as in Table \\ref{tab_count_bestcop}, with abbreviations in the methods for Q1. NA occurs along the diagonal of the matrix because the diagonal represents a sampling location compared to itself.  \\label{tab_ff_bestcop}",
             booktabs=TRUE
             )
```
...
knitr::kable(normal_cop_AICw_matrix(r=RES_aphid_ff),
             format='latex',linesep = "",digits=3,
             caption.short="AIC weights, N copulas, leaf-curling plum aphid first flight data",
...
<!--Do the main computations for nonparametric analysis of aphid ff data-->
...
knitr::kable(RES_plankton_north_sea$info_ord_copname[,,1], #longtable=T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for \\textit{Ceratium furca} abundance data. See methods for Q1 for copula abbreviations. Indep.=Independent. NA occurs along the diagonal because diagonal entries represent a sampling location compared to itself.\\label{tab_pns_bestcop}",
             booktabs=TRUE
             )
```
...
knitr::kable(h,
             format="latex",linesep = "",
             digits=3,
             caption.short="AIC weights, N copulas, \\textit{Ceratium furca} abundance data",
             caption = "$\\text{AIC}_{\\text{w}}$ for the N copula family for each location pair for \\textit{Ceratium furca} abundance data, for comparison with Table \\ref{tab_pns_AICw}.\\label{tab_pns_AICw_normal}",
             booktabs=TRUE)
```
...
<!--Do the main computations for nonparametric analysis of plankton data-->
...
knitr::kable(RES_methane$info_ord_copname[,,1], #longtable=T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for methane flux data. See methods for Q1 for copula abbreviations. Indep.=Independent. NA occurs along the diagonal because diagonal entries represent a sampling location compared to itself.\\label{tab_methane_bestcop}",
             booktabs=TRUE
             )
```
...
knitr::kable(h,
             format="latex",linesep = "",
             digits=3,
             caption.short="AIC weights, N copulas, methane flux data",
             caption = "$\\text{AIC}_{\\text{w}}$ for the N copula family for each location pair for methane flux data, for comparison with Table \\ref{tab_methane_AICw}.\\label{tab_methane_AICw_normal}",
             booktabs=TRUE)
```
...
\vspace{-0.4 cm}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Spearman_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Kendall_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Pearson_vs_Kendall's Tau.pdf}\\
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Corl_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Coru_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Corl-Coru_vs_Kendall's Tau.pdf}\\
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Pl_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Pu_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_Pl-Pu_vs_Kendall's Tau.pdf}\\
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_D2u_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_D2l_vs_Kendall's Tau.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results_ar1_beta_0.5_Clayton/Clayton_D2u-D2l_vs_Kendall's Tau.pdf}
\caption[Comparison of copula structure of noise and model output, AR(1) model,
C copula.]{Comparison of copula structure of noise inputs and model 
outputs, AR(1) model, C copula. Red and blue points give 
means over 50 replicate simulations of the statistics listed on the left axes, 
red points are values for noise inputs and blue points are for
model outputs. Error bars are standard errors. 
$P$-values (purple points, right axis) are for a 
paired $t$-test of the null hypothesis that the distributions of 50 noise and 
output values have the same mean; $p$-values less than $0.05$ indicate that 
copula structure as measured by the listed statistic differs significant
between noise inputs and model outputs. See methods section for Q2 
for additional details.\label{fig_Cause4copula_C_kend_ar1}}
\end{center}
\end{figure}
<!--***DAN: Shya, can you please change the top legend of this and the similar-format
figures so instead of saying "noise copula" for the red points and "population copula" 
for the blue points it says "input noise copula" for the red points and "model output 
copula" for the blue ones? This is because one of our models is much more general than
just a population model, so I have changed the langauge in the text to reflect that.
Please be aware that when you make this change it will mean the next knit will take 
several hours, unless you go to the effort of reorganizing your code to put the sims
and the plots in separate chunks. So maybe make this change right before you go home,
so you can knit over night. Also please let me know, so I can knit overnight, too.

At the same time that you do that above, please see if you can fix the alignment
between the bottom three panels of Fig S32 (and subsequent analogous figures) so they
line up with the panels above that.
***Shya: Done.-->
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
SC copula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, SC copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
G copula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, G copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
\ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, see captions of those figures for 
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
SG copula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, SG copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
\ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, see captions of those figures for 
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
J copula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, J copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
\ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, see captions of those figures for 
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
SJ copula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, SJ copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
\ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, see captions of those figures for 
details.\label{fig_Cause4copula_SJ_kend_ar1}}
\end{center}
\end{figure}

<!--adding results for stochastic ricker with weak noise : nsd=0.1-->
\begin{figure}[!h]
\begin{center}
...
model, weak noise, C copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, C copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
see captions of those figures for details.\label{fig_Cause4copula_C_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, weak noise, SC copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, SC copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
see captions of those figures for details. \label{fig_Cause4copula_SC_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, weak noise, G copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, G copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, weak noise, SG copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, SG copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, weak noise, J copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, J copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, weak noise, SJ copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, weak noise, SJ copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
see captions of those figures for details.\label{fig_Cause4copula_SJ_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}

<!--adding results for stochastic ricker with strong noise : nsd=1-->
\begin{figure}[!h]
\begin{center}
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, C copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, C copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
see captions of those figures for details.\label{fig_Cause4copula_C_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, SC copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, SC copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
see captions of those figures for details. \label{fig_Cause4copula_SC_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, G copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, G copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, SG copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, SG copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, J copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, J copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
\caption[Comparison of copula structure of noise and model output, stochastic Ricker 
model, strong noise, SJ copula.]{Comparison of copula structure of noise inputs and model 
outputs, stochastic Ricker model, strong noise, SJ copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
pop<-ans_b_0.1$pop
plot(pop[,1],pop[,2],xlab=expression(paste(E[1])), ylab=expression(paste(E[2])),cex.lab=1.5,cex.axis=1,col="grey",pch=20,cex=0.6)
...
pdf("./Results/asym_sens_results/a_0.2_b_0.9_r_0.8_asym_sens.pdf",height=3,width=6)
op<-par(mfrow=c(1,2),mar=c(3.5,4,1,1),mgp=c(2.2,0.5,0))
pop<-ans_b_0.9$pop
plot(pop[,1],pop[,2],xlab=expression(paste(E[1])), ylab=expression(paste(E[2])),cex.lab=1.5,cex.axis=1,col="grey",pch=20,cex=0.6)
text(0.1,0.95,"A",cex=2)
...
pdf("./Results/asym_sens_results/a_0.2_b_-0.9_r_0.8_asym_sens.pdf",height=3,width=6)
op<-par(mfrow=c(1,2),mar=c(3.5,4,1,1),mgp=c(2.2,0.5,0))
pop<-ans_b_neg0.9$pop
plot(pop[,1],pop[,2],xlab=expression(paste(E[1])), ylab=expression(paste(E[2])),cex.lab=1.5,cex.axis=1,col="grey",pch=20,cex=0.6)
text(0.1,0.95,"A",cex=2)
pop<-ans_b_neg0.9$pop_d
plot(pop[,1],pop[,2],xlab=expression(paste(E[1])), ylab=expression(paste(E[2])),cex.lab=1.5,cex.axis=1,col="grey",pch=20,cex=0.6)
text(0.1,0.95,"B",cex=2)
par(op)
dev.off()
...
rownames(summary_stat)<-c("Best-fit copula AICw","N-copula AICw","Model-avg. LT","Model-avg. UT","p, CvM","p, KS")
knitr::kable(summary_stat, booktabs = T, escape=F,
             format = "latex", linesep = "",
             digits=3, col.names=c("b=0.1, g$_{1}$","b=0.1, g$_{2}$",
                                   "b=-0.1, g$_{1}$","b=-0.1, g$_{2}$",
                                   "b=0.9, g$_{1}$","b=0.9, g$_{2}$",
                                   "b=-0.9, g$_{1}$","b=-0.9, g$_{2}$"),
...
The output of simulations from the second model detailed in 
the methods section for Q2 (i.e., Figs \\ref{MT-fig_ms_asym_b_0.1} and 
\\ref{fig_ms_asym_b_-0.1}-\\ref{fig_ms_asym_b_-0.9}) were subjected to the model 
...
N-copula AICw: the AIC weight of the N copula family; Model-avg. LT: the lower-tail 
...
gtext<-c("b=0.1, g$_{1}$","b=0.1, g$_{2}$","b=-0.1, g$_{1}$","b=-0.1, g$_{2}$","b=0.9, g$_{1}$","b=0.9, g$_{2}$","b=-0.9, g$_{1}$","b=-0.9, g$_{2}$")
tab_stat_npa_asym<-cbind(b=rep(c(0.1,0.1,-0.1,-0.1,0.9,0.9,-0.9,-0.9),each=3),
...
The output of simulations from the second model detailed in 
the methods section for Q2 (i.e., Figs \\ref{MT-fig_ms_asym_b_0.1} and 
\\ref{fig_ms_asym_b_-0.1}-\\ref{fig_ms_asym_b_-0.9}) were subjected to nonparamtric statistical 
...
of the same statistic computed on surrogate time series randomized to have N-copula structure
...
lower-tail dependence compared to an N-copula null hypothesis. `FS' stands for `first slice'
and refers to the bounds 0 to 0.1; `LS' stands for `last slice' and refers to the bounds
0.9 to 1.\\label{tab_stat_npa_asym}")#%>%
 #            kable_styling()%>%
 #            group_rows(index=setNames(c(3,3,3,3,3,3,3,3), gtext))
 #            group_rows("b=0.1, g$_{1}$", 2,4, escape=F) %>%
 #            group_rows("b=0.1, g$_{2}$", 5,7, escape=F) %>%
 #            group_rows("b=-0.1, g$_{1}$", 8,10, escape=F) %>%
 #            group_rows("b=-0.1, g$_{2}$", 11,13, escape=F) %>%
 #            group_rows("b=0.9, g$_{1}$", 14,16, escape=F) %>%
 #            group_rows("b=0.9, g$_{2}$", 17,19, escape=F) %>%
 #            group_rows("b=-0.9, g$_{1}$", 20,22, escape=F) %>%
 #            group_rows("b=-0.9, g$_{2}$", 23,25, escape=F)
```

\begin{figure}[!h]
\begin{center}
\includegraphics[width=12 cm]{./Results/asym_sens_results/a_0.2_b_-0.1_r_0.8_asym_sens.pdf}
\caption{Analogous to Fig. \ref{MT-fig_ms_asym_b_0.1}, but using $b=-0.1$.\label{fig_ms_asym_b_-0.1}}
\end{center}
\end{figure}

\begin{figure}[!h]
\begin{center}
\includegraphics[width=12 cm]{./Results/asym_sens_results/a_0.2_b_0.9_r_0.8_asym_sens.pdf}
\caption{Analogous to Fig. \ref{MT-fig_ms_asym_b_0.1}, but using $b=0.9$.\label{fig_ms_asym_b_0.9}}
\end{center}
\end{figure}

\begin{figure}[!h]
\begin{center}
\includegraphics[width=12 cm]{./Results/asym_sens_results/a_0.2_b_-0.9_r_0.8_asym_sens.pdf}
\caption{Analogous to Fig. \ref{MT-fig_ms_asym_b_0.1}, but using $b=-0.9$.\label{fig_ms_asym_b_-0.9}}
\end{center}
\end{figure}

```{r skewness_aphidcount_allsp_plot, echo=F, results="hide", cache=T, cache.extra=list(seed,data_aphid_count,mtime("good_loclist.R"),mtime("skewness_multivar_dataset.R"),mtime("ncsurrog.R"),mtime("SkewnessAnd3CentMom.R"))}

# For sp =1, sp=4 and sp=6 : kendall preserving surrogates gives warning : Error in ncsurrog: ncov is not positive semidefinite
# also it can't handle ties for "kendall" and code crashes
# you can see those msg by deleting results="hide" and warning=F option in r chunk
...
```{r skewness_aphidff_allsp_plot, echo=F, results="hide", cache=T, cache.extra=list(seed,data_aphid_ff,mtime("good_loclist.R"),mtime("skewness_multivar_dataset.R"),mtime("ncsurrog.R"),mtime("SkewnessAnd3CentMom.R"))}

# For sp =12 and sp=16 : kendall preserving surrogates gives warning : Error in ncsurrog: ncov is not positive semidefinite,
# also it can't handle ties for "kendall" and code crashes
# you can see those msg by deleting results="hide" and warning=F option in r chunk
...
# There is no error msg or warning coming from ncsurrog : ncov semidefinite matrix for any species with spearman or kendall preserving surrogates.
# But there may be error handling ties with "kendall"
# So,for safety, now we will only analyze with "spearman"
...
for(i in splist){
  ...
  if(i==6){
    saveRDS(ans,"./Results/taylorslaw_results/tl_aphid_count_sp6.RDS") #saving sp=6 results
  }
...
```{r spatialTL_on_realdata_aphid_count_sp6, results="hide", echo=F}
#Now for sp=6

ans<-readRDS("./Results/taylorslaw_results/tl_aphid_count_sp6.RDS")

pdf("./Results/taylorslaw_results/aphid_count_sp6_sTL_mncurv.pdf",height=5,width=5)
op<-par(mar=c(4,5,1,1),mgp=c(3,1,0))
hist(ans$good_surrog_mncurv,breaks=100,main="",xlab="Mean curvature",cex.axis=2,cex.lab=2,cex=2,col="grey",border=F)
points(x=ans$real_sTL_mncurv,y=0,col="black",pch=15,cex=2)
...
op<-par(mfrow=c(1,2),mar=c(4,5,1,1),mgp=c(3,1,0))
plot(ans$realTL_stats$s.points,xlab=expression(log[10]("spatial mean")),
     ylab=expression(log[10]("spatial variance")),col="grey",pch=20,cex=1.5,
...
op<-par(mfrow=c(1,2),mar=c(5,5,1.8,0.5),mgp=c(3,1,0))
s<-varying_d(numsims=numsims,numsteps =numsteps,numlocs=5,p0=p0,params=params,
                 ext_thrs=ext_thrs,scl=scl,model=model,disp_everywhere=F,ploton=F)
s1<-varying_d(numsims=numsims,numsteps =numsteps,numlocs=25,p0=p0,params=params,
                 ext_thrs=ext_thrs,scl=scl,model=model,disp_everywhere=F,ploton=F)
plot(s$d_seq,s$risk_left,xlim=c(0,1),ylim=c(0,1),type="b",col="black",
     xlab="d",ylab="Extinction risk",
     cex.lab=2,cex.axis=1.8,cex=1.5,pch=16,lwd=1.5)
lines(s1$d_seq,s1$risk_left,type="b",lwd=1.5,cex=1.2,pch=1)
legend(x=0.2,y=0.9, title=paste0("r = ",params," , left tail dep. in noise"), c("numlocs = 5","numlocs = 25"), 
       pch=c(16,1), lty=c(1,1), cex=1.4, pt.cex=c(1.5,1.2),
       horiz=F, bty='n') 

plot(s$d_seq,s$risk_right,xlim=c(0,1),ylim=c(0,1),type="b",col="grey",
     xlab="d",ylab="Extinction risk",
     cex.lab=2,cex.axis=1.8,cex=1.5,pch=16,lwd=1.5)
lines(s1$d_seq,s1$risk_right,type="b",lwd=1.5,cex=1.2,pch=1,col="grey")
legend(x=0.2,y=0.9, title=paste0("r = ",params," , right tail dep. in noise"), c("numlocs = 5","numlocs = 25"), 
       pch=c(16,1), lty=c(1,1), col=c("grey","grey"), cex=1.4,pt.cex=c(1.5,1.2),
       horiz=F, bty='n') 
...
op<-par(mfrow=c(1,2),mar=c(5,5,1.8,0.2),mgp=c(3,1,0))
s<-varying_d(numsims=numsims,numsteps =numsteps,numlocs=5,p0=p0,params=params,
                 ext_thrs=ext_thrs,scl=scl,model=model,disp_everywhere=T,ploton=F)
s1<-varying_d(numsims=numsims,numsteps =numsteps,numlocs=25,p0=p0,params=params,
                 ext_thrs=ext_thrs,scl=scl,model=model,disp_everywhere=T,ploton=F)
plot(s$d_seq,s$risk_left,xlim=c(0,1),ylim=c(0,1),type="b",col="black",xlab="d",ylab="Extinction risk",
     cex.lab=1.5,cex.axis=1.5,cex=1.5,pch=16,lwd=1.5)
lines(s1$d_seq,s1$risk_left,type="b",lwd=1.5,cex=1.2,pch=1)
legend(x=0.2,y=0.9, title=paste0("r = ",params," , left tail dep. in noise"), c("numlocs = 5","numlocs = 25"), 
       pch=c(16,1), lty=c(1,1), cex=1.4, pt.cex=c(1.5,1.2),
       horiz=F, bty='n') 

plot(s$d_seq,s$risk_right,xlim=c(0,1),ylim=c(0,1),type="b",col="grey",xlab="d",ylab="Extinction risk",
     cex.lab=1.5,cex.axis=1.5,cex=1.5,pch=16,lwd=1.5)
lines(s1$d_seq,s1$risk_right,type="b",lwd=1.5,cex=1.2,pch=1,col="grey")
legend(x=0.2,y=0.9, title=paste0("r = ",params," , right tail dep. in noise"), c("numlocs = 5","numlocs = 25"), 
       pch=c(16,1), lty=c(1,1), col=c("grey","grey"), cex=1.4,pt.cex=c(1.5,1.2),
...
\caption{Analogous to figure \ref{MT-fig_extrisk_lcohen_local} in maintext but with global dispersion.\label{fig_extrisk_lcohen_global}}
\end{center}
\end{figure}
...
using the `iRho` function (or the `iTau function) of the `VineCopula` package
...
# Effects of non-N copula structure on Taylor's law \label{Consequence_TL_simulation}

\noindent Pursuing ideas that sprang from earlier theoretical work [@Reuman2017], we 
tested whether the copula structure of dependence between population variables 
measured in different locations influences the spatial version of Taylor's
law. Taylor's law is an extremely commonly observed and widely studied 
[@Taylor1961, @Taylor1988, @Cohen2013, @Xu2016] empirical pattern that relates the 
...
*slope*. There are several versions of Taylor's law. For spatial Taylor's law,
...
So means and variances are computed across space. Taylor's law has been 
...
agriculture [@Cohen2015,@Reuman2017].
...
were organized into an $N \times n \times 1000$ array representing $1000$ simulations of 
populations measured in $n$ locations over $N$ time steps. 
...
These transformations made gamma distributed the marginal distributions of 
each population sampling location in each simulation. 
...
resulting $N \times n \times 1000$ array as our *N array* for this simulation exercise;
copula structure of dependences between populations in different sampling locations
was normal (N) for the simulations in this array. 

We then considered an $n$-dimensional C copula with parameter chosen so that the 
Kendall correlations between each pair of the $n$ components were the same as those
of the N distribution constructed above. 


the nature of the slope and how it changed if population time series (taken as $50$ years) from different locations (say, $numloc=25$) had some certain type of copula structure (namely, \textbf{C}, \textbf{F} and \textbf{SC}) other than the $\textbf{N}$ormal copula. First, we simulated (N $\times$ n $\times$ nr) dimensional normal copula with co-variance $\rho =0.7$, where $(N=50,n=25,nr=10)$ are the length of time series, number of sampling locations and number of replica for that copula, respectively. Then we generated Kendall-correlation preserving \textbf{C}layton, \textbf{F}rank and $\textbf{S}$urvival \textbf{C}layton surrogates with same dimension of \textbf{N}ormal. With these four different types of copula we computed `log`(spatial variance) and `log`(mean) and plotted in Fig.\ref{fig_spTL}(A-D). We also tested statistically that whether linearity, homoskedasticity is maintained in each copula data set or not and plotted our results along with root mean square error, intercept and slope in Fig.\ref{fig_spTL}(E-I). We did a quadratic regression with each simulated copula for all types and measured quadratic coefficient and mean curvature for the regression line which is plotted in Fig.\ref{fig_spTL}(J-K) (For code see [`copsurrognd.R`](https://github.com/sghosh89/BIVAN/blob/master/copsurrognd.R) and [`TaylorsLawTools.R`](https://github.com/sghosh89/BIVAN/blob/master/TaylorsLawTools.R)).


<!--
Curvature calculations:
***DAN: Shya, I need a reminder how we did this, can you please tell me where the
code is, then I can write this section. Feel free to take a stab at it yourself, too,
though.
***Shya: What do you want to know? how did you calculate mean curvature and 
slope for made up data or you want to see code with real data?

In case of made up data :
For each non-N copula family (C,F,SC) you generated surrogates
having similar correlation (spearman/ kendall, though for 
consistency we added plots in suppmat with spearman) that of N copula
[using copsurrognd.R] : this thing is done in r chunk named "taylors_law_make_surrogs".

Then you did spatial-TL calculation for each set of copula family in r chunk "tl_analysis"
[using TaylorsLawTools.R which calls mncurvat.R code to calculate mean curvature]

Next two r chunks : taylor_plots_kendall and taylor_plots_spearman generates plot with those data.

For spatial TL calculation on real data :

We used Aphid count all sp with spearman only, but to show a plot for example chose sp=6 (corn leaf aphid)
as it had significant p_left value for mean curvature.
code is in r chunk named "spatialTL_on_realdata_aphid_count".
-->

<!--
Stuff on TL cut from the main text
From Q3 methods:
We tested whether the linearity of the spatial Taylor's law relationship was partly
due to the copula structure of the dependences between the populations sampled. 
Using corn leaf aphid abundance data as an example, but also
applying the same analysis to the abundance data we had for 19 other aphid species
(see *Data*), we calculated the curvature (details in Appendix \ref{SM-TL_curvature}) 
of the empirical Taylor's law relationship; Taylor's law hypothesizes that this 
curvature should be close to 0. We then calculated the same curvature quantity for
10000 surrogate datasets, using the same surrogate procedure described above in 
this section, and compared the empirical Taylor's law curvature to the 
distribution of surrogate curvatures using a two-tailed test. If the empirical
curvature value was below the $5^{\text{th}}$ quantile of the surrogate distribution, 
then copula structure contributes to the linearity of spatial Taylor's law 
because randomizing copula structure while retaining other statistical properties
of the data cause the Taylor's law relationship to become significantly more 
curved. 

From Q3 results:
```{r read_tl_aphid_count_sp6, echo=F}
tlsp6<-readRDS("./Results/taylorslaw_results/tl_aphid_count_sp6.RDS")
tl_aphid_count<-readRDS("./Results/taylorslaw_results/tl_aphid_count.RDS")
aphid_info<-readRDS("./Results/aphid_info.RDS")
pleft<-tl_aphid_count$p_left_mncurv
stat<--2*sum(log(pleft))
pchi2<-1-pchisq(stat,40)
idsp<-which(pleft<0.05)
pleftgood<-pleft[idsp]
```

Results also suggested that copula structure contributes to the linearity of 
spatial Taylor's law. For instance, the spatial Taylor's law relationship 
for corn leaf aphid abundance data was significantly less curved than for 
surrogate datasets (Fig. \ref{fig_sTL_mncurv_sp6}). Three of $20$ aphid species
had cuvatures less than at least $95\%$ of surrogate datasets ($p=$ `r pleftgood` for 
the `r aphid_info$c1[idsp]` species, respectively). 
***DAN: Shya, please fill in X, Y, Z, A, B, C with autolinks)
***Shya: Done.
Three such significant
results of 20 species could have occurred by chance due to 
multiple-testing error. However, for each of the 20 species, we computed the fraction
of surrogate curvatures less than the curvature that came from real data, and we
combined these $p$-values using Fisher's combined probability test to get an
aggregate $p$-value of `r pchi2`
***DAN: Shya, see the Wikipedia article on Fisher's method to learn about it. Then 
you need to put this analysis into the code suite and have the resulting p value
typed in above be an autolink instead. Here is what I did: 
1) I copied by hand all the pleft values on the pdf you sent me into a vector pleft. Instead you should create such a vector in your code from your test output, not typing in the results.
2) I used the formula stat<--2*sum(log(pleft)) to get the Fisher method statistic
3) I compared to a chi-squared distribution with 40 degress of freedom using 1-pchisq(stat,40) to get my p-value
Please let me know if any of this is unclear.
***Shya: Done.
for the test of the null hypothesis that 
copula structure did not contribute to linearity of Taylor's law for any of the 
species. This result is usefully suggestive and can guide future work, but still does not
provide certainty, because the Fisher method provides $p$-values that are too 
small if its assumption of independent tests is violated. Though our 20 species
have different host plants so may be independent for the purposes of our analyses, 
it would be difficult to fully assess independence.
***DAN: Shya, can you remind me why the same analyses were not done for plankton,
if there is a good reason? Or if you did it, what were the results?
***Shya : we did it, I have the pdf but I don't have the code where I called to generate the pdf,
As we could not get any significant results for a single species (see attachment in the mail I sent you)
we did not show the results. 
We provide simulation results in Appendix \ref{SM-Consequence_TL_simulation}
that robustly illustrate, at least in the model setup we used, 
that copula structure influences spatial Taylor's law.
...
\caption{Mean curvature of the spatial Taylor's law relationship for 
corn leaf aphid abundance
data (A) was significantly less than for surrogates (B, C). The Taylor's law 
relationship for one example surrogate (B) shows shows greater curvature than
for data (A), and the significance of the difference was demonstrated
by the fact that only `r (tlsp6$p_left_mncurv)*10000` of $10000$ surrogates had curvature less than that
of the data (C). See Appendix \ref{SM-TL_curvature} for details on 
how curvature was calculated.\label{fig_sTL_mncurv_sp6}}
\end{center}
\end{figure}
***DAN: Shya, pls make the x axis label on C read simply "Mean curvature"
***sHYA: dONE.
-->
