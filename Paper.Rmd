...
For all $3$ bivariate-datasets indicated by each column name (SoilCN, BMR, Anolis) 
...
Each dataset presented there, has significant upper tail dependence 
compared to Normal as indicated by negative AIC-weights model averaged 
(lower-upper) tail dependence and very low ranks of actual copula 
data for $\Ps_l - \Ps_u$ statistics, which answers the second and third part of Q1
(For detailed results of each dataset see Tables \ref{SM-tab_soilCN_asym} - \ref{SM-tab_contrasts_67_asym} and 
Figs. \ref{SM-fig_soilCN_nonparam} - \ref{SM-fig_contrasts_67_nonparam}). 
...
methane flux data from $13$ sampling locations). We counted for how many location pair (excluding self-comparison)
copula are non-independent.
