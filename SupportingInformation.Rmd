...
```{r read_plot_BMR_data, echo=FALSE, results="hide"}
set.seed(seed)
...
\caption{\textbf{(A)} Plot of the raw BMR vs. ln(mass) data \textbf{(B)} Copula plot for BMR dataset. Obtained by applying ranks separately to each variable, and then dividing the ranks by $n+1$ where $n=`r nrow(v_BMR)`$ is the number of contrasts (see Sec. \ref{Model_selection}).\label{fig_BMR_raw_cop}}
\end{center}
\end{figure}
...
make_table_stat_fn<-function(data){
 ...
 atdres<-data.frame(Statistic=c("$\\cor_{first.slice}-\\cor_{last.slice}$",
                               "$\\Ps_{first.slice}-\\Ps_{last.slice}$",
                                "$\\Dsq_{first.slice}-\\Dsq_{last.slice}$"),
                   Kendall=rep("",3),
...
```{r cause4copula, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
