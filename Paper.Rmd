...
whenever we currently cite one of those, change it so we cite the combined table instead.
8) The nonparam stats are not estimators of tail dependence, they measure 
...
knitr::kable(h_ord, #longtable = T, 
      ...
      caption = "Model fitting results for soil C and N dataset using 16 copula families: normal (N), Frank (F), Clayton (C), survival Clayton (SC), Gumbel (G), survival Gumbel (SG), Joe (J), survival Joe (SJ), BB1, survival BB1 (SBB1), BB6, survival BB6 (SBB6), BB7, survival BB7 (SBB7), BB8, survival BB8 (SBB8). Copulas are sorted according to their AIC values (2nd column). AICw = AIC weight; BIC = BIC value; BICw = BIC weight; LT = the lower-tail dependence statistic for the indicated copula family, with fitted parameters; UT = the same for upper-tail dependence. AIC, BIC and their corresponding weights are rounded upto 2 decimal place whereas tail dependences are rounded upto 4 decimal places. \\label{tab_soilCNfit}",
      booktabs = TRUE)
```
