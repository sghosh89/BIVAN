...
Fig. \ref{fig_cop_pedag1}, panel A or B), the associated dependence
...
ones. Datasets of the same corelation can have a range of tail dependence 
...
Kendall correlation (to two digits) in all cases. (D-F) Similar to A-C, but
with stronger correlation. Asymmetry of tail dependence (LT-UT) is the same 
within each column of panels.\label{fig_cop_pedag}}
\end{center}
\end{figure}
<!--***DAN: Shya, please reduce it to three panels in each row (the middle one and the most extreme ones), while keeping the panels the same size, and label the panels A-F. Please also get rid of the title over each panel, and express the UT and LT values to two digits, and make the font slightly bigger for those as well as for the axis labels and numbers. Please also remove the 
color - we don't need it and it would cost a lot of money.-->
...
of the distributions that dominante the association is visible in the copula
...
distributions are principally responsible for the association between variables.
...
ecology [@Liebhold2004; @walteretal2017].
...
pest species in different locations are all positively asscoiated and also strongly
...
<!--DAN: made it to here 2018 12 06
edit and add references to the following-->
Though a few examples have been found which shows copula approach to model dependencies in bioinformatics [@Kim2008], medical research [@Emura2016], a short period study of intersite dependence of rainfall data [@Serinaldi2008], a recent study on characterizing and assessing risks in drought management [@She2018] or monitoring changes in precipitation-extremes [@Goswami2018], the multivariate copula approach still, we believe, remains unexplored in ecology or related fields and thus has potential scope to better capture the evolution of ecological time series data. 
 ...
 In this article, we analyzed the dependence structure for several bivariate and multivariate ecological, community and biogeochemical datasets. In addition to a copula based \nameref{Model_selection}, we also carried out a \nameref{NPA} with three different statistics to verify our model selection based results. 
