...
\caption[For definitions of non-parametric statistics]{Schematic diagram supporting definitions of non-parametric tail dependence statistics. The red and blue lines are, respectively, $u+v=2l_b$ and $u+v=2u_b$.\label{fig_stat_image}}
\end{center}
\end{figure}
...
\caption[Non-parametric tests against an N-copula null, bird body mass and BMR data]{Non parametric tests for tail dependence and other deviations from N-copula structure, for bird body mass and BMR data. Same format as Fig. \ref{fig_soilCN_nonparam}, see caption of that figure. \label{fig_bmr_birds_nonparam}}
\end{center}
\end{figure}
...
\caption[Non-parametric tests against an N-copula null, Cedar Creek data]{Non parametric tests for tail dependence and other deviations from N-copula structure, for Cedar Creek data. Same format as Fig. \ref{fig_soilCN_nonparam}, see caption of that figure.\label{fig_cc2000_nonparam}}
\end{center}
\end{figure}
...
\caption[Non-parametric tail statistics versus distance between locations for green spruce aphid abundance data]{Non parametric tail statistics versus great-circle distance, $D$, between sampling locations for green spruce aphid abundance data. \label{fig_aphid_count}}
\end{center}
\end{figure}
...
\caption[Non-parametric tail statistics versus distance between locations for leaf-curling plum aphid first flight data]{Non parametric tail statistics versus great-circle distance, $D$, between sampling locations for leaf-curling plum aphid first flight data. \label{fig_aphid_ff}}
\end{center}
\end{figure}
...
\caption[Non-parametric tail statistics versus distance between locations for \textit{Ceratium furca} abundance data]{Non-parametric tail statistics versus great-circle distance, $D$, between sampling locations for \textit{Ceratium furca} abundance data. \label{fig_pns}}
\end{center}
\end{figure}
...
\caption[Non-parametric tail statistics versus distance between locations for methane-flux data]{Non-parametric tail statistics versus great-circle distance, $D$, between sampling locations for methane-flux data. \label{fig_methane}}
\end{center}
\end{figure}
...
Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=fcode,nsd=nsd,method="spearman",lb=0,ub=0.1,num_keep_last=500,resloc=resloc,
                          params=params,p0=c(0,0),model=model)

Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=fcode,nsd=nsd,method="kendall",lb=0,ub=0.1,num_keep_last=500,resloc=resloc,
                          params=params,p0=c(0,0),model=model)
...
for(insd in nsdlist){
  ...
  # for clayton with spearman and kendall method
    Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=fcode,nsd=insd,method="spearman",lb=0,ub=0.1,num_keep_last=500,resloc=resloc,
                          params=params,p0=p0,model=model)

    Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=fcode,nsd=insd,method="kendall",lb=0,ub=0.1,num_keep_last=500,resloc=resloc,
                          params=params,p0=p0,model=model)
  }
 }
}
...
\caption[AR(1) model : comparing correlations and non-parametric statistic for between noise-copula (\textbf{Clayton}) and population copula for different Kendall's correlation]{AR(1) model : results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from lower tail dependent \textbf{Clayton} copula and of the affected populations for 9 equidistant Kendall's correlation ($\tau$) values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values from paired t-test if $> 0.05$ (above horizontal purple line) indicates exactly same estimate between noise and population copulae. \label{fig_Cause4copula_C_kend_ar1}}
\end{center}
\end{figure}
...
\caption[AR(1) model : comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different Kendall's correlation]{AR(1) model : results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from upper tail dependent \textbf{Survival Clayton} copula and of the affected populations for 9 equidistant Kendall's correlation ($\tau$) values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values from paired t-test if $> 0.05$ (above horizontal purple line) indicates exactly same estimate between noise and population copulae. Last row in the figure compared the linear regression line with the 1:1 green line and if noise and population tail-dependence stats came up with Pearson correlation p-value $< 0.05$, then they are significantly similar.\label{fig_Cause4copula_SC_kend_ar1}}
\end{center}
\end{figure}
...
\caption[AR(1) model : comparing correlations and non-parametric statistic between noise-copula (\textbf{Gumbel}) and population copula for different Kendall's correlation]{AR(1) model: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SC_kend_ar1} but for noise generated from upper tail dependent \textbf{Gumbel} copula. \label{fig_Cause4copula_G_kend_ar1}}
\end{center}
\end{figure}
...
\caption[AR(1) model : comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Gumbel}) and population copula for different Kendall's correlation]{AR(1) model: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SC_kend_ar1} but for noise generated from lower tail dependent \textbf{Survival Gumbel} copula. \label{fig_Cause4copula_SG_kend_ar1}}
\end{center}
\end{figure}
...
\caption[AR(1) model : comparing correlations and non-parametric statistic between noise-copula (\textbf{Joe}) and population copula for different Kendall's correlation]{AR(1) model: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SC_kend_ar1} but for noise generated from upper tail dependent \textbf{Joe} copula \label{fig_Cause4copula_J_kend_ar1}}
\end{center}
\end{figure}
...
\caption[AR(1) model: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Joe}) and population copula for different Kendall's correlation]{AR(1) model: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SC_kend_ar1} but for noise generated from lower tail dependent \textbf{Survival Joe} copula. \label{fig_Cause4copula_SJ_kend_ar1}}
\end{center}
\end{figure}


# Similar tail dependence in population copula as that of noise copula for stochastic Ricker model \label{cause4copula_ricker}

To find whether nature of tail dependence in noise copula preserved for affected population copula or it would be destroyed because of underlying non-linearity in the model, we simulated the following stochastic Ricker model with intrinsic growth rate $r = 0.5$ and carrying capacity $K = 100$ for two population patches ($i = 1, 2$);

\begin{equation}\label{eq.sRicker}
P_{i}(t+1)=P_{i}(t) exp \left[ r \left\{ 1-\frac{P_{i}(t)}{K} \right\} +\epsilon_{i}(t) \right]
\end{equation}

The noise ($\epsilon_{i}$), initially drawn from a bivariate asymmetric copula family, was transformed into a normally distribution with mean zero and standard deviation $0.1$ (considered as weak noise) or $1$ (considered as strong noise), for code see [`GetNoise`](https://github.com/sghosh89/BIVAN/blob/master/Cause4copula.R) function. For weak noise, we found similar tail dependence in noise copula and the resultant population copula from the stochastic Ricker model (Figs. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} - \ref{fig_Cause4copula_SJ_kend_sRicker_nsd_0.1}) as we obtained in linear AR(1) model (see \nameref{MT-Results_Q2} in the main text). But for strong noise the tail-dependence between noise and population copula was not significantly similar (Figs. \ref{fig_Cause4copula_C_kend_sRicker_nsd_1} - \ref{fig_Cause4copula_SJ_kend_sRicker_nsd_1}). We repeated the whole analysis with $r > 1$ (i.e. Ricker model's over-compensatory dynamical regime) and came up with same conclusion that strong noise with non-linearity could disturb the tail-dependence information translated from noise to population copula.
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Clayton}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from lower tail dependent \textbf{Clayton} copula (having normal marginal with standard deviation 0.1) and of the affected populations for 9 equidistant Kendall's correlation ($\tau$) values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values from paired t-test if $> 0.05$ (above horizontal purple line) indicates exactly same estimate between noise and population copulae. Last row in the figure compared the linear regression line with the 1:1 green line and if noise and population tail-dependence stats came up with Pearson correlation p-value $< 0.05$, then they are significantly similar. \label{fig_Cause4copula_C_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Survival Clayton} copula having normal marginal with standard deviation 0.1. \label{fig_Cause4copula_SC_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Gumbel}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Gumbel} copula having normal marginal with standard deviation 0.1. \label{fig_Cause4copula_G_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Gumbel}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from lower tail dependent \textbf{Survival Gumbel} copula having normal marginal with standard deviation 0.1. \label{fig_Cause4copula_SG_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Joe}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Joe} copula having normal marginal with standard deviation 0.1. \label{fig_Cause4copula_J_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Joe}) and population copula for different Kendall's correlation]{Stochastic Ricker model with weak noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from lower tail dependent \textbf{Survival Joe} copula having normal marginal with standard deviation 0.1. \label{fig_Cause4copula_SJ_kend_sRicker_nsd_0.1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with strong noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_C_kend_sRicker_nsd_0.1} but for noise generated from lower tail dependent \textbf{Clayton} copula  having normal marginal with standard deviation 1. \label{fig_Cause4copula_C_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with strong noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SC_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Survival Clayton} copula  having normal marginal with standard deviation 1. \label{fig_Cause4copula_SC_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with strong noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Gumbel}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_G_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Gumbel} copula  having normal marginal with standard deviation 1. \label{fig_Cause4copula_G_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with strong noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Gumbel}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SG_kend_sRicker_nsd_0.1} but for noise generated from lower tail dependent \textbf{Survival Gumbel} copula having normal marginal with standard deviation 1. \label{fig_Cause4copula_SG_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with strong noise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Joe}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_J_kend_sRicker_nsd_0.1} but for noise generated from upper tail dependent \textbf{Joe} copula having normal marginal with standard deviation 1. \label{fig_Cause4copula_J_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
...
\caption[Stochastic Ricker model with weak nonise: comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Joe}) and population copula for different Kendall's correlation]{Stochastic Ricker model with strong noise: similar analysis as depicted in Fig. \ref{fig_Cause4copula_SJ_kend_sRicker_nsd_0.1} but for noise generated from lower tail dependent \textbf{Survival Joe} copula having normal marginal with standard deviation 1. \label{fig_Cause4copula_SJ_kend_sRicker_nsd_1}}
\end{center}
\end{figure}
