...
will tend to decreases skewness of the spatial mean. Skewness of the 
spatial mean corresponds to the "spikiness" of the spatial-mean time series. 
The spatial-mean time series may be the quantity of principal importance for 
...
of the same quantity calculated for each of 10000 surrogate datasets that were 
produced from the empirical data by randomizing
it in a special way to have the copula structure of a multivariate-normal distribution
(Appendix \ref{SM-Surrog_multi}). The surrogate/randomized datasets had exactly the same
marginal distributions (i.e., distributions of values for each sampling location)
as the original data. 
They also had very similar Spearman correlations between pairs of sampling locations
as the original data. But the surrogates had multivariate normal copula structure. 
...
unchanged. Significant differences indicate that copula structure contributed 
to the skewness of the spatial mean, i.e., to its "spikiness."
...
Pursuing ideas that sprang from earlier theoretical work [@Reuman2017], we tested whether
the copula structure of dependence between population variables influences the
spatial version of Taylor's
...
variances of groups, $g$, of population measurements to the means of these 
groups via a power law, $v_g = a \times m_g^b$, or equivalently via a log-log linear 
relationship, $\log(v_g)=\log(a)+b \times \log(m_g)$. Here $b$ is called 
...
So the means and variances in the Tayor's law formula are computed across 
space. Taylor's law has been 
verified in data from hundreds of taxa and has been applied in numerous fields
...
<!--***DAN: Shya,  want to add the extinction risk stuff back in, let's discuss-->
...
<!--***DAN: I want to put back in some extinction risk results. Let's talk about this, please, Shyamolina-->
