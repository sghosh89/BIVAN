...
have significance, as discussed below, but is not captured by 
correlation. 
...
<!--***DANr: reword this para so people don't think the ranks *are* the 
...
dependence). From a data perspective, given 
...
<!--***DANr: Maybe move the below two sentences into the new section on Background-->
...
methods for Q1 below).

The goal of this paper is to explore the potential for 
applications of copulas in ecology. 
<!--***DANr: Maybe emphasize here that we provide some, but not a comprehensive,
intro to copulas - for that go elsewhere. Instead we are trying to determine
to what extent they may be useful in ecology.-->
Our results provide support for the hypothesis that environmental, ecological and evolutionary processes 
generate and transmit nontrivial 
<!--***DANr: nontrivial? what does it mean-->
copula structure and tail dependence, with substantial 
consequences for theoretical and conceptual understanding, for inference, and for 
applications across ecology. The work we present here has begun to test this
hypothesis, and we hope our results stimulate further
...
<!--***DANr: Maybe use the words tail dependence more and the word copula
less in the below? Generally the words "copula structure" may be rubbing some
...
One way we may expect, *a priori*, a copula approach 
...
approach, but may not be revealed by standard techniques. 
...
environmental variable are associated with local phenomena, which differ
...
copula structure to be a common feature of the 
...
<!--***DANr: Need some caveating here to accommodate the people who slavishly
...
distributions are principally responsible for the association between 
variables, and this 
information can then help support or refute alternative causal hypotheses
about which species is driving the dynamics of the other. 

A fourth reason we may expect a copula approach to be important
...
We will explore how details, captured by copulas, of the dependence between 
...
in ecology have copula structure distinct from that of a multivariate 
Gaussian/normal
...
the marginals of a normal distribution. Such 
approaches do not utilize the power of copulas, so Q1 asks whether ecological
...
copula structure and tail dependence? And (Q3) what are the consequences of non-normal copula
structure and tail dependence for our understanding of ecology and for 
applications? We address Q1, in the second through fourth sections of the paper, 
...
We address Q2, in the fifth and sixth sections of the paper, using 
simple models. We address Q3, in the seventh and eigths sections of the paper, using 
both data and models. The ninth section is the Discussion.
...
To our knowledge, non-normal copulas have not been used in ecology. 
...
<!--***DANr: Add a new section above this one called Background on copulas.
...
The definition
The explanation of how ranks relate to estimating the copula parameters
How to estimate carry out fitting (basically, MLE fitting on ranks)
