...
raw_b[303,2]<-0.66  #to convert as numeric
raw_b[,2]<-as.numeric(raw_b[,2])
raw_b<-raw_b[-which(raw_b[,1]<0),]  # omit -ve mass?

b<-log10(raw_b)
colnames(b)<-c("log10M(g)","log10BMR(KJ/h)")
pdf("./Results/BMR_results/birds/raw_birds_BMR.pdf")
op<-par(mar=c(5.1, 4.1, 0.2, 2.1))
plot(b[,1],b[,2],type="p",col="red",xlab="log(Mass)",ylab="log(BMR)",cex.lab=1.5,cex.axis=1.5)
par(op)
dev.off()
...
op<-par(mar=c(5.1, 4.1, 0.2, 2.1))
plot(m[,1],m[,2],type="p",col="red",xlab="log(Mass)",ylab="log(BMR)",cex.lab=1.5,cex.axis=1.5)
par(op)
dev.off()
...
multivar_tab_summary_fn<-function(r,rs){
  ...
  m$`num.LT-UT>0`<-sum(r$LTdep_AICw-r$UTdep_AICw>0,na.rm=T)
...
op<-par(mar=c(5.1, 4.1, 0.2, 2.1))
plot(d1$Dat,d2$Dat,type="p",col="red",xlab="Green-spruce aphid count from location 4",ylab="Green-spruce aphid count from location 11",cex.lab=1.5,cex.axis=1.5)
par(op)
dev.off()
...
op<-par(mar=c(5.1, 4.1, 0.2, 2.1))
plot(d1$Dat,d2$Dat,type="p",col="red",xlab="Leaf-plum curling aphid's first-flight from location 2",ylab="Leaf-plum curling aphid's first-flight from location 5",cex.lab=1.5,cex.axis=1.5)
par(op)
dev.off()
...
if (!dir.exists(paste(resloc,"Cause4copula_stat_results",sep=""))){
  ...
  resloc<-"./Results/Cause4copula_results/Cause4copula_stat_results/"
}
...
Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=4,method="spearman",lb=0,ub=0.1,num_keep_last=500,resloc=resloc)
                          
Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=4,method="kendall",lb=0,ub=0.1,num_keep_last=500,resloc=resloc)

Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=14,method="spearman",lb=0,ub=0.1,num_keep_last=500,resloc=resloc)
               
Plotter_Cause4copula_stat(N=5000,numsim=50,fcode=14,method="kendall",lb=0,ub=0.1,num_keep_last=500,resloc=resloc)
```
...
\caption[Comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different \textbf{Kendall's Tau}]{Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from \textbf{Survival Clayton} copula and of the affected populations for 9 equidistant \textbf{Kendall's Tau} values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values $< 0.05$ (below horizontal dashed line) indicates significantly similar estimate between noise and population copulae. \label{fig_Cause4copula_SC_kend}}
\end{center}
\end{figure}
...
\caption[Comparing correlations and non-parametric statistic between noise-copula (\textbf{Clayton}) and population copula for different \textbf{Spearman's Rho}]{Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from \textbf{Clayton} copula and of the affected populations for 9 equidistant \textbf{Spearman's Rho} values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values $< 0.05$ (below horizontal dashed line) indicates significantly similar estimate between noise and population copulae. \label{fig_Cause4copula_C_spear}}
\end{center}
\end{figure}
...
\caption[Comparing correlations and non-parametric statistic between noise-copula (\textbf{Survival Clayton}) and population copula for different \textbf{Spearman's Rho}]{Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from \textbf{Survival Clayton} copula and of the affected populations for 9 equidistant \textbf{Spearman's Rho} values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values $< 0.05$ (below horizontal dashed line) indicates significantly similar estimate between noise and population copulae. \label{fig_Cause4copula_SC_spear}}
\end{center}
\end{figure}
...
summary_stat<-summary_stat[,c(1:4)]
rownames(summary_stat)<-c("bestcop.AICw","N.AICw","relLTdep.AICw","relUTdep.AICw","p.CvM","p.KS","BS.success.percent")
knitr::kable(summary_stat, booktabs = T, 
             format = "pandoc",
             digits=3,
      #caption.short = "Results for tests of whether asymmetry of tail dependence was significant compared to a normal-copula null hypothesis.",
      caption="summary of model selection results for asymmetric sensitivity : \\textbf{bestcop.AICw} and \\textbf{N.AICw} represent the AIC-weightage for best fit copula and Normal copula, \\textbf{relLTdep.AICw} and \\textbf{relLTdep.AICw} represent relative lower tail and upper tail dependence based on AIC-weightage respectively, \\textbf{p.CvM} and \\textbf{p.KS} are the p-values coming from goodness of fit test (Cramer-von Mises and Kolmogorov-Smirnov statistics based), \\textbf{BS.success.percent} percentage of successful bootstraps in goodness of fit test. Each column summarizes those statistics for each copula plotted in Fig. \\ref{MT-fig_ms_asym_b_0.1}.\\label{tab_summary_stat_asym_sens}")
```
...
pdf("./Results/skewness_results/skewness_aphid_count/aphid_count_allsp_skewness_spearman.pdf",height=20,width=20)
...
pdf("./Results/skewness_results/skewness_aphid_count/aphid_count_allsp_skewness_kendall.pdf",height=20,width=20)
op<-par(mfrow=c(4,5),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
pdf("./Results/skewness_results/skewness_aphid_ff/aphid_ff_allsp_skewness_spearman.pdf",height=20,width=20)
op<-par(mfrow=c(4,5),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
pdf("./Results/skewness_results/skewness_aphid_ff/aphid_ff_allsp_skewness_kendall.pdf",height=20,width=20)
op<-par(mfrow=c(4,5),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
pdf("./Results/skewness_results/skewness_plankton_north_sea/plankton_allsp_skewness_spearman.pdf",height=24,width=24)
op<-par(mfrow=c(4,6),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
pdf("./Results/skewness_results/skewness_plankton_north_sea/plankton_allsp_skewness_kendall.pdf",height=24,width=24)
op<-par(mfrow=c(4,6),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
op<-par(mar=c(3.5,3.5,1.8,0.1), mgp=c(2, 1, 0))
...
mtext(paste0("p = ",p_left),side=3,line=-0.5,col="navyblue",cex = 2)
par(op)
dev.off()
...
mtext(paste0("p = ",p_right),side=3,line=-0.5,col="navyblue",cex = 2)
par(op)
dev.off()
...
\caption[Spatial Taylor's Law results]{Spatial Taylor's Law results: log(spatial variance) vs. log(spatial mean) for $ns=10$ number of (A) Clayton, (B) Normal, (C) Frank, (D) Survival Clayton surrogates; (E) p-value for test of linearity, (F) p-value for test of homoskedasticity, (G) root mean square error, (H) intercept, (I) slope, (J) quadratic coefficient, (K) mean curvature of regression line from first panel-plots. \label{fig_spTL}}
\end{center}
\end{figure}
...
mtext(paste0("p= ",round(ans$p_left_mncurv,3)),side = 3, line=-2, adj=0.5, col="blue")
par(op)
dev.off()
...
plot(ans$realTL_stats$s.points,xlab=expression(log[10]("spatial mean")),
     ylab=expression(log[10]("spatial variance")),col="red",
     cex.axis=1.5,cex.lab=1.5)
mtext(paste0("mean curvature= ",round(unname(ans$realTL_stats$s.stats[7]),3)),side = 3, line=-2, adj=0.5, col="blue")
...
plot(ans_s[[id]]$s.points,xlab=expression(log[10]("spatial mean")),
     ...
     cex.axis=1.5,cex.lab=1.5)
mtext(paste0("mean curvature= ",round(ans$good_surrog_mncurv[id],3)),side = 3, line=-2, adj=0.5, col="blue")
