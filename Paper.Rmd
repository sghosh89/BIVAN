...
the Marine Biological Association. Data pre-processing was similar but not identical to that 
of @Sheppard2017; see Appendix \ref{SM-Data_details} for details.
Community-level data obtained from the Cedar Creek Ecosystem Science Reserve were <!--DAN : check this rephrasing-->
plant aboveground biomass and plant percent cover data (we used this percent cover data to calculate 
Shannon's diversity index) for `r nrow(d_cc[[5]])` plots, each 9m by 9m, as sampled in 
the year 2000 [@Tilman2001; @Tilman2006] <!--actually they collected data in each year of 1996,1997,1998,1999,2000,2007-->. 
...
functioning (methane flux) dataset was  multivariate in this sense (see \ref{tab_data_info}).
...
tab_data_info$Data<-c("Soil C and N","Bird masses and BMR", "Mammal masses and BMR", "Cedar Creek data", 
                      "Green spruce aphid abundance", "Leaf-curling plum aphid first flight","Ceratium furca abundance","Methane flux")
...
knitr::kable(tab_data_info, 
             ...
             caption = "Summary table for data we used, bolds are the multivariate data, rest are bivariates. \\label{tab_data_info}", 
...
rephrase.
***Shya: I used the exact term "vegetation-related drought" they used in the paper @liu2016probabilistic, I think they 
wanted to mean that you can monitor the drought risk by tracking change in NDVI spatial greenness with precipitation or temperature (climatic variable), which can be considered as a proxy indicator of vegetation response to climate change. They also showed vegatation-drought stress analysis, which I also noticed in some other papers too. There are several studies using drought and NDVI data (mainly using correlation, regression) but they reported it as the first study using joint probability for drought-risk assessment. I have highlighted the info which may be important to us and sent you in mail.-->
