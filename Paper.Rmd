...
correspond to potential consequences we examined of non-Normalcopula structure for
...
between the variables. An Normal copula with $p=0.7$ was already introduced (Fig.
...
example pdfs corresponding to bivariate Normalcopulas for various values of $p$.
...
\ref{PedagogFigCopThry}B), is another one-parameter family. In contrast to Normalcopulas, Clayton copulas
...
Clayton family (here abbreviated SC) is the symmetric opposite of the Claytonfamily, showing
...
(@nelsen2006_copula, section 5.4) that $\lambda_L=\lambda_U=0$ for the Normalcopula,
$\lambda_L=2^{-1/p}$ and $\lambda_U=0$ for a Clayton copula with parameter $p>0$,
and $\lambda_L=0$ and $\lambda_U=2^{-1/p}$ for an survival Clayton  copula with parameter $p>0$.
...
\caption[Example Normalcopulas]{Log-transformed pdfs (A-E) and samples (F-J) from example Normalcopulas. 
S is Spearman correlation; $p$ is the value of the parameter for the Normal
family (it is a 
...
copulas. S is Spearman correlation; $p$ is the value of the parameter for the Clayton  family (it 
...
Information Criteria (AIC and BIC). One of the fitted copulas was the N
copula, making possible comparisons of the degree to which the Normalcopula versus
...
minimum-AIC copulas with that of an Normalcopula. We also counted the number of location pairs 
...
location pairs showing stronger lower-tail dependence. We carried out
pairwise bivariate analyses for simplicity and because they were
...
We also used the BB8 copula, which shows zero lower-tail dependence,
and zero upper-tail dependence except for a boundary case for the parameters.
We used the survival BB8 copula, which shows zero upper-tail dependence, 
and zero lower-tail dependence except for a boundary case for the 
paremeters.
...
<!--***DAN: You decided to reverse the decision to use pithy abbreviations for each
copula. Now the format is longer, so this might just annoy people. So this will take
some work to replace all those throughout the main text and sup mat,
including in figures and table and captions - everywhere.-->
...
$\Dsq_{l_b,u_b}$ indicate strong association. 
...
<!--***SHYAMOLINA: Please do the following to help me revise this section, so far only covering changes in the presentation of the bivariate results. Please let me know if any of this is unclear or you want to discuss anything.
1) Replace the pithy abbreviations for the different copulas (N, C, SC, G, etc.) with their full names throughout the main text and sup mat, including in figure and table captions. This will not affect BB1, BB6, etc., but it will affect SBB1, SBB6, etc., which will now be called survival BB1, survival BB6, etc.

>>>> Done. I don't know whether it should be called as a Normal copula or an Normal copula? Now, we use "an".

2) Add AIC and BIC columsn to table S3, and all analogous figures (e.g., S5, S7, S9, etc). Also reduce the number of digits shown after the decimal to 2 for AIC and BIC and 4 for the other columns.
3) Sort the rows of Table S3 (and all analogous tables, e.g., S5, S7, etc.) by AIC, putting the lowest AIC at the top. This should mean the highest AICw is also at the top.


>>>> Added, sorting based on min AIC.

3) Move Table S3 to an appropriate place in the main text. Choose the place based on where it is first cited in the main text. Same for Fig. S21 and Table S4.
7) Combine tables S6, S8, S10 into one table, and fix it so
whenever we currently cite one of those, change it so we cite the combined table instead.
8) Put the cor results only, for Spearman only (or is Kendall more consistent with what we do elsewhere - what do you think?) from tables S4, S6, S8, S10 into Table 2 as a new row.
...
4) Sort the rows of Table S2 (and all analogous tables) AIC, putting the lowest
AIC at the top. This should mean the highest AICw is at the top.
5) move some of the content from appendices S6 and S7 into methods, and move fig S21 and table S4 into main text. These are non-parametric results for
...
Across essentially all our datasets, copula structures were typically non-Normaland 
...
These values are distinct from what an Normal copula would give, namely, 0, answering the second part 
...
both that tail dependence was distinct from that of an Normal copula, and that it 
was asymmetric (Appendices \ref{SM-Q1bivarnonparam}, \ref{SM-surrog_test}, 
Fig. \ref{fig_soilCN_nonparam}, Table \ref{tab_soilCN_asym}). Our 
...
The Normal copula was always a poor fit compared to the highest-AIC-weight copula 
...
Tables \ref{tab_soilCNfit} - \ref{tab_soilCN_asym},  \ref{SM-tab_bmr_birds_fit} - \ref{SM-tab_3biv_npa_summary} and 
Figs. \ref{fig_soilCN_nonparam},  \ref{SM-fig_bmr_birds_nonparam} - \ref{SM-fig_cc2000_nonparam}. 
...
from what an Normal copula would give (i.e., 0), and answering the second
...
better than those of Normal copulas (Table \ref{tab_multivar_summary}, rows 4-5).
...
dt <- tibble(Description = c("1. p-value, independence test",
           ...
           "10. How many times Non-parametric tail asymmetry ($Cor_{FS}-Cor_{LS}$) on given data are less (or greater) than the Spearman-preserving Normal surrogates")
)
...
knitr::kable(tab_bivar_summary, 
             ...
             caption = "Summary of Q1 results for bivariate datasets. The $p$-values (rows 5-6) are for the minimum-AIC copula. Model averaging used for rows 7-9 was based on AIC weights. First 3 bivariate data use the default range for first slice (FS) is (0,0.1) and for last slice (LS) is (0.9, 1), whereas Cedar Creek result uses range (0, 0.25) for FS and (0.75, 1) for LS for the year 2000.\\label{tab_bivar_summary}",
             booktabs=T, linesep = "\\addlinespace",
...
dt <- tibble(
           ...
           "5. Average AIC-weights of Normal copula across pairs",
...
neither of these is an Normal copula.
