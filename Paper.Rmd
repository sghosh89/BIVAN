...
for a few words on multivariate copulas. We present
the number of pairs for which a non-normal copula
was the AIC-best copula. We also computed the model-averaged lower- and
upper-tail statistics for each pair of locations, and computed the
mean.  


We used a spatial resampling scheme 
...
Copula structures were often non-normal and 
...
Often, either lower- and upper-tail dependence statistics differed substantially from 0 
...
than lower-tail dependence, and the last dataset had the reverse, though non-sgnificantly. Our 
generally affirmative, empirically based answer to Q1 for the bivariate datasets is 
represented in Fig. \ref{MasterFigure} as solid outlines around the names of the soil C and N and bird and mammal body mass versus metabolic rate
...
<!--***DAN: to do for dealing with multivariate results
1) Here is all you want to present for each system
a) number of total pairs and nonindep pairs
b) number of pairs for which non-N was best; skip AICw values, i.e., rows 4 and 5 of Table 5
c) number of well-fitting locs
d) also keep rows 7-9 of table 5

Get rid of the statement in the methods that says we should not try to combine
model selection results with p-values

Delete the following stuff from sup mat:
Figures S24-27
Tables S7-14, keep Table S15
Tables S16-23, keep S24
S25-32, keep S33
S34-42, keep S42

You will have to go back to the methods and revise those parts
-->
...
dt <- tibble(Description = c("1. p-value, independence test",
           "2. Minimum-AIC bestfit copula",
           "3. AIC for bestfit copula",
...
tab_multivar_summary<-rbind(tab_multivar_summary[c(1:3,5,6),],
...
dt <- tibble(
           ...
           #"4. Avg. of normal-AICw minus bestfit-copula AICw across location pairs",
           "4. Average of AIC differences (normal minus bestfit-copula) across location pairs",
           ...
           "9. 2.5$^{th}$ and 97.5$^{th}$ quantiles of $<$P$_{l}$ - P${_u}$$>$ across all location pair")
           )

tab_multivar_summary<-cbind(dt,tab_multivar_summary)

knitr::kable(tab_multivar_summary[1:8,], 
