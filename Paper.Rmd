---
title: "Complex, informative, and previously ignored dependencies propogate throughout ecology and can be revealed using copulas"
...
The importance of a complete copula description of dependence between variables for ecology and related fields

The importance of a complete description of dependence between variables for ecology and related fields

Complex, informative, and previously ignored dependencies propogate throughout ecology and are revealed using copulas

Copulas reveal complex, informative, and previously ignored dependence
structures between variables across ecological disciplines

Copulas reveal complex, informative, and previously ignored dependence
structures which in turn illuminate multiple ecological disciplines

Copulas reveal complex, informative, and previously ignored dependence
structures which in turn can illuminate every subdiscipline of ecology

Copulas reveal complex, informative, and previously ignored dependence
structures which in turn can illuminate every subfield of ecology

Copulas reveal complex, informative and previously ignored dependence
structure between variables throughout ecology

The complexity of dependencies between ecological variables, previously
ignored, is revealed by copulas and in turn illuminates multiple 
disciplines of ecology

\noindent \emph{Keywords:}

\noindent dependence, regression, correlation, copula, population, community

\noindent \emph{Type of Article:}

\noindent Ideas and Perspectives

\noindent \emph{The number of words in the abstract:}

\noindent 

\noindent \emph{The number of words in the main text (excluding abstract, acknowledgements, references, table and figure legends):} 

\noindent \emph{The number of words in each text box:}

\noindent No text boxes

\noindent \emph{The number of references:}

\noindent \emph{The number of figures, tables, and text boxes:}

\noindent X figures, X tables, 0 text boxes

\noindent \emph{The name and complete mailing address (including telephone and fax numbers and e-mail address) of the person to whom correspondence should be sent:} 

\noindent Daniel Reuman, 2101 Constant Ave, Lawrence, KS, 66047, reuman@ku.edu, 626 560 7084. 

\newpage

# Abstract

All branches of ecology study relationships among environmental and biological variables. However, ubiquitously used approaches to studying such relationships, based on correlation and regression, provide only a small slice of the complex information contained in the relationships. Other statistical tools exist that provide a complete description of relationships between variables, based on the concept of the *copula*; they are applied in finance, neuroscience and other fields, but not in ecology. We here explore the concepts that underpin copulas and examine the potential for those concepts to improve our understanding of ecology. We find that informative copula structure in dependences between variables is exceedingly common across all the environmental, species-trait, phenological, population, community, and ecosystem functioning datasets we considered. Many datasets exhibited asymmetric tail dependence, whereby two variables are more strongly related in their left compared to right tails, or vice versa. Copula structure and tail dependence can help reveal causal relationships between variables. We describe mechanisms by which observed copula structure and tail dependence can arise in ecological data, including a Moran-like effect whereby dependence structures between environmental variables are inherited by ecological variables; and asymmetric or nonlinear influences of environments on ecological variables, such as under Liebig's law of the minimum. We describe consequences of copula structure, including impacts on extinction risk and the stability through time of ecosystem services. By documenting the importance of a complete description of dependence, advancing conceptual frameworks, and demonstrating a powerful approach, we aim to elevate copula reasoning to widespread use in ecology.
...
such relationships are limited - standard 
...
have profound ecological significance, as discussed below, but is not captured by 
correlation. 
...
of density functions of Clayton (C) and survival-Clayton 
(D) copulas with parameter 5 (see section on methods for OH), together
...
density function of a normal copula (methods for OH) with 
parameter chosen to
...
show two bands) can be computed 
for any band (methods for OH) to describe how dependence
...
applied widely in finance, neuroscience, bioinformatics and other fields, 
...
$n+1$; and $v_i$ is defined in the same way but using the $y_i$. The $u_i$ and 
...
Mathematically, if $(X,Y)$ is a continuous random
...
This random variable takes values in $[0,1] \times [0,1]$, and has 
uniform marginal distributions. Probability density 
functions (pdfs) corresponding to the copulas associated 
with Fig. \ref{fig_cop_pedag1}C-D are superimposed on those panels. 
...
broader application of copulas in ecology. 
...
consequences for theoretical and conceptual understanding and for 
applications across ecology; analyzing copula structure 
provides more detailed tests of ecological theories and more revealing 
desciptions of ecological datasets. This is a very broad hypothesis, and 
even the substantial empirical and theoretical
evidence we assembled here has only begun to test it. So we propose the hypothesis
principally for further evaluation by the research community.
The overall hypothesis is examined by assessing several more specific 
...
in the text to refer to parts of the figure. CaH="causal hypothesis" (a hypothesis about 
causes of copula structure), CoH="consequential hypothesis" (a hypothesis about 
consequences of copula structure) and OH="observational hypothesis" (a hypothesis about 
copula structure of data).\label{MasterFigure}}
\end{center}
\end{figure}
...
The portion of the distributions that dominate the association is visible in the copula
approach, but not via standard techniques. We therefore frame "causal hypothesis" H1: 
...
will commonly show tail dependence. This is labeled CaH1 in Fig. \ref{MasterFigure};
we use the term "causal hypothesis" because it is about a prospective cause of 
copula structure.

A second reason we may expect a copula approach to benefit 
...
X and Y are strong competitors, abundances of the two species
across quadrats should be negatively related, as in 
Fig. \ref{fig_cop_pedag1}E-F. If X is plotted on the horizontal axis
and Y on the vertical, then Fig. \ref{fig_cop_pedag1}E 
would suggest X is the dominant 
competitor: when X is abundant, Y is necessarily rare because it is suppressed; 
whereas when X is rare, Y
may be abundant, or may also be relatively rare due to 
limiting factors other than X. 
Alternatively, Fig. \ref{fig_cop_pedag1}F suggests that Y
...
frame causal hypothesis H2 (CaH2 on Fig. \ref{MasterFigure}): copulas of abundances
through time or space of two competing or otherwise interacting species will
reflect the asymmetries in the interaction. 
...
causal hypothesis 3 (CaH3 on Fig. \ref{MasterFigure}): Moran effects
...
common features of spatial synchrony in ecological variables. We will later
frame CaH4, which concerns character evolution.
...
We therefore frame "consequential hypothesis" H1 (CoH1 on Fig. \ref{MasterFigure}):
...
ecology and human concerns. We use the term "consequential hypothesis"
because it is about a prospective consequence of copula structure.
...
correlation were unchanged, would have much more benign effects. We will
later frame CoH2, about extinction risk, and CoH3, about Taylor's law.

Our causal and consequential hypotheses imply, or rely on, an "observational
hypothesis," OH, which we here frame: datasets in ecology will commonly have copula structure
...
show asymmetric tail dependence. We use the term "observational hypothesis"
because it pertains to what copula structures will be observed in data. 

Because OH bears upon all the causal
and consequential hypotheses, we test OH first, in sections 2-4 of the paper.
...
copula desription of dependence will be substantial.
...
# Background, concepts and methods for OH: Detecting copula structure in data \label{Methods}

To make results interpretable
to unfamiliar readers, we first introduce the copula families we used, and 
measures of tail dependence. 
Conceptually, a bivariate copula is a probability density function (pdf)
on the unit square with uniform maginals. 
The copula is actually defined as the corresponding 
cdf rather than as the pdf,
but both functions contain essentially the same information. 
The normal copula family (henceforth abreviated N) is a one-parameter
...
tail dependence measures correspond to dependence stronger than a normal
...
positive upper-tail ($0.87$ in Fig. \ref{fig_cop_pedag1}H)
and $0$ lower-tail dependence. For our model selection procedure (see below), we used 
several one- and two-parameter families of copulas exhibiting a variety of 
positive lower-tail dependence values 
(Clayton, survival-Gumbel, survival-Joe, survival-BB6, henceforth
abbreviated C, SG, SJ, SBB6) and upper-tail dependence values
(survival-Clayton, Gumbel, Joe, BB6, henceforth abbreviated 
...
but that had zero tail dependence (BB8, survival-BB8, abbreviated BB8 and SBB8), and
a symmetric copula called 
...
R language. Visualizations of the copulas we used are in Figs \ref{SM-figped_N}-\ref{SM-figped_SBB8}. 
See @nelsen2006_copula, @joe2014_dependence and the `VineCopula` documentation
for more details. 
...
Shown are the normal (N) copula with parameter $0.8$ and the Frank (F) copula with 
parameter $10$. See also Fig. \ref{fig_cop_pedag1}E-H for the Clayton (C) and  
survival-Clayton (SC) copulas with parameter 5. Figs \ref{SM-figped_N}-\ref{SM-figped_SBB8}
have visualizations of all the copulas we used.\label{fig_cop_pedag3}}
\end{center}
\end{figure}

We addressed OH principally via a model selection procedure in which the 
...
since many of the bivariate copula families we considered include the independent copula in the 
family or as a boundary case. If independence could be rejected ($0.05$ significance level), model 
selection proceeded. Third, we fit the bivariate copula families listed previously to the 
normalized ranks via maximum likelihood, using the `BiCopEst` function in `VineCopula`, 
...
and as the final step of
our procedure, we tested the goodness of fit of our AIC-best copula family using a 
bootstrapping procedure (`BiCopGofTest` of `VineCopula`). The method performed 
...
The first part of OH was answered for a bivariate dataset 
by comparing the AIC weights of the non-N copula
families with that of the N family. 
The second part of OH was answered by comparing 
the model-averaged lower- and upper-tail dependence
values to the lower- and upper-tail dependence values of an N copula (i.e., 0). The third part of OH
...
provide greater 
confidence in our answer to OH. We used three 
...
for points between the bounds, small values of 
...
We tested our statistics in Appendix \ref{SM-Test_npa_stat} (see also Figs \ref{SM-fig_stat_testing35}-\ref{SM-fig_stat_testing1000}). All code for this
project is archived at \url{www.github.com/sghosh89/BIVAN}.

# Results for OH: Ecological datasets often have non-normal copula structure and asymmetric tail dependence \label{Results}
...
for birds, (C) the same for mammals, and (D) above-ground plant biomass vs. Shannon's index \textit{(H)} from Cedar Creek. Bottom panels show the corresponding copula plots. See text for units used in upper panels. \label{fig_biv_multi_raw_cop}}
\end{center}
\end{figure}

Across essentially all our datasets, copula structures were typically non-normal and 
commonly showed asymmetric tail 
dependence. To make results easier to absorb: we first 
...
(previous section) to the data and computed AIC and BIC weights and corresponding
...
first part of OH for these data. 
...
of OH. The values also differed substantially 
from each other, answering the third part of OH. The numbers reflect stronger upper- than lower-tail
...
Although these results convincingly show non-normal copula structure, 
...
<!--bivariate summary-->
Table \ref{tab_bivar_summary} summarizes OH results for the bivariate datasets. The two 
...
(Table \ref{tab_bivar_summary}, rows 3-4), answering the first part of OH. 
...
answering the second and third parts of OH. The first three datasets had stronger upper- 
...
We present green spruce aphid abundance results (Table \ref{tab_data_info})
...
larger than N-copula weights (Table \ref{SM-tab_count_AICw_normal}), answering 
the first part of OH. Goodness of fit tests 
...
copula families we used was apparently sufficiently broad that all pairs of 
...
OH. These data have greater tail dependence 
...
than the spatial synchrony of outbreaks. This also answers the third part of OH.
...
Table \ref{tab_multivar_summary} summarizes OH results from
multivariate data. Results supported the 
conclusions that non-N copula structure, non-N tail dependence, and asymmetric tail 
dependence were common. Most location pairs were non-independent 
...
knitr::kable(tab_bivar_summary[1:9,], 
             format="latex",
             caption = "Summary of OH results for bivariate datasets. The $p$-values in rows 5-6 are for the minimum-AIC copula. Model averaging used for rows 7-9 was based on AIC weights. Cedar Creek results are for the year 2000.\\label{tab_bivar_summary}",
             booktabs=T, linesep = "\\addlinespace",
...
<!--Question 2 analyses-->
# Background, concepts and methods for CaH: Causes and mechanisms of non-normal copula structure \label{Causes}

Having demonstrated that non-N copula structure and asymmetric tail dependence are 
common, we addressed CaH by exploring, using 
models, three possible mechanisms. The first mechanism 
...
in two places, may beget similarly 
structured dependence between ecological variables measured through time in those places. 
...
The $\epsilon_i(t)$ represent environmental noise and are 
...
flux of a biogeochemical variable such as methane or CO$_2$, or other quantities.

To explore the importance of nonlinear dynamics for our proposed mechanism, we also considered 
...
For each of the three model setups above, $\tau=0.1,0.2,\ldots,0.9$, and for each of
...
correct that copula structure in environmental noises begets similar dependence 
structure for ecological variables, 
plots should be similar for populations and noises. 

Our second mechanism, also summarized in the 
Introduction,
relates to Liebig's law of the minimum and to nonlinear 
influences of environmental variables on ecological variables. 
If an environmental variable influences an ecological variable
more strongly in one of its tails, we explored whether the ecological variable
could then exhibit asymmetric tail dependence across space. 
Let $E_i(t)$ again be an ecological variable, $i=1,2$. Assume the dynamics 
...
and $(\delta_1,\delta_2)$ have N copula structure, the Moran-like 
mechanism discussed previously does not operate here.
...
non-parametric statistics (methods for OH) to these 
outputs to discover if the model could
produce non-N copula structure and asymmetric tail dependence in the 
$E_i(t)$.

Our third mechanism is evolutionary, and pertains to bivariate 
...
structure: 1) extreme left-tail dependence, 2) moderate left-tail dependence,
...
the impact of copula structure only. For each of the five copula structures, mammalian character
...
# Results for CaH: Moran-like effects and asymmetric dependencies produce non-normal copula structure\label{Results_Q2}

The hypothesis that copula structure in ecological variables can arise through
Moran-like effects was largely confirmed, though similarities between environmental- and 
ecological-variable copula structure can be reduced when dynamics are strongly nonlinear.
...
for this model. The 
AR(1) model and a Clayton copula were used (see methods section for CaH).
...
the nonlinear models described in the methods for CaH.\label{fig_Cause4copula_scatter_C_kend_ar1}}
\end{center}
\end{figure}

For weak noise and using our nonlinear model, copula statistics
...
and simulations, the overall hypothesis was supported that copula structure in the noise translates
...
of tail dependence, to an extent that depended on the value of $b$ used. 
For $b=0.1$ (Fig. \ref{fig_ms_asym_b_0.1}) and $b=-0.1$ (Fig. \ref{SM-fig_ms_asym_b_-0.1}), 
for both $g_1$ and $g_2$,
asymmetric tail dependence was strong; for 
$b=\pm 0.5$, asymmetric tail dependence was weaker but still apparent
...
ecological outputs had non-N copula structure, but did not always reflect the 
asymmetry of tail dependence, perhaps because those methods measured
dependence in the most extreme tails. Results using our nonparametric statistics, however,
...
Our hypothesis was correct that asymmetric tail dependence in evolutionary changes 
...
<!--Question 3 analyses-->
# Background, concepts and methods for CoH: Consequences of non-normal copula structure for our understanding of ecology \label{Consequences}

An idea was presented in the Introduction that
the distribution (through time) of a spatially averaged quantity should be influenced by the 
dependence between the local quantities being averaged, including their
copula structure and tail dependence. For instance, suppose an ecological variable 
$E_i(t)$ is measured at locations $i=1,\ldots,N$ and times $t=1,\ldots,T$, and
the distribution and temporal dynamics of the spatial mean $\sum_i E_i(t)/N$ is of
...
This should tend to increase the skewness of the distribution through time 
of the spatial mean. Similarly, left-tail dependence between local variables
will tend to decrease skewness of the spatial mean. Positive skewness of the 
...
in the spatial-mean have disproportionately large effects.

We tested these ideas using our multivariate datasets (Table \ref{tab_multivar_summary}).
...
via a one-tailed test in the left tail: the $p$-value result was the fraction
...
average to have significantly higher skewness than would have been
expected without tail dependence.

We also examined whether spatial 
tail dependence of an environmental variable could influence the 
...
We tested the validity of this hypothesis using a metapopulation
...
rate was $d$. After each time step, 
if the population density in a patch was below $1$, it was
...
# Results for CoH: Tail dependence influences skewness of spatial average, extinction risk, and Taylor's law
...
\ref{tab_multivar_summary}), the skewness of the spatial average 
was less than (respectively, greater than) a significant fraction of the
surrogate skewness values 
(Fig. \ref{fig_skewness_spearman}A,B). For the datasets with moderate lower-tail 
dependence (\emph{Ceratium furca} abundance and methane data), the skewness of the spatial
...
dispersal. We also showed (Appendix \ref{SM-Consequence_TL_simulation}, \ref{SM-sect:copsurrognd})
that Taylor's law can be strongly influenced by
tail dependence between populations.
...
exceedingly common in datasets across multiple sub-disciplines
and levels of organization in ecology, although no published 
research we are aware of has 
taken this structure into account. Via simple models, we hypothesized three 
mechanisms that may cause non-N copula structures. One
mechanism was via a Moran-like effect, whereby dependencies between environmental variables 
beget similar dependencies between ecological variables in different locations.
A second mechanism produces tail dependence via nonlinear environmental influences
on ecological variables. We discuss below how commonly these and other
potential mechanisms may opperate. 
We also demonstrated important consequences of 
non-N and tail-dependent copula structure in ecology. First, the skewness
of a spatial-average time series is influenced by tail dependencies between its 
constituent time series. Thus right-tail-dependent local time series 
lead to "spiky" spatially averaged time series, with large outbreaks.
These observations could have implications for numerous exploited and pest species,
for which the spatial average time series have applied importance.
Second, metapopulation extinction risk can be accentuated or reduced by tail dependence
properties of environmental variables. We also showed effects of copula 
structure on Taylor's law, which has been applied in many fields.
In our view, our results demonstrate that comprehensive understanding 
of many ecological phenomena is impossible without 
a complete characterization of dependence between variables. 
...
`VineCopula` packages in R); ecologists can start applying these tools
immediately. We created a variety of interrelated randomization procedures
...
Our Moran-like mechanism for copula structure and tail dependence seems likely to
...
If large meteorological events are 
also spatially widespread, then
...
made through time at different locations. Non-extreme values, not associated with extreme, 
spatially extensive weather events, may
be more associated with local phenomena, and therefore 
less prone to being correlated across large areas.
For instance, @Serinaldi2008 examined the spatial dependence 
of rainfall observations in Central Italy. G or Student 2-copulas 
were possible candidates for modelling dependence, and 
neither of these is an N copula.
A long-term study (1950–2014) in the Loess Plateau of China [@She2018]
showed that a G copula (which has upper tail dependence) could effectively model 
the spatial dependence of drought variables in the study area.
Bivariate copula analysis was also used in forecasting the 
co-occurrence of extreme events (flood or drought) 
over the North Sikkim Himalayas, India, using spatial datasets [@Goswami2018]. 

<!--a para on how likely one is to get copula structure via the Liebig mechanism-->
There are also reasons to expect that our second mechanism operates commonly. 
The second mechanism is the one that involves asymmetric or nonlinear influences of 
environmental variables. Liebig's law and the idea of a "limiting 
nutrient" of growth are dominant paradigms in ecology.
Furthermore, innumerable studies document nonlinear influences of environmental
variables on ecological quantities at all levels of organization. 

<!--Holder stuff-->
Our simulations of character evolution suggest the hypothesis that evolutionary
changes through time in bird and mammal BMR and body size may exhibit right-tail dependence,
contrary to standard assumptions of models of character evolution. This is
...
Appendix \ref{SM-sect:missingdata} (also Fig. \ref{SM-fig:missingdata}), 
...
copulas for studies of character evolution.
...
ecology. Copulas probably interrelate with 
...
(e.g., growth, predation avoidance; this is called the principle of 
allocation).
...
to $Y_1$, $Y_2$, $Y_3$ or to some combination. This constitutes 
asymmetric tail dependence between X and $Y_1$. For instance, @Winemiller1992
...
more of the characters being studied then statistical artefacts
may bias conclusions (Appendix \ref{SM-sect:missingdata}, 
...
Other mechanisms that we have not considered probably also operate, 
and should be considered in future work. For instance, 
measurement error may modify copula structure. 
Our models were intentionally simple in other respects, as well, not including phenomena such as 
...
the influence of such interspecific relationships on the 
variability, through time, of total-community
...
may also be of interest because it can characterize the extent to
which community properties show "spikiness". Future work on copula structure of interspecific relationships in communities and its implications for community stability seems likely to be an exciting avenue of research.
...
can influence extinction risk, substantial work remains regarding the 
circumstances under which this result holds, and its magnitude in different scenarios. 
First, we used 
a non-density-dependent model to examine extinction risk. Do similar results pertain
...
IUCN red list of threatened species. Finally, is the copula structure of environmental variables
...
of environmental variables measured in different places. 
