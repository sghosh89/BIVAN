...
If you agree with me, then I will made all "gaussian" as "normal" copula in both main and supmat.-->
...
$(x_i,y_i)$ for $i=1,...,n$ by comparing the AIC weights of the non-Gaussian copula
families with that of the Gaussian family. The same was also done using BIC.
The second part of question 1 was answered by comparing 
the model-averaged lower- and upper-tail dependence
values to the lower- and upper-tail dependence values of a Gaussian copula, both of which 
are 0. The third part of question 1
was answered by comparing the difference of the model-averaged lower- and upper-tail 
dependence values to the analogous difference for a Gaussian copula, again 0.

For multivariate datasets, e.g., the aphid abundance or first flight data, 
each of which consists of time series 
from 10 locations...
...
but had Gaussian copula structure. Thus our comparisons tested the null hypothesis 
that our statistics took values on the empirical data no different from what would have been expected 
if the copula structure of the data were Gaussian, but the data were otherwise 
...
Gaussian null hypothesis, and answering question 1 from the Introduction.
...
\begin{figure}[!h]
\textbf{ \hspace{4.3 cm} (A) \hspace{4.3 cm} (B) \hspace{4 cm} (C) } \\
\vspace{-0.4 cm}
\begin{center}
\includegraphics[width=5 cm]{./Results/SoilCNraw.pdf}
\includegraphics[width=5 cm]{./Results/BMR_results/rawdataplot_lnBMR_vs_lnmass.pdf}
\includegraphics[width=5 cm]{./Results/Anolis_results/contrasts_csv_results/rawdataplot_AVG_ltoe_humerus.pdf}\\
\end{center}
\textbf{ \hspace{4.3 cm} (D) \hspace{4.3 cm} (E) \hspace{4 cm} (F) } \\
\vspace{-0.4 cm}
\begin{center}
\includegraphics[width=5 cm]{./Results/SoilCNcop.pdf}
\includegraphics[width=5 cm]{./Results/BMR_results/copula_lnBMR_vs_lnmass.pdf}
\includegraphics[width=5 cm]{./Results/Anolis_results/contrasts_csv_results/copula_AVG_ltoe_humerus.pdf}
\caption{Upper panel shows raw data plots for \textbf{(A)} soil C and N data, \textbf{(B)} ln(BMR) of contrasts vs. ln(mass) of contrasts, \textbf{(C)} contrasts for AVG.ltoe.IV vs. contrasts for AVG.humerus phylogenetic traits.
Lower panel \textbf{(D - F)} shows copula plots for the corresponding datasets of the upper panel. These copula plots are obtained by applying ranks separately to each variable, and then dividing the ranks by $n+1$ where $n$ is the number of sampling locations for soil C and N data and number of contrasts for last two datasets (see \nameref{Model_selection} of manuscript). \label{fig_biv_raw_cop}}
\end{center}
\end{figure}
...
tab_biivar_summary<-readRDS("./Results/tab_bivar_summary.RDS")
