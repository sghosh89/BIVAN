...
minimum-AIC copulas with that of a normal copula. We also counted the number of location pairs 
...
both that tail dependence was distinct from that of a normal copula, and that it 
...
larger than normal copula weights (Table \ref{SM-tab_count_AICw_normal}), answering the first part of Q1. Goodness of fit tests 
...
from what a normal copula would give (i.e., 0), and answering the second
...
knitr::kable(h_ord, #longtable = T, 
      ...
      caption = "Model fitting results for soil C and N dataset using 16 copula families: copula name abbreviations are given in the main text. AICw = AIC weight; BICw = BIC weight; LT = the lower-tail dependence statistic for the indicated copula family, with fitted parameters; UT = the same for upper-tail dependence. Wights and tail dependences are rounded upto 2 and 4 digits, respectively. \\label{tab_soilCNfit}",
      booktabs = TRUE)
```
