...
available from `BiCopGofTest` in the `VineCopula` package). However, non-parametric statistics verified
...
But our non-parametric statistics confirmed the same conclusions 
...
Non-parametric statistics verified that tail dependence was asymmetric for the green 
...
data showed the reverse. Asymmetry results were verified by non-parametric approaches 
...
seem likely to operate in population systems. The first mechanism 
is an extension of the well known Moran
effect, and was summarized in the Introduction. 
...
$P_i(t+1)=\beta P_{i}(t-1)+ \sqrt{1-\beta^2} \epsilon_i(t)$ where the $\epsilon_i(t)$ are 
standard-normal random variables that are independent for distinct times, $t$, but correlated
