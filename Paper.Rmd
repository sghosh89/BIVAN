---
title: "Complex and informative dependencies propagate throughout ecology and can be revealed using copulas"
...
Complex, informative, and previously ignored dependencies propagate throughout ecology and can be revealed using copulas

The complexity of ecological relationships is revealed by copulas

The true complexity of ecological relationships is revealed by copulas

The true complexity of ecological relationships is only revealed by copulas
...
Copulas reveal complex and informative dependence
structures between variables across ecological disciplines
...
density function of a normal copula (methods for the observational hypothesis) with 
parameter chosen to
...
Our overall hypothesis is examined by assessing several more specific 
...
consequences of copula structure) are labeled with letters X-Z. The "observational 
hypothesis" (i.e., our hypothesis about 
copula structure of data) is labeled with O.\label{MasterFigure}}
\end{center}
\end{figure}
...
hypothesis" (O in Fig. \ref{MasteFigure}), which we here frame: datasets in 
...
Conceptually, a bivariate copula is a pdf
on the unit square with uniform maginals. 
The copula is actually defined instead as the corresponding 
cummulative dstribution function (cdf), but both functions contain the same information.
If $(X,Y)$ is a continuous random
variable and the cdfs of $X$ and $Y$ are 
...
R language. Visualizations of the copulas we used are in 
...
We tested the observational hypothesis (O in Fig. \ref{MasterFigure}) 
...
The third part of the observational hypothesis
...
test the first two parts of causal hypothesis A (Fig. \ref{MasterFigure})
because nonlinear and thresold effects of 
environmental variables on ecological variables are already well documented in
the literature (however, see Discussion material about the green spruce aphid). To test 
the third part of causal hypothesis A, we formulated a model in which 
...
mechanism analyzed below (causal hypotheis B on Fig. \ref{MasterFigure}) 
...
Causal hypothesis B is an extension of the well known Moran
effect, and was summarized conceptually in the Introduction. 
Let $E_i(t)$ again be the ecological variable, $i=1,2$.
...
To explore the importance of nonlinear dynamics for causal hypothesis B, we also considered 
...
Causal hypothesis C was not tested. Our next 
...
data. This proposed mechanism is a hypothetical explanation for the right tail dependence
...
thus confirming the third part of causal hypothesis A for the model we used. 
...
Causal hypothesis B (Fig. \ref{MasterFigure}), i.e. that copula structure 
in the spatial dependence of ecological variables can arise through
Moran effects, was largely confirmed; however, similarities between environmental- and 
...
dependence between the local quantities, including their
copula structure and tail dependence (consequential hypothesis 
X on Fig. \ref{MasterFigure}). 
To refine the hypothesis, suppose an ecological variable 
...
We therefore hypothesize that this 
should tend to increase the skewness of the distribution through time 
of the spatial mean. Similarly, left-tail dependence between local variables
should tend to decrease skewness of the spatial mean. Strong positive skewness of the 
...
We tested consequential hypothesis X 
...
We also examined consequential hypothesis Y on Fig. \ref{MasterFigure}, i.e., that spatial 
...
reducing rescue effects. Here we are assuming, for simplicity, 
...
\noindent Skewness results were generally consistent with consequential 
hypothesis X. For the datasets that
...
Consistent with consequential hypothesis Y, 
...
dispersal. We also substantiated using models (Appendix \ref{SM-Consequence_TL_simulation}, \ref{SM-sect:copsurrognd})
a consequential hypothesis Z (Fig. \ref{MasterFigure}), that 
Taylor's law can be strongly influenced by
...
common in datasets across multiple sub-disciplines
and levels of organization in ecology (Fig. \ref{MasterFigure}, O), although no published 
research we are aware of has 
taken this structure into account. We hypothesized four 
mechanisms (Fig. \ref{MasterFigure}, A-D) that may cause non-N copula structures. One
mechanism (A) was via nonlinear environmental influences
on ecological variables. A second mechanism (B) produces tail dependence via
a Moran effect, whereby dependencies between environmental variables 
beget similar dependencies between ecological variables in different locations. 
We discuss below how commonly these and other
potential mechanisms may opperate. 
We also hypothesized and demonstrated important consequences of 
non-N and tail-dependent copula structure in ecology 
(Fig. \ref{MasterFigure}, X-Z). First (X), the skewness
...
Second (Y), metapopulation extinction risk can be accentuated or reduced by tail dependence
properties of environmental variables. We also showed effects of copula 
structure on Taylor's law (Z), which has been applied in many fields.
In our view, our results make our "overall hypothesis," framed in the Introduction, quite 
reasonable and worthy of broader testing by the research community. We have illustrated 
extensively how such tests might be carried out by performing them ourselves on 
example datasets and models. Further suggestions future possible tests are below. 
<!--***DAN: make sure you have enough of this, Dan-->
It already seems likely that comprehensive understanding 
of many ecological phenomena is impossible without 
a complete copula characterization of dependence between variables. 
...
Our first causal mechanism (Fig. \ref{MasterFigure}, A) seems likely to operate commonly.
Liebig's law and the idea of a "limiting 
nutrient" of growth are dominant paradigms in ecology.
Although we did not systematically test the first two parts of causal 
hypothesis A, many studies document nonlinear influences of environmental
variables on ecological quantities at all levels of organization. 
These parts of A could be tested
straightforwardly for numerous datasets by examing the copulas of environmental and ecological
variables (see below discussion about the green spruce aphid, and 
Appendix \ref{SM-sect:greenspruce}, which illstrate how to do this).

<!--***DAN: stoped here and went back to put in Shya/Lawrence recent results with 
aphid counts and winter temp-->

<!--para on how likely one is to get copula structure via the Moran mechanism-->
There are also reasons to expect that our Moran mechanism (Fig. \ref{MasterFigure}, B) for 
copula structure and tail dependence will
...
as well, not including phenomena such as 
...
<!--***DAN: green spruce aphid all the way from causes to consequences-->
Our hypotheses cover propagation of copula structure from causes, 
to observable data, to consequences for human concerns.
We here take a closer look at the green spruce aphid, which illustrates 
all these stages in one example.
Green spruce aphid abundance, as detected in the suction trap data we use (Data,
Appendix \ref{SM-Data_details}),
is known to be strongly influenced by temperature the previous winter (@Sheppard2016,
supplementary fig. 6).
For each of our 10 sampling locations, we examined (Appendix \ref{SM-sect:greenspruce}, 
Fig. \ref{SM-fig:greenspruce}) the copula of winter temperature
and green spruce aphid abundance time series for the location, finding 
substantial left-tail dependence: apparently winter temperature has an asymmetric
influence on abundance in the sense that cold winters produce low abundances
but warm winters do not yield especially higher abundances than moderate winters
(in passing, this is evidence for the first two parts of causal mechanism A).
