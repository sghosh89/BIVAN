...
```{r cop_pedagog_fig1,echo=F,results="hide"}
set.seed(seed)
...
op<-par(mfrow=c(2,3),mar=c(8,8,1.5,3), mgp=c(5,2,0))
...
for(i in c(1:length(desLTmUT_vector))){
  ...
  plot(c[,1],c[,2],col="grey",xlab="u",ylab="v",cex.lab=3.5,cex.axis=3.5)
  legend("topleft",LETTERS[i],bty="n",cex=4,x.intersp=-0.5)
...
for(i in c(1:length(desLTmUT_vector))){
 ...
 c<-BiCopSim(N=500,obj)
 plot(c[,1],c[,2],col="grey",xlab="u",ylab="v",cex.lab=3.5,cex.axis=3.5)
 legend("topleft",LETTERS[3+i],bty="n",cex=4,x.intersp=-0.5)
 #mtext(paste0("( LT - UT ) = ",round((obj$taildep$lower - obj$taildep$upper),2),sep=""),side=3,cex=2)
  legend("bottomright",c(paste0("KC= ", round(obj$tau,2)),
                                    paste0("LT=",round(obj$taildep$lower,2)),
                                    paste0("UT=",round(obj$taildep$upper,2))),bty="n",horiz = F,cex=2.5)
...
<!--****DAN: Need a 10-panel fig for each one-parameter family showing samples and 
pdfs for 5 values of the parameter. May also need something for the two-parameter
families, but not sure yet how to handle it. After doing the univariate families, then
maybe it is enough to show just the pdf, in which case it can be done in a 
5-by-5 array of panels for the 2-parameter families. That seems like it would work.-->
