...
```{r load_data_Loecke, echo=F, fig.cap="plot of the raw data", fig.height=5, fig.width=5}
...
```{r rankandplot_Loecke, echo=F, fig.cap="Copula plot of soil C/N data",  fig.height=5, fig.width=5}
...
knitr::kable(h, caption = "Fitting info for soilC/N dataset")
```

`r table_nums("soilCNfit",display="c")` shows that several copulas are much better supported than the Gaussian copula, answering the first part of question 1 for these data.
...
```{r stat_soilCN, echo=F, cache=T, cache.extra=list(seed,v)}
set.seed(seed) 
