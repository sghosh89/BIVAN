...
Application of the 
cdfs of the component random variables translates
...
and $p$ in the off diagonal positions. Formulas for the normal copula and all the 
other copula families we use were provided by @Brechmann2013. 
...
described in more detail elsewhere [@Joe1997; @Brechmann2013]; so 
...
180 degrees. 
See @Joe1997 and @Brechmann2013 for details on all these families. We used the implementations
...
tail association can also be explored using, for instance, a regression 
model with heteroskedastic error term, $y=mx+b+\epsilon(x)$. Whereas this 
approach seems capable of yielding insight for some datasets, it implicitly assumes
a particular causal relationship between the variable, an untenable assumption for our
synchrony applications and many other applications. Furthermore, no equivalent
of Sklar's theorem exists under this approach, so information on marginal 
distributions cannot be separated from association information. Sklar's theorem
means that the copula associated with two variables contains all the information
about the association. Such statements of 
mathematical completeness will not be avilable for others methods.
