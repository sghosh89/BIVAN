...
tab_multivar_summary<-readRDS("./Results/tab_multivar_summary.RDS")
tab_multivar_summary<-tab_multivar_summary[c(1:11,16,17),]
row.names(tab_multivar_summary)<-NULL
library(kableExtra)
library(dplyr)
library(tibble)
dt <- tibble(
#Items = c("num.pairs","num.non-indep", "num.non-N", "Avg.best.AICw", "Avg.N.AICw", "num.good.fit", "0.025-CI.LT",
#          "0.975-CI.LT","0.025-CI.UT","0.975-CI.UT","num.LT-UT>0","0.025-CI.Pl-Pu","0.975-CI.Pl-Pu"),
Description = c("1. total number of location pairs (excluding self-comparison)",
           "2. how many location pair have non-independent copula structures",
           ...
           "6. number of location pair having best fit copula with p-values > 0.01
            (both Cramer-von Mises and Kolmogorov-Smirnov method)",
           "7. 2.5 percentile of model-avg. lower tail-dep. from all location pair",
           "8. 97.5 percentile of model-avg. lower tail-dep. from all location pair",
           "9. 2.5 percentile of model-avg. upper tail-dep. from all location pair",
           "10. 97.5 percentile of model-avg. upper tail-dep. from all location pair",
           "11. number of location pair showing stronger lower tail dependence",
           "12. 2.5 percentile of Pl-Pu statistics from all location pair",
           "13. 97.5 percentile of Pl-Pu statistics from all location pair")
)
...
knitr::kable(tab_multivar_summary, 
             ...
             booktabs=T, linesep = "\\addlinespace",
             col.names = c(" ","Green-spruce aphid count",
                        "Leaf-plum curling aphid firstflight",
                        "Ceratium furca count","Methane-flux")
             )%>%column_spec(2:5,width="1.4cm")%>%
             column_spec(1, width = "11cm")%>%kable_styling(latex_options = "basic",position = "center")
```
...
<!--what we did?-->
In the earlier work, most of the studies in ecology focused on measuring correlations between variables and thus ignoring the possibility to recognize importance of asymmetric tail-dependence and non-Normal copulae in this field. We 
