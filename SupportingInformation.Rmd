...
\vspace{-0.4 cm}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results/Survival Clayton_Spearman_vs_Spearman's Rho.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results/Survival Clayton_Kendall_vs_Spearman's Rho.pdf}
\includegraphics[width=5 cm]{./Results/Cause4copula_results/Cause4copula_stat_results/Survival Clayton_Pearson_vs_Spearman's Rho.pdf}\\
...
\caption[Comparing correlations and non-parametric statistic between noise-copula (\textbf{Clayton}) and population copula for different \textbf{Spearman's Rho}]{Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 3 non-parametric stats ($\cor_l - \cor_u$, $\Ps_l - \Ps_u$ and $\Dsq_u - \Dsq_l$) of environmental noise generated from upper-tail dependent \textbf{Survival Clayton} copula and of the affected populations for 9 equidistant \textbf{Kendall's Tau} values in a range (0.1 to 0.9) (Initially $5000$ points are generated but we kept last $500$ points for both noise and population data for comparison). p-values $< 0.05$ (below horizontal purple line) indicates significantly similar estimate between noise and population copulae. \label{fig_Cause4copula_SC_spear}}
\end{center}
\end{figure}
...
plotter_asym_sens_b_varn<-function(mg,ml,a,b,numkeep_last,ploton,resloc){
  ...
  if(ploton==T){
    ...
    pop<-pop_g$pop
    geps<-pop_g$g_noise
    plot(mg[,1],geps[,1],col="red",cex.lab=2,cex.axis=2,xlab=expression(epsilon),ylab=expression(paste(g[1])),main="location=1")
    plot(mg[,2],geps[,2],col="red",cex.lab=2,cex.axis=2,xlab=expression(epsilon),ylab=expression(paste(g[1])),main="location=2")
    plot(pop[,1],pop[,2],col="red",xlab=expression(paste(P[1])), ylab=expression(paste(P[2])),cex.lab=2,cex.axis=2)
    title(expression(paste("g = ",g[1]," , ")), adj=0.06,cex.main=1.5)
    mtext(paste("a = ",a,", b = ",b),side = 3, line=0.2, adj=0.9, col="black")
    
    pop_d<-pop_d_g$pop
    geps<-pop_d_g$g_noise
    plot(mg[,1],geps[,1],col="blue",cex.lab=2,cex.axis=2,xlab=expression(epsilon),ylab=expression(paste(g[2])),main="location=1")
    plot(mg[,2],geps[,2],col="blue",cex.lab=2,cex.axis=2,xlab=expression(epsilon),ylab=expression(paste(g[2])),main="location=2")
    plot(pop_d[,1],pop_d[,2],col="blue",xlab=expression(paste(P[1])), ylab=expression(paste(P[2])),cex.lab=2,cex.axis=2)
