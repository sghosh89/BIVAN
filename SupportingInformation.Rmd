---
title: "Supporting Information : Complex and informative dependencies propogate throughout ecology and can be revealed using copulas"
...
Fig. \ref{MT-fig_cop_pedag1}C (the points, not the contours) can be obtained from Fig. \ref{MT-fig_cop_pedag1}A via transformation
...
copula (see section on methods for the observational hypothesis) 
with three different 
sets of parameters chosen to produce a variety of 
lower-tail (LT) and upper-tail (UT) dependence strengths, but the same 
...
Data pre-processing for the aphid data was the same as 
...
\textbf{Community-level data, Cedar Creek.} Above-ground biomass and percent cover data were 
obtained from biodiversity experiment E120 conducted at the Cedar Creek Ecosystem Science Reserve 
by Tilman *et al*. In 1994, 168 plots (each 9 m $\times$ 9 m) of 39, 35, 29, 30 and 35 replicates,
...
knitr::kable(h,
             ...
             caption = "Results of fitting our 16 copula families to the Cedar Creek dataset, year 2000. Analogous to Fig. \\ref{tab_soilCNfit}, see caption of that figure for details. \\label{tab_cc2000fit}",
             booktabs=T)
```
...
\caption[Nonparametric tests against an N-copula null, Cedar Creek data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for Cedar Creek data, year 2000. As described in the main text, the values of the statistics $\cor_{l_b,u_b}$, $\Ps_{l_b,u_b}$ and $\Dsq_{l_b,u_b}$ for real data (red crosses) were compared to distributions of their values on 1000 Kendall- or Spearman-preserving N surrogates of the data (blue and green x's show 0.025 and 0.975 quantiles), separately in two comparisons for each of the ranges $(l_b,u_b)=(0,0.25),\ldots,(0.75,1)$. If the red cross was outside the range given by the x's, text at the top of panels indicates the number of surrogate values the real-data value was greater than or less than. For instance, a value $>N$ (respectively, $<N$) means the value of the statistic on real data was greater than (respectively, less than) its value on $N$ surrogates. When the statistic was greater than 975 or less than 975 surrogate values, it indicates significance ($95\%$ confidence level). When $\cor$ or $\Ps$ values (respectively, $\Dsq$ values) were greater than surrogates, it means dependence in that part of the distributions was stronger than (respectively, weaker than) expected from a normal-copula null hypothesis.\label{fig_cc2000_nonparam}}
\end{center}
\end{figure}
...
# Verifying model selection results of testing the observational hypothesis on bivariate datasets using nonparametric statistics \label{Q1bivarnonparam}
...
Kendall (or Spearman)
correlation. Thus the comparison addresses the first two parts of the observational hypothesis
...
provides a test of asymmetry of dependence. Thus this comparison helps address the
third part of the observational hypothesis for bivariate 
datasets. 
...
for Cedar Creek data we used $P_{0,0.25}-P_{0.75,1}$.
For each dataset, the rank of the statistic for real-data in 
the distribution of surrogate values was computed. 
If ranks were $>975$ or $<25$, asymmetry of tail 
dependence was significant compared to an N-copula null hypothesis. Ranks were `r unname(tab_bivar_summary[10,])`
for, respectively, the soil C and N dataset, the bird body masses and metabolic rates dataset, 
the mammal body masses and metabolic rates dataset, and the Cedar Creek data for the year 2000. 
Thus the first three datasets showed stronger 
upper- than lower-tail dependence, and the Cedar Creek data showed the reverse 
(though not significantly), confirming the 
results based on model selection presented in the main text (see 
Table \ref{MT-tab_bivar_summary}, row 9). Model selection results for Cedar Creek showed 
only slightly stronger lower- than upper-tail dependence, consistent with the non-significant 
tendency toward lower-tail dependence also revealed by the non-parametric analysis for that
dataset.
...
survival Joe, survival BB1, survival BB6, survival BB7, and survival BB8 copulas, with abbreviations in the methods for the observational hypothesis. NA occurs along the diagonal of the matrix because the diagonal represents a sampling location compared to itself.\\label{tab_count_bestcop}",
             booktabs=TRUE
...
knitr::kable(RES_aphid_ff$info_ord_copname[,,1], #longtable = T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for leaf-curling plum aphid first flight data. Copulas considered were as in Table \\ref{tab_count_bestcop}, with abbreviations in the methods for the observational hypothesis. NA occurs along the diagonal of the matrix because the diagonal represents a sampling location compared to itself.  \\label{tab_ff_bestcop}",
             booktabs=TRUE
             )
```
...
knitr::kable(RES_plankton_north_sea$info_ord_copname[,,1], #longtable=T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for \\textit{Ceratium furca} abundance data. See methods for the observational hypothesis for copula abbreviations. Indep.=Independent. NA occurs along the diagonal because diagonal entries represent a sampling location compared to itself.\\label{tab_pns_bestcop}",
             booktabs=TRUE
             )
```
...
knitr::kable(RES_methane$info_ord_copname[,,1], #longtable=T,
             ...
             caption = "Best-fitting copulas for each location pair from among those considered, according to AIC, for methane-flux data. See methods for the observational hypothesis for copula abbreviations. Indep.=Independent. NA occurs along the diagonal because diagonal entries represent a sampling location compared to itself.\\label{tab_methane_bestcop}",
             booktabs=TRUE
             )
```
...
# Verifying model selection results of testing the observational hypothesis on multivariate datasets using nonparametric statistics \label{Q1multvarnonparam}
...
matrix were constructed similarly, but using an SC copula and an N copula,
respectively. 

Once character evolution was simulated 100 times for each dependence structure, 
we had 817 bivariate characters for each of 500 simulations. We thus analyzed 500 corresponding 
bivariate copulas (one for each simulation), each comprised of 817 points. 
To assess the symmetry or asymmetry of tail dependence for each copula, we computed 
the three non-parametric statistics $\cor_{0,0.2}-\cor_{0.8,1}$, $\Ps_{0,0.2}-\Ps_{0.8,1}$,
and $\Dsq_{0.8,1}-\Dsq_{0,0.2}$ for each copula. For each statistic and for each
dependence structure, we made a histogram of values 
(Fig. \ref{fig_cop_evo}). Asymmetry of tail dependence in evolutionary changes to characters
was associated with similar asymmetry of tail dependence in extant characters.
...
uncertain. If branch lengths were well know, alternative simulation strategies may
include selecting one draw from the matrix of evolutionary changes per unit time, 
or selecting one draw for the whole branch but rescaling variance according to the 
...
between noise inputs and model outputs. See methods section for the causal hypotheses 
...
the methods section for the causal hypotheses (i.e., Figs \\ref{MT-fig_ms_asym_b_0.1} and 
\\ref{fig_ms_asym_b_-0.1}-\\ref{fig_ms_asym_b_-0.5}) were subjected to the model 
selection procedures detailed in the 
methods section for the observational hypothesis. Best-fit copula AICw: the AIC weight of the best-fitting copula family;
...
analyses decribed in the methods section for the observational hypothesis. The listed statistic was computed for the
...
(see methods for the observational hypothesis). A table entry $<X$ (respectively, $>X$) indicates the value of the 
...
\caption[Non-parametric statistics on a mechanism of tail dependence in character distributions]{Three 
measures of asymmetry of tail dependence ($\cor_{0,0.2}-\cor_{0.8,1}$ (A, D, G, J, M), 
$\Ps_{0,0.2}-\Ps_{0.8,1}$ (B, E, H, K, N), and $\Dsq_{0.8,1}-\Dsq_{0,0.2}$ (C, F, I, L, O)) between two characters, across extant species,  were 
computed for each of 100 simulations of mammalian character evolution for each of five types of tail dependence between 
evolutionary changes in the characters (extreme left-tail dependence (A-C), moderate left-tail dependence (D-F), 
symmetry of tail dependence (G-I), moderate right-tail dependence (J-L), and extreme right-tail dependence 
(M-O)). Asymmetry of tail dependence in evolutionary changes in characters tended to beget similar
asymmetry in the characters across extant species. For additional details see 
Appendix \ref{evol_mech}. The number above each panel indicates how many statistics out of $100$ were less than or equal to $0$. Values substantially lower (respectively, higher) than $50$ therefore indicate greater left (respectively, right) tail-dependence between characters.\label{fig_cop_evo}}
\end{center}
\end{figure}
...
year $t$. Data, which pertained to the same locations and years as the aphid data,
were obtained and processed exactly as in [@Sheppard2016]. Copulas
with winter temperature are in Fig. \ref{fig:greenspruce}. Results of analyses of these
copulas that paralleled the analyses applied to bivariate ecological datasets 
(see section on methods for the observational hypotheses) are in Table \ref{tab:greenspruce}.
    ...
    tauval<-unname(ct$estimate)
      ...
      if(IndepTestRes<0.05 && tauval>0){
        plot(vi,vj,type='p',col=rgb(0,0,0,0.3),pch=19,xlim=c(0,1),ylim=c(0,1),
             xlab=names(data_aphid_count_with_temp)[sp],ylab="winter temp.",cex.lab=1.5)
        mtext(paste0("Loc = ",loc," , p = ",round(IndepTestRes,3)),side = 3, line=0.15, adj=0.5)
...
\caption[Copula plot for green spruce aphid abundance data with winter temperature]{Copula plot for green spruce 
aphid abundance data with winter temperature for each selected locations. \label{fig:greenspruce}}
\end{center}
\end{figure}
...
colnames(res)<-c("p-value from indep. test","bestfit copula",
                 "bestfit copula AICw","Normal Copula AICw","p-value from CvM stats","p value from KS stats","LT dependence","UT dependence","(LT - UT) dependence","cor$_{l}$ - cor$_{u}$","P$_{l}$ - P$_{u}$","D$_{u}^2$ - D$_{l}^2$")
res<-res[,-1]
knitr::kable(res,"latex", booktabs = T, linesep = "",digits=3, escape=F,longtable=T,
             caption.short="Summary results for copula made with time-series of green spruce aphid abundance data and winter temperature for each selected locations",
      caption = "Summary results for copula made with time-series of green spruce aphid abundance data and winter temperature for each selected locations, tail dependence were calculated using model averaged AIC-weights, NA represents copula was independent at given location as p-value from independence test was beyond the threshold 0.05. Last three columns indicate non-parametric statistics computed on $(0,0.5)$ and $(0.5,1)$ bounds for lower and upper statistics, respectively.\\label{tab:greenspruce}") %>% column_spec(1:12,width="1.2 cm")#%>%
...
copula structure, we first generated 5000 points from a bivariate N copula
