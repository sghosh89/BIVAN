...
#------------------------------------------------------
id_plot_nosp<-which(db$NumSp==0 & db$Biomass==0) # These plots have no sp. seeded initially and have no biomass data
plots_no_biomass<-sort(unique(db$Plot[id_plot_nosp]))
plots_no_biomass

plots<-sort(intersect(db$Plot,dp$Plot)) # common plots for both datasets for a specific year
length(plots)

excluded_plots<-intersect(plots,plots_no_biomass) 

plots_168<-setdiff(plots,excluded_plots)
