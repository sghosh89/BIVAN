...
**bmr-contrasts.csv** : This contains the trait contrasts
from phylogenetically independent contrasts [@SukumaranH2010] 
...
For each contrast, the path length spanned by the contrast (sum of the branch
  lengths) and the contrast for the mass, BMR and natural
...
**contrasts.csv** : Raw data on measurements of anatomical
features of several species of *Anolis* lizards was downloaded
...
```{r asym_sens_beta_variation, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("OurBiCopSelect.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"),mtime("bivfunctionplot.R"),mtime("CopulaFunctions_flexible.R"),mtime("copsurrog2d.R"))}
set.seed(seed)
#-----------------------------------------------------------------------------------
source("OurBiCopSelect.R")
source("bivfunctionplot.R")
...
#                         1. (for beta=+ve, decrease=F)
#                         2. (for beta=+ve, decrease=T)
#                         3. (for beta=-ve, decrease=F)
#                         4. (for beta=-ve, decrease=T)

plotter_asym_sens_beta_varn<-function(mg,ml,alpha,beta,numkeep_last,ploton){
  
  pop<-Simulator_asym_sens(mg=mg,ml=ml,alpha=alpha,beta=beta,p0=c(0,0),decrease=F,numkeep_last=numkeep_last)
  pop_d<-Simulator_asym_sens(mg=mg,ml=ml,alpha=alpha,beta=beta,p0=c(0,0),decrease=T,numkeep_last=numkeep_last)
  
  neg_beta<- -beta
  pop_neg_beta<-Simulator_asym_sens(mg=mg,ml=ml,alpha=alpha,beta=neg_beta,p0=c(0,0),decrease=F,numkeep_last=numkeep_last)
  pop_d_neg_beta<-Simulator_asym_sens(mg=mg,ml=ml,alpha=alpha,beta=neg_beta,p0=c(0,0),decrease=T,numkeep_last=numkeep_last)
  
  if(ploton==T){
    op<-par(mfrow=c(2,2),mar=c(4,3,1.5,1),mgp=c(2,1,0))
    
    plot(pop[,1],pop[,2],col="red",main = bquote(beta ==.(beta)))
    mtext(paste0("(A)"),side = 3, line=0.3, adj=0.3, col="black")
    plot(pop_d[,1],pop_d[,2],col="blue",main = bquote(beta ==.(beta)))
    mtext(paste0("(B)"),side = 3, line=0.3, adj=0.3, col="black")
    plot(pop_neg_beta[,1],pop_neg_beta[,2],col="orange",main = bquote(beta ==.(neg_beta)))
    mtext(paste0("(C)"),side = 3, line=0.3, adj=0.3, col="black")
    plot(pop_d_neg_beta[,1],pop_d_neg_beta[,2],col="skyblue",main = bquote(beta ==.(neg_beta)))
    mtext(paste0("(D)"),side = 3, line=0.3, adj=0.3, col="black")
  ...
  return(list(pop=pop,
              pop_d=pop_d,
              pop_neg_beta=pop_neg_beta,
              pop_d_neg_beta=pop_d_neg_beta))
...
alpha<-0.2
numkeep_last<-1000
temp<-get_noise_glob_loc_mat(N=N,rho=rho)
mg<-temp$mg
ml<-temp$ml

pdf("./Results/asym_sens_results/beta_0.1_asym_sens.pdf")
ans_beta_0.1<-plotter_asym_sens_beta_varn(mg=mg,ml=ml,alpha=alpha,beta=0.1,numkeep_last=numkeep_last,ploton=T)
dev.off()

pdf("./Results/asym_sens_results/beta_0.9_asym_sens.pdf")
ans_beta_0.9<-plotter_asym_sens_beta_varn(mg=mg,ml=ml,alpha=alpha,beta=0.9,numkeep_last=numkeep_last,ploton=T)
dev.off()

vlist_beta_0.1_beta_0.9<-c(ans_beta_0.1,ans_beta_0.9) # a list of 8 matrices

summary_stat<-as.data.frame(matrix(NA,7,8),stringsAsFactors = F) # Table for model selection summary
rownames(summary_stat)<-c("bestcop_AICw","N_AIC_w","relLTdep_AICw","relUTdep_AIC_w","p_CvM","p_KS","Numboot_success")
bg <- intToUtf8(0x03B2) 
cnm<-c("_0.1","_0.1_decrease_T","_-0.1","_-0.1_decrease_T","_0.9","_0.9_decrease_T","_-0.9","_-0.9_decrease_T")
cname<-paste0(bg,cnm)
colnames(summary_stat)<-cname

ms_asym_beta_0.1_beta_0.9<-vector(mode = "list", length = 8) # initializing list to store model selection results for all possible beta variation
names(ms_asym_beta_0.1_beta_0.9)<-cname

stat_npa_asym_beta_0.1_beta_0.9<-vector(mode = "list", length = 8) # initializing list to store NPA results for all possible beta variation
names(stat_npa_asym_beta_0.1_beta_0.9)<-cname

for(ic in c(1:8)){
  v<-vlist_beta_0.1_beta_0.9[[ic]]
  ...
  ms_asym_beta_0.1_beta_0.9[[ic]]<-res
  ...
  summary_stat[7,ic]<-res$Numboot_success
  
  stat_npa_asym_beta_0.1_beta_0.9[[ic]]<-bivfunctionplot(v=v,resloc="./Results/asym_sens_results/",nametag=cname[ic],numbin=10)
  
}

saveRDS(summary_stat,"./Results/asym_sens_results/beta_variation_summary_stat.RDS")
saveRDS(ms_asym_beta_0.1_beta_0.9,"./Results/asym_sens_results/ms_asym_beta_0.1_beta_0.9.RDS")
saveRDS(stat_npa_asym_beta_0.1_beta_0.9,"./Results/asym_sens_results/stat_npa_asym_beta_0.1_beta_0.9.RDS")
```

![results for model selection \label{fig_ms_asym_beta_0.1}](./Results/asym_sens_results/beta_0.1_asym_sens.pdf){ width=40%, height=40% }

![results for model selection \label{fig_ms_asym_beta_0.9}](./Results/asym_sens_results/beta_0.9_asym_sens.pdf){ width=40%, height=40% }

```{r tab_summary_stat_asym_sens,echo=F,results="markup"}
cnm2<-c("b_0.1","b_0.1d_T","b_-0.1","b_-0.1d_T","b_0.9","b_0.9d_T","b_-0.9","b_-0.9d_T")
colnames(summary_stat)<-cnm2
...
\newpage
```{r tab_stat_npa_asym,echo=F,results="asis"}
#knitr::opts_knit$set(kable.force.latex = TRUE)
tab_asym<-readRDS("./Results/asym_sens_results/stat_npa_asym_beta_0.1_beta_0.9.RDS")
