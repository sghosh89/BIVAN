...
author: "Shyamolina Ghosh, Lawrence W. Sheppard, Mark T. Holder, Terrance E. Loecke, Philip C. Reid, Daniel C. Reuman"
...
\caption[Lower- and upper-tail dependence can vary independently of correlation]{Lower- and upper-tail dependence can vary independently of correlation. (A-C) Data generated using the BB1 copula (see section on methods for Q1) 
...
within each column.\label{fig_cop_pedag}}
\end{center}
\end{figure}
...
<!--additional details about Cedar creek data, etc.-->
Above-ground biomass and percent cover data were obtained from long-term (? or short-term)
biodiversity experiment (E120) conducted in Cedar Creek Ecosystem Science Reserve by Tilman *et al*.
In 1994, 168 plots (each 9 m $\times$ 9 m) of 39, 35, 29, 30 and 35 replicates,
were seeded with 1, 2, 4, 8, or 16 perennials, respectively, chosen randomly from a pool of 
18 species (4 species, each, of C4 grasses, C3 grasses, legumes, non-legume forbs; 2 species of woody
...
and sometimes selective herbicides were used to set up the monocultures either with only grasses or with only forbs.
We downloaded the data on July, 2018 from Cedar Creek website \url{http://www.cedarcreek.umn.edu/research/experiments/e120} and 
analyzed those [data](https://github.com/sghosh89/BIVAN/tree/master/Data/Ceder_creek_e120) for 6 common years 
(1996-2000, 2007) when both biomass and percent cover data were available. Each plots were annually sampled by clipping 
different locations each year. For each available year, we computed above-ground biomass ($gm^{-2}$) 
and Shannon's biodiversity index (using percent cover data) averaged over the sampled strips/ quadrats taken from each 
of 168 plots.

<!--we need to add aphid and plankton data details, Lawrence will give those-->
...
<!--reading multivariate methane flux data-->
...
We also define $\Ps_{l_b,u_b}$, using the bounds
...
<!--Function to generate a table for npa results-summary with bivariate data, this one about asymmetry of tail dependence-->
...
<!--Displaying a table with npa summary results for Birds' BMR data-->
...
<!--Displaying a table with npa summary results for Mammals' BMR data-->
...
\caption[Nonparametric tests against an N-copula null, Cedar Creek data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for Cedar Creek data. As described in the main text, the values of the statistics $\cor_{l_b,u_b}$, $\Ps_{l_b,u_b}$ and $\Dsq_{l_b,u_b}$ for real data (red crosses) were compared to distributions of their values on 1000 Kendall- or Spearman-preserving N surrogates of the data (blue and green x's show 0.025 and 0.975 quantiles), separately in two comparisons for each of the ranges $(l_b,u_b)=(0,0.25),\ldots,(0.75,1)$. If the red cross was outside the range given by the x's, text at the top of panels indicates the number of surrogate values the real-data value was greater than or less than. For instance, a value $>N$ (respectively, $<N$) means the value of the statistic on real data was greater than (respectively, less than) its value on $N$ surrogates. When the statistic was greater than 975 or less than 975 surrogate values, it indicates significance ($95\%$ confidence level). When $\cor$ or $\Ps$ values (respectively, $\Dsq$ values) were greater than surrogates, it means dependence in that part of the distributions was stronger than (respectively, weaker than) expected from a normal-copula null hypothesis.\label{fig_cc2000_nonparam}}
\end{center}
\end{figure}

<!--Displaying a table with npa summary results for Cedar creek data-->
...
knitr::kable(tab_cc_asym_2000,
             ...
             caption = "Cedar Creek dataset results for nonparametric tests of whether asymmetry of tail dependence was significant, compared to an N-copula null hypothesis. The values of the listed statistics (cor$_{FS}$-cor$_{LS}$ = $\\cor_{0,0.25}-\\cor_{0.75,1}$, P$_{FS}$-P$_{LS}$ = $\\Ps_{0,0.25}-\\Ps_{0.75,1}$, D$_{LS}^2$-D$_{FS}^2$ = $\\Dsq_{0.75,1}-\\Dsq_{0,0.25}$; here FS stands for `first slice' and LS for `last slice') for real data were compared to their values for 1000 Kendall- or Spearman-preserving N surrogates, in seperate comparisons. A table entry $>X$ indicates the value of the given statistic on the data was higher than its value on $X$ of the surrogates, so entries of the form $>X$ for $X$ equal to $975$ or above would indicate that lower-tail dependence was significantly stronger than upper-tail dependence.\\label{tab_cc_asym_2000}",
             booktabs=T)
```
...
# Verifying Q1 results for bivariate datasets using nonparametric statistics \label{Q1bivarnonparam}
...
tail dependence (Appendix \ref{surrog_test}). 
...
correlation (the sorrogate algorithm had two versions, one for preserving each correlation coefficient), 
...
(Fig. \ref{fig_soilCN_nonparam}). Results show that tail dependence of data is stronger in 
...
on 1000 Kendall- or Spearman-preserving N surrogates, separately in two 
...
For first three bivariate datasets in Table \ref{MT-tab_bivar_summary},
the statistic $P_{0,0.1}-P_{0.9,1}$ was computed for real data and 
for 1000 Kendall-preserving surrogate datasets with N copulas. Due to relatively smaller sample size,
for Cedar creek data we used a wider range of boundaries $(l_b,u_b)$ to compute $P_{0,0.25}-P_{0.75,1}$.
For each data, rank of the said statistics with real-data value in 
the distribution of surrogate values was computed. 
If ranks were $>975$ or $<25$, asymmetry of tail 
dependence was significant compared to a N-copula null hypothesis. Ranks were `r unname(tab_bivar_summary[10,])`
for, respectively, the soil C and N data, bird masses and metabolic rates, mammal masses
and metabolic rates, and the Cedar Creek data. Thus the first three datasets showed stronger 
upper- than lower-tail dependence, and the Cedar Creek data which showed the reverse though not significantly with
non-parametric analysis, confirming the 
results based on model selection presented in the main text (see Table \ref{MT-tab_bivar_summary}, row 9 
showed Cedar creek had relatively weak tail-dependence than the other three datasets).
...
multivar_tab_summary_fn<-function(r,rs){
  ...
  m$`num.non-N`<-sum(bcop!=1,na.rm = T)
...
# Verifying Q1 results for multivariate datasets using nonparametric statistics \label{Q1multvarnonparam}
...
(Appendix \ref{spatial_resamp}) to calculate confidence intervals of these means.

For the green spruce aphid abundance data, mean values across all pairs of 
...
(Table \ref{tab_count_resamp}), confirming the results in the main text on 
asymmetry of tail dependence and 
stronger lower- than upper-tail dependence for this dataset. Each of the statistics 
was plotted against geographic (great-circle) distances between sampling locations, 
across all pairs of sampling locations (Fig. \ref{fig_aphid_count}). 

Asymmetry of tail dependence was also verified by our nonparametric methods for
the other multivariate datasets. For instance, the $95\%$ confidence intervals of the
mean over pairs of sampling locations of the statistic $\Ps_l-\Ps_u$ were
$(-0.085,-0.041)$ for the leaf-curling plum aphid first flight data, 
indicating greater upper-tail dependence, and consistent with
<!--***DAN: Shya, pls make above results autolink-->
the results of Table \ref{MT-tab_multivar_summary} (row 9). 
For the *Ceratium furca* abundance data, confidence intervals were
$(0.011,0.044)$, indicating greater lower-tail dependence, and again
<!--***DAN: Shya, pls make above results autolink-->
consistent with the results of Table \ref{MT-tab_multivar_summary} (row 9).
...
for each location. If left-tail dependent noise was desired, we took the negative of the
entire matrix. The moderately left-tail dependent matrix was based on a C copula 
...
matrix were constructed similarly, but using an SC copula and a N copula,
respectively. 

Once character evolution was simulated 100 times for each dependence structure, 
we had 817 bivariate characters for each of 500 simulations. We generated 500 bivariate copulas 
each made of 817 points. To assess the symmetry or asymmetry of tail dependence, we computed 
our three non-parametric statistics ($\cor$, $\Ps$, $\Dsq$) on both extreme tips (lower, henceforth subscripted with `l` 
and upper, henceforth subscripted with `u`) of each copula.
To measure lower tail and upper tail statistics we used the bounds as $(l_b=0,u_b=0.2)$ and 
$(l_b=0.8,u_b=1)$, respectively. Therefore, positive (or negative) difference of tail dependence statistics
($\cor_l-\cor_u$, $\Ps_l - \Ps_u$ and $\Dsq_u - \Dsq_l$) would indicate asymmetry in dependence structure with
stronger association in lower tail (or upper tail) of the bivariate copula. 

Five separate histograms were made with those 500 $\cor_l-\cor_u$ statistics, where each 100 
sets were representative of 
five different groups of simulations. These five groups of simulations used random draws from
1) extreme left-tail, 2) moderate left-tail,
3) symmetric tail (bivariate `N` copula), 4) moderate right-tail,
and 5) extreme right-tail dependence copula, respectively, as two traits evolved through each branch.
Five histograms each consisting 100 $\Ps_l - \Ps_u$ statistics and another five histograms each consisting
100 $\Dsq_u - \Dsq_l$ statistics were produced in a similar way. Fig. \ref{fig_cop_evo} represents 
three statistics for each five sets of dependence structure. If most of the statistics showed major
occurrence on either side of zero line (vertical dashed line in Fig. \ref{fig_cop_evo}), then
asymmetry in copula structure was confirmed. Fig. \ref{fig_cop_evo}(A-C, D-F) showing 
stronger lower tail dependence with three statistics majorly distributed 
on positive side were consistent with left tail-dependent copula as mentioned (1) and (2), respectively. 
Similarly, Fig. \ref{fig_cop_evo}(J-L, M-O) showing 
stronger upper tail dependence with three statistics majorly distributed 
on negative side were consistent with right tail-dependent copula as mentioned (4) and (5), respectively.
Evenly distributed tail dependence-difference statistics (see Fig. \ref{fig_cop_evo}G-I) 
around zero line was indicative of symmetric dependence as emerged from bivariate normal copula.

<!--***DAN: Dan does not understand the following two paragraphs yet, but is planning to
speak to Mark about them-->
...
Biases in estimating the evolutionary distance can affect down stream analyses [@Phillips2009].
...
\caption{Analogous to Fig. \ref{MT-fig_ms_asym_b_0.1}, but using $b=0.5$.\label{fig_ms_asym_b_0.5}}
\end{center}
\end{figure}
...
\caption{Analogous to Fig. \ref{MT-fig_ms_asym_b_0.1}, but using $b=-0.5$.\label{fig_ms_asym_b_-0.5}}
\end{center}
\end{figure}
...
\\ref{fig_ms_asym_b_-0.1}-\\ref{fig_ms_asym_b_-0.5}) were subjected to the model 
...
The output of simulations from the second model detailed in 
the methods section for Q2 (i.e., Figs \\ref{MT-fig_ms_asym_b_0.1} and 
\\ref{fig_ms_asym_b_-0.1}-\\ref{fig_ms_asym_b_-0.5}) were subjected to nonparamtric statistical 
...
<!--adding plots showing results to confirm hypothesis behind third mechanism-->
\begin{figure}[!h]
\textbf{ \hspace{6.5 cm} (A) \hspace{2 cm} (B) \hspace{2 cm} (C)} \\
\vspace{-0.5 cm}
\begin{center}
\includegraphics[width=9 cm]{./Results/BMR_results/evo_cop/ExtremeL_hist_taildep.pdf}\\
\end{center}
\textbf{ \hspace{6.5 cm} (D) \hspace{2 cm} (E) \hspace{2 cm} (F)} \\
\vspace{-0.5 cm}
\begin{center}
\includegraphics[width=9 cm]{./Results/BMR_results/evo_cop/SomewhatL_hist_taildep.pdf}\\
\end{center}
\textbf{ \hspace{6.5 cm} (G) \hspace{2 cm} (H) \hspace{2 cm} (I)} \\
\vspace{-0.7 cm}
\begin{center}
\includegraphics[width=9 cm]{./Results/BMR_results/evo_cop/BivNormal_hist_taildep.pdf}\\
\end{center}
\textbf{ \hspace{6.5 cm} (J) \hspace{2 cm} (K) \hspace{2 cm} (L)} \\
\vspace{-0.5 cm}
\begin{center}
\includegraphics[width=9 cm]{./Results/BMR_results/evo_cop/SomewhatR_hist_taildep.pdf}\\
\end{center}
\textbf{ \hspace{6.5 cm} (M) \hspace{2 cm} (N) \hspace{2 cm} (O)} \\
\vspace{-0.5 cm}
\begin{center}
\includegraphics[width=9 cm]{./Results/BMR_results/evo_cop/ExtremeR_hist_taildep.pdf}\\
\caption[Non-parametric statistics to check probable mechanism behind asymmetric tail dependence in character distributions across phylogeny tips]{Three measures of asymmetric tail dependence using non-parametric statistics ($\cor,\Ps$ and $\Dsq$)
on lower ($l_b=0,u_b=0.2$) and upper ($l_b=0.8,u_b=1$) tips confirms our hypothesis that copula structure in evolutionary changes in bivariate characters can give rise to similar tail dependent copula in character distributions across phylogeny tips (for details see Appendix \ref{evol_mech}). Stronger lower tail dependence (distributed more on positive values) was found in bivariate traits when (A-C) extreme left tail and (D-F) moderately left tail dependent copula was used in evolutionary shifts through each branch. Similarly, non-parametric statistics showed symmetric tail dependence for (G-I) normal copula and stronger upper tail dependence for (J-L) moderately right tail and (M-O) extreme right tail dependent copula, respectively. The number $(n)$ in each subfigure designates how many statistics out of $100$ is equal or less than $0$, thus if $n$ is substantially lower (or higher) than $50$, it would indicate asymmetric left (or right) tail-dependence in evolved traits-copula. \label{fig_cop_evo}}
\end{center}
\end{figure}
...
*slope*, and we refer to $\log(a)$ as the Taylor's law intercept. 
...
So means $m_g$ and variances $v_g$ are computed across space. We view the assumptions
of Taylor's law to be validated for a dataset if the 
...
resulting $N \times n \times 1000$ array as our *N-copula population array* 
for this simulation exercise;
copula structure of dependences between populations in different locations
was normal (N) for this array. 

We then generated an series of arrays that were the same dimensions ($N \times n \times 1000$)
as the N-copula population array, and that were statistically similar to 
the N-copula population array in all ways except the new
arrays were constructed to have C (respectively, F, SC) copula structure for 
the dependences between 
populations in different sampling locations within a simulation. We 
refer to the new arrays as our
*C-copula*, *F-copula*, and *SC-copula population arrays*, respectively. The C-copula array was
constructed as follows; the others were constructed analogously. First, we constructed an
$n$-dimensional C copula with parameter chosen so that the 
Spearman correlations between each pair of the $n$ components were the same as those
of the N distribution constructed above. This was done using the `rho` and `iRho` functions 
in the `copula` package in R. Because the C copula is a one-paremeter family, this 
...
the N-copula population array in a special way 
(see Appendix \ref{sect:copsurrognd}) so that the resulting 
surrogate/randomized dataset had the copula structure
of the C copula just constructed. The randomization procedure worked 
by permuting time series of values in the N-copula array, separately for each 
sampling location and simulation. So population marginal distrbutions for each location
and simulation were *exactly* the same as those in the N-copula population array. 
Permutations were done in such a way
that resulting pairwise Spearman correlations were the same as the C-copula 
constructed above, and hence were the same as the N-copula array, 
except for sampling variation. Thus, to summarize using mathematical notation,
if $m^{(\text{N})}$ is our N-copula population array and $m^{(\text{C})}$ is our 
C-copula population array, then,
...
simulations were compared for the C, N, F, and SC simulations to determine if copula structure 
influences Taylor's law.

Taylor's law was strongly influenced, and was often even invalidated in its assumptions of 
linearity and homoskedasticity, by non-N copula structure. For N copula structure 
(i.e., for simulations in our N-copula population array), $p$-values for the linearty
...
But linearity or homoskedasticity were violated more frequently for non-N copula structure
...
relationship when copula structure was not N.
