...
All branches of ecology and related areas study relationships among environmental and biological 
variables. However, commonly used correlation- and regression-based approaches to studying 
such relationships are limited - it is well understood that common statistical
approaches may provide only a very partial description of a relationship. 
...
Fig. \ref{fig_cop_pedag1}A-B show two such datasets. The panel A 
(respectively, B) variables of Fig. \ref{fig_cop_pedag1} are principally 
related in the left (respectively, right) portions of their distributions, a difference that may
have ecological significance, as discussed below, but is not captured by standard
...
Well-developed statistical tools do exist (REFS), and are applied widely in other fields 
(REFS), that provide a complete description of the dependence between two or more variables. 
The goal of this paper is to illustrate the potential for broader application of these
tools in ecology and related fields. The tools are based on the statistical concept of the *copula*, which we here introduce conceptually. 

  a) then do it. some things to do include...
  b) the rank-based approach for data (use bivariate data)
  c) the inverse cdf approach for the copula of a bivariate distribution
  d) statement that copulas are also multivariate, though we won't explore that here (or maybe leave that for Discussion?)
  e) a one- or two-sentence description of sklars theorem
...
\caption{(A-F, I, J) Diverse bivariate datasets, many of which have the same Pearson (P)
...
normal marginal distributions of A and B to gamma (C, D) or uniform (E, F) marginals. 
Details in Appendix X.\label{fig_cop_pedag1}}
\end{center}
\end{figure}
...
Community-level data were plant aboveground biomass and Shannon's diversity index for X 
