...
plotter_asym_sens_b_varn<-function(mg,ml,a,b,numkeep_last,ploton){
  ...
  if(ploton==T){
    ...
    mtext(expression(paste("(A) ","b = ",b,", g = ",g[1])),side = 3, line=0.3, adj=0.5, col="black")
    plot(pop_d[,1],pop_d[,2],col="blue",xlab=expression(paste(P[1])), ylab=expression(paste(P[2])))
    mtext(expression(paste("(B) ","b = ",b,", g = ",g[2])),side = 3, line=0.3, adj=0.5, col="black")
    plot(pop_neg_b[,1],pop_neg_b[,2],col="orange",xlab=expression(paste(P[1])), ylab=expression(paste(P[2])))
    mtext(expression(paste("(C) ","b = ",b,", g = ",g[1])),side = 3, line=0.3, adj=0.5, col="black")
    plot(pop_d_neg_b[,1],pop_d_neg_b[,2],col="skyblue",xlab=expression(paste(P[1])), ylab=expression(paste(P[2])))
    mtext(expression(paste("(D) ","b = ",b,", g = ",g[2])),side = 3, line=0.3, adj=0.5, col="black")
...
rownames(summary_stat)<-c("bestcop_AICw","N_AICw","relLTdep_AICw","relUTdep_AICw","p_CvM","p_KS","Numboot_success")
#bg <- intToUtf8(0x03B2) 
...
![results for model selection \label{fig_ms_asym_b_0.1}](./Results/asym_sens_results/a_0.2_b_0.1_r_0.8_asym_sens.pdf){ width=40%, height=40% }

![results for model selection \label{fig_ms_asym_b_0.9}](./Results/asym_sens_results/a_0.2_b_0.9_r_0.8_asym_sens.pdf){ width=40%, height=40% }
