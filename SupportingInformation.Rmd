...
# From causes to consequences for green spruce aphid abundance data\label{sect:greenspruce}

For each locations (selected based on a threshold of 30 years of data, see \ref{Data_details}), 
we computed yearwise average temperature of winter season (spanning over 4 months from December, previous 
year to March, present year). With this winter temperature time series and green spruce aphid abundance time series data 
a bivariate copula was made for each selected locations and shown in Fig. \ref{fig:greenspruce} whenever it was 
significantly dependent with a p-value $<0.05$. Stronger lower tail dependence
was visually prominent in those copulas. We used our model selection and non-parametric statistical tools 
(see Table \ref{tab:greenspruce}) to confirm our hypothesis that winter temperature 
was the main driver in "lower tail dependence" of the green
spruce aphid abundance copula from any two location pairs (see Table \ref{tab_count_LTmUT}).
...
knitr::kable(res,"latex", booktabs = T, linesep = "",digits=3, escape=F,longtable=T,
             caption.short="Summary results for copula made with time-series of \\textbf{green spruce aphid} abundance data and winter temperature for each selected locations",
