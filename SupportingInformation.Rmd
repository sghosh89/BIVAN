...
rownames(summary_stat)<-c("bestfit.copula.AICw","Normal.copula.AICw","LT.model.avg","UT.model.avg","p.CvM","p.KS")
...
knitr::kable(tab_stat_npa_asym, booktabs = T, longtable=T,
             format = "latex",linesep = "",escape=F,
             caption.short = "Results for tests of whether asymmetry of tail dependence was significant
                              compared to a normal-copula null hypothesis.",
             caption ="A table entry $<N$(or $>N$) indicates the value of the given statistic on the data was less (or more)
                      than its value on $N$ of the surrogates, so entries of the form $<X$ (or $>X$) for $X$ equal to $975$ 
                      or above indicate that upper-tail (or lower-tail) dependence was significantly stronger than lower-tail         
                      dependence.\\label{tab_stat_npa_asym}")#%>%
 #            kable_styling()%>%
