...
\noindent \emph{Type of Submission:} Articles, or Concepts and Synthesis
...
# Abstract
<!--***DANr: I have not edited this abstract at all since Ecol Monogr-->
...
\end{center}
\end{figure}
<!--***DAN: When I cut panels out of this figure (saving them for use in a 
later figure), I also cut the caption, and here is the cut stuff for later use:
(C, D) Contours 
of probability density functions (pdf) of Clayton (C) and survival-Clayton 
(D) copulas with parameter 5 (see sections on methods for Q1), together
...
pdf of a normal copula (methods for Q1) with 
parameter chosen to
have the same Spearman correlation as A-D, together with a sample.
(H) The portion of the Spearman correlation within a band 
can be computed
for any band (methods for Q1) to describe how dependence
...
dependence than the right/upper band.
Subscripts h and v 
stand for "horizontal" and "vertical".
-->

<!--DONE ***DANr: reword this para so people don't think the ranks *are* the 
copulas, and to try to pre-defend against some of what ref 1 said. Also,
state that these are conceptual introductions only and are not formal,
a more comprehensive introduction is in xyz-->
...
information on the marginals. Spearman and Kendall correlations, being rank-based, 
...
that it can detect tail dependence and asymmetries of tail dependence. 
Tail dependence (defined precisely in the section on Background, below) 
...
Spearman or Kendall correlation can have a range of tail dependence 
...
complex dependence structure of data, we have found tail dependence to be
a useful aspect of the full structure of dependence, so tail dependence 
...
only - readers can find abbreviated [@Anderson2018] 
...
second through
fourth sections of the text, see also Table \ref{tab_data_info}). Upper boxes 
correspond to potential causes of non-normal (non-N) copula structure that
were examined (in the fifth and sixth sections of the text). Lower boxes 
correspond to potential consequences we examined of non-N copula structure for
ecological understanding and for applications (in the seventh and eighth
...
<!--DONE ***DANr: Maybe use the words tail dependence more and the word copula
less in the below few paras? Generally the words "copula structure" may be rubbing some
people wrong. Replacement words? What do you actually mean here? Maybe call it
asymmetric dependence, or complex dependence structure?-->
...
measured through time in two locations to show strong tail dependence between
the locations if
...
<!--DONE ***DANr: Need some caveating here to accommodate the people who slavishly
follow the old maxim that correlation is not causation.-->
...
dependence distinct from that of a normal distribution, and in particular
do they show asymmetric tail dependence? Normal copula structure is assumed
...
data contain dependency information distinct from that standard or default cases. 
(Q2) If the answer to
Q1 is "yes," then what are some possible causes/mechanisms of non-normal
copula structure and tail dependence in ecology? And (Q3) what are the 
consequences of non-normal copula
structure and tail dependence for ecological understanding and
...
for the rest of the paper. See @nelsen2006_copula, @joe2014_dependence, and
...
distributions functions of the component random variables translates
...
are the same. Fig. \ref{PedagogFigCopThry}D shows stronger left- than right-tail dependence. 
Sklar's theorem
...
<!--***DAN: I still feel like we have to make this more intuitive for people
at this point. Maybe a figure with two panels showing two copulas on
the left, and two panels shwoing two sets of marginals at the top,
and then four panels showing what you get when each of these combine?
Good idea. Steps: 
DONE split the current Fig 1 so the new Fig 1 has the first 6 panels,
but C-D should only have the points. 
DONE Then make a Fig 2 as above, 
DONE and edit the text in the above para to use that figure. 
STILL HAVE TO DO The rest of the panels of the current Fig 1 will make a new Fig 3 later.
-->
...
copula family (henceforth abbreviated N), which consists of copulas of bivariate normal
...
between the variables. An N copula with $p=0.7$ was already introduced (Fig.
...
example pdfs correspondng to bivariate N copulas for various values of $p$.
Note that the pdfs are symmetric
around the diagonal line $v=-u+1$ (Fig. \ref{figped_N}), so normal copulas have symmetric 
tail dependence (see
below for a more precise defintion of tail dependence). 
The Clayton copula family (henceforth abbreviated C),
of which we also already pictured an example (Fig.
\ref{PedagogFigCopThry}B), is another one-parameter family. In contrast to N copulas, C copulas
have stronger left- than right-tail dependence. The formula is 
...
Clayton family (here abbreviated SC) is the symmetric opposite of the C family, showing
stronger right- than left tail dependence (Fig. \ref{SM-figped_C}). The BB1 copula 
family is a two-parameter family, thereby providing more flexibility: for some parameters
BB1 copulas have stronger left- than right-tail dependence, and for others the reverse
...
Having introduced tail dependence in a conceptual way and referred conceptually to it
in examples, we now need a more precise definition. 
<!--***DAN: fill in-->
...
regarded, for large $n$, as a sample from the bivariate pdf associated with the copula of $(X,Y)$,
...
<!--***DANr: Add a new section called Background on copulas.
It can have the following kinds of stuff (probably not exhaustive, and probably
not in order, and some of this material is currently below):
1) DONE - The definition, Sklar's theorem
2) DONE What good are copulas - intuitive idea, basically, but also have to be linked to 
the formalities of the definition
3) DONE What some of the different copula families look like - in particular you will use BB1 in Fig S2 and cite Background, so mention it
  Three new figures: Normal, Clayton, BB1
STILL HAVE TO DO 4) What tail dependence actually is
DONE 5) The explanation of how ranks relate to copulas and estimating the copula parameters
How to carry out fitting (basically, MLE fitting on ranks - Genest et al paper)
-->

<!--***DAN:
You are going to have to rearrange the order of some figs in Sup Mat because now the data section comes after Bacground, in which the different copula famlies are introduced
-->

<!--***DAN: text storage for later:
...
and $0$ lower-tail dependence. -->

<!--***DAN: put this text in the methods. It won't help people here in the ms to have all
these copulas intro'd. The sole purpose of them is give a variety of options to have a robust model
selection and multi-model inference procedure for tail dependence, so mention them in that context in 
the Methods in a way that make clear we don't need to give a damn about these things:
For our model selection procedure (see below), we used 
...
Visualizations of these families (Figs \ref{SM-figped_N}-\ref{SM-figped_SBB8})
can be compared with normalized rank plots to estimate visually 
which families may effectively model a given bivariate dataset. 
If LT and UT denote 
...
family (it is a 
one-parameter family); and LT and UT are the measures of lower- and upper-tail dependence, respectively (see Background section for definition).\label{figped_N}}
\end{center}
\end{figure}
...
\caption[Example C copulas]{Log-transformed pdfs (A-E) and samples (F-J) from example C 
copulas. S is Spearman correlation; p is the value of the parameter for the C family (it 
is a one-parameter family); and LT and UT are the measures of lower- and 
upper-tail dependence, 
respectively (see Background section for definition).\label{figped_C}}
\end{center}
\end{figure}
...
respectively (see Background section for definition).\label{figped_BB1}}
\end{center}
\end{figure}
...
<!--***DANr: I moved some text from here and have not smoothed the transtion,
if it needs smoothing-->
...
In addition to our model selection approach, we also used 
nonparametric approaches to provide greater 
...
# Results for Q1: Ecological datasets have non-normal copula structure and asymmetric tail dependence \label{Results}
...
for birds, (C) the same for mammals, and (D) above-ground plant biomass vs. Shannon's index (\textit{H}) from Cedar Creek. Bottom panels show the corresponding copula plots. See Data section for units used in upper panels. \label{fig_biv_multi_raw_cop}}
\end{center}
\end{figure}

Across essentially all our datasets, copula structures were typically non-N and 
commonly showed asymmetric tail 
dependence, answering Q1 in the affirmative. 
...
N copula had AIC weight essentially 0, answering the 
...
These values are distinct from what an N copula would give, namely, 0, answering the second part 
Q1 for these data. The values also differed substantially 
from each other, answering the third part of Q1. 
...
Fig. \ref{SM-fig_soilCN_nonparam}, Table \ref{SM-tab_soilCN_asym}). Our 
affirmative, empirically based answer to Q1 for the soil C and N data is 
...
for the bivariate datasets. The two 
...
(Table \ref{tab_bivar_summary}, rows 3-4), answering the first
part of Q1. 
...
answering the second and third parts of Q1. 
...
Our 
affirmative, empirically based answer to Q1 for the bivariate datasets is 
represented in Fig. \ref{MasterFigure} as solid outlines around the names of those
...
larger than N-copula weights (Table \ref{SM-tab_count_AICw_normal}), answering the first part of Q1. Goodness of fit tests 
...
from what an N copula would give (i.e., 0), and answering the second
...
dependence were common, answering Q1 in the affirmative for these data. 
...
thus this is a mechanism that can produce non-N copula structure and
...
for details). 
Thus this is a mechanism that may produce non-N copula structure and
...
mechanisms that may cause non-N copula structures and tail dependence;
...
non-N copula structure and tail-dependence for the field of ecology. 
...
To our knowledge, only N copulas have 
...
tail depencence and non-N copula struture of ecological dependencies 
across space.
...
species. This is another theoretical mechanism for non-N copula
...
of non-N copula structure and tail dependence in ecological 
