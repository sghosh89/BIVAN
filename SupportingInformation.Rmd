...
The third statistic, $\Dt_{l_b,u_b}(u,v)$, is the average squared 
...
$u_i+v_i<2u_b$ cluster close to the line $v=u$, $\Dt_{l_b,u_b}$ is 
then small. 
...
$\Dt_{l_b,u_b}$ indicate strong dependence. 
...
$\Ps_u=\Ps_{0.5,1}$, $\Dt_l=\Dt_{0,0.5}$, and $\Dt_u=\Dt_{0.5,1}$.
...
The difference $\Dt_u-\Dt_l$, in which the "upper" and
...
$\Dt_{1-u_b,1-l_b}-\Dt_{l_b,u_b}$ with $l_b=0$ and $u_b$ close to $0$.
