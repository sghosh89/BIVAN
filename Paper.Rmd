...
d_soilCN<-readRDS("Data/RaCA_soilorganicC_soiltotalN_stocks100cm.RDS")
...
The database comprises measurements of soil organic carbon and total soil nitrogen (Mg C or N per m$^{2}$ of soil surface) 
<!--***DAN: Shya, this used to say Mg C, but what is Mg? Do you mean mg for miligram? I have written it out, please check and correct if necessary. Also, I added per m^2 of soil surface, please check.
***Shya: Mg means mega gram, So an "Mg C" is a ton of Carbon - the standard SI unit when it comes to talking about soil Carbon measurement.-->
from `r nrow(d_soilCN)` 
<!--***DAN: Shya, make this an autolink to the number of data points - 5930, but should be an autolink, so if the number changes it updates
***Shya: Done.-->
locations across the coterminous United States (Fig. X for sampling locations).
<!--***DAN: Shya, please add a sup mat figure showing the sampling locations and a link to it here in place of the X-->
...
<!--***DAN: Shya to add units, and replace X and Y with autolinks that give the numbers-->
<!--***Shya: These species number for birds and mammals came from the processed data without any NA and -ve(unrealistic) mass, is that okay? 
for example, species of birds [@Mcnab_2009] raw data (the xls file you gave me) shows 712 species out of which some have NA in both mass and BMR column and one entry has -ve mass!!!-->
These data have been much studied, but to our knowledge the copula structure of the dependence between metabolic rate and body mass has not been examined. 
...
<!--***DAN: Shya to add a figure shwoing the locations on a map of the UK and cite it here with an automatic citation
***Shya: It is already in suppmat figure S1.-->
across the United Kingdom (UK) for 20 aphid species (Table \ref{SM-tab_aphid_info}), for the 35 years from 1976 to 2010. These time series were computed from the the Rothamsted Insect Survey suction-trap dataset [@Macaulay1988, @Harrington2014]. The first of our two population-level datasets was also derived from the RIS suction-trap data, and comprised total counts of aphids trapped for the same locations, species, and years; these are indicators of flying aphid abundance. Data pre-processing for the aphid data was the same as that of @Sheppard2016,
<!--***DAN: Shya, let's discuss please - if we say this we have to be sure we did *exactly* the same pre-processing as Lawrence, whereas I seem to remember we did something slightly different-->
but we exlcuded one of the 11 sites they used so as to include only sites with at least 30 years data.  Our second population dataset comprised average annual abundance estimates for 14 locations (Fig. \ref{SM-fig_plankton_map})
in the North Sea and British seas for 22 plankton taxa (Table \ref{SM-tab_plankton_info}) for the 56 years from 1958 to 2013. The locations are $2^\circ$ by $2^\circ$ grid cells. These data were obtained from the Continuous Plankton Recorder (CPR) dataset maintained by the Sir Alistair Hardy Foundation for Ocean Science (SAHFOS), now part of the Marine Biological Association of the UK. Data pre-processing was similar but not identical to that of @Sheppard2017; see Appendix \ref{SM-Data_details} for details.
Community-level data were plant aboveground biomass and Shannon's diversity index for X plots, each 9m by 9m, maintained at Cedar Creek Ecosystem Science Reserve and sampled in the year 2000 [@Tilman2001; @Tilman2006]. 
Finally, ecosystem functioning data were methane (CH$_{4}$) fluxes between the soil or water surface and the troposphere, measured at 13 locations at daily to weekly intervals from September 2015 to September 2016 at the Great Miami Wetland mitigation bank in Trotwood, Ohio [@Holland1999; @Jarecke2016]. Each included location was measured on at least 50 dates. Sampling locations spanned a hydrologic gradient from upland to seasonal wetland.
See Appendix \ref{SM-Data_details} for additional data details. 

The environmental, species-trait, and community datasets, as we studied them, are henceforth called the "bivariate" datasets in the sense that they comprise two quantities measured at different locations or for different species. The phenology and population datasets are "multivariate" in that they comprise, for each taxon, measurements at multiple locations; copula structure can be studied for each pair of locations. Likewise the the ecosystem functioning (methane flux) dataset was  multivariate in this sense.

# Concepts for \hyperref[Intro]{\textbf{(Q1)}}: Statistical tools for detecting copula structure \label{Methods}
...
Descriptions of these copula families can be found in introductory texts on copulae [@nelsen2007_copula ; @joe2014_dependence]  and in `VineCopula` documentation. The lower- and upper-tail dependence of the best-fitting member of each family was also provided by `BiCopEst`. Finally, $\text{AIC}_{\text{w}}$ values for all fitted models were used to get
