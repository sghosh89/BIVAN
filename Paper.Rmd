...
This section is attributed to analysis of question \hyperref[Intro]{\textbf{(Q3)}} from \nameref{Intro}. We found that asymmetric tail dependence had **(i)** effects on skewness of spatially averaged time series data. Spatial mean of the time series data extracted that tail dependence from the multivariate data set, if any, and reflected in the averaged skewness. We tested null hypothesis $H_{0}$ which presumed that multivariate copula actually came from normal copula structure with no tail dependence and thus should have the same skewness as of normal copula (for details see Appendix \ref{SM-Consequence_skewness}). If $p$-value is less than $0.05$, then we reject the null hypothesis in support of significant tail dependence (and henceforth significantly different skewness). Figure \ref{fig_skewness_methane_maintext} showed asymmetric copula structure's effect on spatial skewness for biogeochemical methane-flux data.
The result, showing significantly different skewness of spatial mean from that of Normal surrogates, was consistent with model selection and nonparametric approach which supported lower tail dependence copula for methane-flux data. 

We then showed **(ii)** asymmetric copula structure affected slope of spatial Taylor's Law differently than as expected from symmetric Normal copula (For details see Appendix \ref{SM-Consequence_TL}). Finally, we studied **(iii)** the effects of non-Normal copula on extinction risk with two metapopulation model, namely, Lewton-Cohen model and Ricker model (For details see Appendix \ref{SM-Consequence_ext_risk}). 

\begin{figure}[!h]
\begin{center}
\includegraphics[width=16 cm]{./Results/skewness_results/skewness_methane/skewness_methane.pdf}
\caption[Skewness-methane-flux data]{Skewness of spatially averaged methane-flux data (red line) was compared with respect to that of 10000 Normal surrogates preserving $(A)$ Kendall correlation, and $(B)$ Spearman correlation. p-value < 0.05 in both case showed significant difference in skewness (as expected for lower tail dependent methane flux data from model selection results) from Normal surrogates. \label{fig_skewness_methane_maintext}}
\end{center}
\end{figure}
