...
(H) copulas with parameter 5 (Appendix \ref{SM-pedagog1_details}). (I,J) Reflections
...
Second, we did a test of the independence of the $u_i$ and $v_i$ (implemented in `BiCopIndTest` in the `VineCopula` package). 
<!--***DAN: X and Y to be flled in, can't remember right now what they are and don't want to stop
to look it up, Shya, you could also fill these in if you get a chance
***Shya: Done.-->
Subsequent model 
...
We also defined the statistic $\Ps_{l_b,u_b}$ (Appendix \ref{SM-nonparam_stats}), 
which has a similar 
...
present results in detail for an example bivariate dataset; followed by a summary of results
for all bivariate datasets; then we present results in 
detail for an example multivariate dataset; followed by a summary of results 
for all multivariate datasets.

<!--bivariate example-->
We discuss the example of the soil C and N data (see Data), pictured in 
...
(Fig. \ref{fig_biv_multi_raw_cop}E).
...
of Q1 for these data. The values are also substantially 
distinct from each other, answering the third part of Q1. 

<!--***DAN: made it to here 2018 12 20 920pm-->

Although these results do convincingly show non-normal copula structure, 
we caution that the model-averaged tail-dependence statistics may be biased 
...
\caption{Upper panels shows raw data plots for (A) soil C and N data, (B) $log_{10}$(basal metabolic rate (BMR)) vs. log(body mass) 
for birds, (C) log(BMR) vs. log(body mass) for mammals, and (D) above-ground plant biomass vs. Shannon's index (H) from Cedar Creek. Bottom panels show the corresponding copula plots. \label{fig_biv_multi_raw_cop}}
\end{center}
\end{figure}
<!--***DAN: Shya, we need to produce a single-pdf figure using R, not use latex to
cobble together separate pdfs for each panel. Also, I think we can dispense with the 
plots of the multivariate data. I have created a script that will do both of these things, 
and inserted a call to it in the sup mat Rmd. Right now it only plots dummy, filler data. 
Please look at MakeBivarDataPlot.R (that's the script) and search for
"Shya", and do as it says in those comments, so the actual data are imported into that
script and plotted. I have changed the above so this new figure is plotted instead of the
cobbled together old figures.
***Shya: filled in MakeBivDataPlot.R : Done.-->
...
knitr::kable(tab_bivar_summary, 
             format="latex",
             caption = "Summary of Q1 results for bivariate datasets. The $p$-values in rows 5-6 and are for the minimum-AIC copula. Model averaging used for rows 7-9 was based on AIC weights. For row 10, the statistic $P_l-P_u$ was computed for real data and for 1000 surrogate datasets with N copulas, and the rank of real-data value in the distribution of surrogate values is presented. \\label{tab_bivar_summary}",
