...
<!--***DAN: 
1) Shya put concepts on why we would expect lt dependence of noise to mean
more ext risk in a metapop context
2) then methods for how we test that (present the model, say what sims you did-->

On a second thought, we addressed the question how asymmetry in 
tail-dependence of environmental
variable could influence the extinction risk from a meta-population
context. We hypothesized to get higher 
extinction risk if the environmental noises from multiple patches
were more correlated to their smaller values 
(resulting left-tail dependence
in copula structure) than larger. This idea came from the context that
"bad" years (e.g. extreme cold) would have adverse affect 
on all metapopulation patches simultaneously, so there would 
not be enough neighboring-populations to rescue the "extinct"
patches. But in "good" years as the environmental variables 
from multiple locations were not correlated strongly, some of
extinct metapopulation-patches could be rescued by dispersal from
other patches. We tested our hypothesis in the following section.
...
<!--***DAN: Shya to insert extinction risk results with one fig-->
We tested our hypothesis of asymmetric copula-effect on extinction-risk
as discussed in
earlier section, with Lewinton-Cohen metapopulation
model (Eq. \ref{eq.extrisk_LC}), where population density 
$P$ could update exponentially depending on its intrinsic rate $r$.
$\epsilon(t)$ represent environmental noise; they are standard-normal 
random variables that are independent for distinct times, t, but 
that were made to exhibit either extreme left tail or right tail dependence
across patches.
We considered metapopulation patches were arranged in
a linear chain fashion
and the population may disperse to its nearerst
two neighbors (local dispersion) or to any patches (global dispersion); for code see [`mydispersal.R`](https://github.com/sghosh89/BIVAN/blob/master/mydispersal.R).

\begin{equation}\label{eq.extrisk_LC}
P(t+1) = \exp\left [  r + \epsilon(t) \right ] P(t)
\end{equation}

We simulated Eq. \ref{eq.extrisk_LC} with local dispersal for $r=0$ and calculated extinction risk after 25 years
from $10000$ simulations for both left-tail and right-tail dependent environmental noise across $5$ patches. 
Whenever population, at any patch after each year, went below the extinction threshold ($=1$) we considered 
that patch for that time went extinct. We found, as we
