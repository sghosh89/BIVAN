...
\end{center}
\end{figure}
<!--***DAN: Shya, I changed the abbreviation for Kendall Correlation from KC to K in the 
caption, please change in the code that generates the figures, as well
***Shya: Done.-->
...
dependence for the soil C and N data, and this can also be seen visually in the extreme tails 
...
bivariate datasets, the two variables were always significantly related (1$^{st}$ row of 
Table \ref{tab_bivar_summary}). A non-normal copula 
always emerged as the lowest-AIC (highest-AIC-weight) copula 
(2$^{nd}$ row of Table \ref{tab_bivar_summary}). 
The N copula was always a poor fit compared to the highest-AIC-weight copula 
(3$^{rd}$ and 4$^{th}$ rows of Table \ref{tab_bivar_summary}), answering the first part of Q1. 
Upper- and lower-tail statistics (7$^{th}$ and 8$^{th}$ rows of 
Table \ref{tab_bivar_summary}) differed substantially from 0, the tail dependence statistics of 
the N copula, answering the second part of Q1; and also differed substantially from each other
(9$^{th}$ row of Table \ref{tab_bivar_summary}),
answering the third part of Q1. The first three datasets had stronger upper- than lower-tail dependence, and the last dataset had the reverse. 
...
(5$^{th}$ and 6$^{th}$ rows of Table \ref{tab_bivar_summary}).
But our nonparametric statistics confirmed the same conclusions (10$^{th}$ row of 
Table \ref{tab_bivar_summary}).
...
The model-averaged lower-tail dependence statistic minus the model-averaged 
upper-tail dependence statistic (Table \ref{SM-tab_count_LTmUT}) was positive 
...
<!--Non-parametric analysis of tail dependence, green spruce abundance data-->
