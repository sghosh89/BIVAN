...
### Model selection approach
```{r bivmodselect_Loecke, echo=F, cache=T, cache.extra=list(seed,v,mtime(list("OurBiCopSelect.R")))}
set.seed(seed)
...
<!--DAN: wherever copsync is needs to be sourced and also added to the mlist below-->
```{r stat_soilCN, echo=F, cache=T, cache.extra=list(seed,v,mlist(list("CopulaFunctions.R","copsurrog2d.R")))}
set.seed(seed) 
...
surv<-copsurrog2d(v,cop,1000)
#DAN: once you get the code for copsync, apply it to each surrogate and keep the results
```

<!--DAN: modify the table so it also shows tail asymmetry stats (differences) and also shows fractions of surrogates for both the tail dependence stats and the asymmetry stats that are greater than the real data stat-->
```{r table_stat_soilCN, echo=F, results="asis"}
table_nums("table_stat_soilCN",display=F)

knitr::kable(data.frame(stat=stat),caption = "Non-parametric stats on soil C/N dataset")
```

<!--DAN: The below will no longer be necessary once you make the above changes-->
```{r ,echo=F}
#sCN
CorlmCoru<-stat_soilCN$Corl-stat_soilCN$Coru
cat("Corl-Coru = ",CorlmCoru,"\n")
PlmPu<-stat_soilCN$Pl-stat_soilCN$Pu
cat("Pl-Pu = ",PlmPu,"\n")
TlmTu<-stat_soilCN$Tl-stat_soilCN$Tu
cat("Tl-Tu = ",TlmTu,"\n")
D2umD2l<-stat_soilCN$D2u-stat_soilCN$D2l
cat("D2u-D2l = ",D2umD2l,"\n")
```
