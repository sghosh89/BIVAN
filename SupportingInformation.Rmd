...
\caption{\textbf{(A)} Plot of the raw soil C and N data, \textbf{(B)} Copula plot of soil C and N data. Obtained by applying ranks separately to the C and N data, and then dividing the ranks by $n+1$ where $n = `r nrow(d)` $ is the number of sampling locations (see Sec. \ref{Model_selection}).\label{fig_raw_cop_soilCN}}
\end{center}
\end{figure}
...
\caption{\textbf{(A)} Plot of the raw BMR vs. ln(mass) data \textbf{(B)} Copula plot for BMR dataset. Obtained by applying ranks separately to each variable, and then dividing the ranks by $n+1$ where $n=`r nrow(v_BMR)`$ is the number of contrasts (see Sec. \ref{Model_selection}).\label{fig_raw_cop_BMR}}
\end{center}
\end{figure}
...
\caption{\textbf{(A)} Plot of the raw AVG.ltoe.IV vs. AVG.humerus phylogenetic traits. \textbf{(B)} Copula plot of AVG.ltoe.IV vs. AVG.humerus phylogenetic traits. Obtained by applying ranks separately to each variable, and then dividing the ranks by $n+1$ where $n=`r nrow(v_contrasts_67)`$ is the number of contrasts (see Sec. \ref{Model_selection}). \label{fig_raw_cop_AVG_humerus_ltoe}}
\end{center}
\end{figure}
...
We also tested statistically for asymmetry of tail dependence, which seems visible in Figure \ref{fig_raw_cop_soilCN}(B). 
...
and got AIC values and upper- and lower-tail dependency estimates based on the fitted copulas (Table \ref{tab_BMRfit}). The table shows that Frank is much better supported (higher AIC and BIC weights) than the Gaussian copula, answering the first part of question 1 for these data.
...
The model-averaged (AIC weights used for model averaging) lower-tail and upper-tail dependence statistic was $0$ as expected, because the contributions came from Frank and SBB8 which are both symmetric copula.
<!--$`r BivMS_BMR$relLTdep_AICw`$ and the model-averaged upper-tail dependence statistic was 
$`r BivMS_BMR$relUTdep_AICw`$. These values are distinct from what a Gaussian copula would give (i.e., 0), answering the second part of question 1 for these data. Furthermore, the difference of these two quantities (lower minus upper), 
$`r BivMS_BMR$relLTdep_AICw-BivMS_BMR$relUTdep_AICw`$, was different from 0, answering the third part of question 1. 
-->
We also tested that lowest-AIC copula Frank was a good-enough fit, giving $p$-value 
$`r BivMS_BMR$GofRes_CvM`$ (to within the precision available from `BiCopGofTest` in the `VineCopula` package) in the Cramer-von Mises-based test of the goodness of its fit, and $p$-value $`r BivMS_BMR$GofRes_KS`$ in the Kolmogorov-Smirnov-based test of the goodness of fit. The next section looks at tail dependence in a non-parametric way.
...
![Nonparametric tests for tail dependence and other deviations from a normal copula for BMR data. As described in Methods, each statistic was computed on the data using the ranges $0-0.1$, $0.1-0.2$, ..., $0.9-1$ (red lines), and was also computed using the same ranges for 1000 normal-copula surrogate datasets (dashed lines show 0.025 and 0.975 quantiles of these results and dotted lines show 0.005 and 0.995 quantiles). \label{fig_BMR_nonparam}](./Results/BMR_results/BMR_numbin_10_bivfunctionplot.pdf)


![Nonparametric tests for tail dependence and other deviations from a normal copula for BMR data. As described in Methods, each statistic was computed on the data using the ranges $0-0.2$, $0.2-0.4$, ..., $0.8-1$ (red lines), and was also computed using the same ranges for 1000 normal-copula surrogate datasets (dashed lines show 0.025 and 0.975 quantiles of these results and dotted lines show 0.005 and 0.995 quantiles). \label{fig_BMR_nonparam5}](./Results/BMR_results/BMR_numbin_5_bivfunctionplot.pdf)
...
The test of indepedence gave $p=$ `r BivMS_contrasts_67$IndepTestRes` (to within the precision available 
from `BiCopIndTest` in the `VineCopula` package). We fitted several copulas
and got AIC values and upper- and lower-tail dependency estimates based on the fitted copulas (Table \ref{tab_contrasts_67fit}). The table shows that several copulas are much better supported (higher AIC and BIC weights) than the Gaussian copula, answering the first part of question 1 for these data.
...
![Nonparametric tests for tail dependence and other deviations from a normal copula for contrasts_67 data. As described in Methods, each statistic was computed on the data using the ranges $0-0.1$, $0.1-0.2$, ..., $0.9-1$ (red lines), and was also computed using the same ranges for 1000 normal-copula surrogate datasets (dashed lines show 0.025 and 0.975 quantiles of these results and dotted lines show 0.005 and 0.995 quantiles). \label{fig_BMR_nonparam}](./Results/Anolis_results/contrasts_csv_results/contrasts_67_bivfunctionplot.pdf)
...
set.seed(seed=101)
#---------------------------------------
pdf("./Results/Cause4copula_results/compare_params_pGOF_num_keep_last_500.pdf", width=12, height=6)
op<-par(mfrow=c(2,4),mar=c(3,3.5,3,3.5), mgp=c(1.5,0.5,0))

Plotter_Cause4copula_GOF(N=10000,fcode=3,method = "spearman",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=13,method="spearman",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=4,method = "spearman",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=14,method="spearman",num_keep_last=500,BS=1000)

Plotter_Cause4copula_GOF(N=10000,fcode=3,method = "kendall",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=13,method="kendall",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=4,method = "kendall",num_keep_last=500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=14,method="kendall",num_keep_last=500,BS=1000)
...
legend("top", c("noise","population", "p_CvM", "p_KS"), col = c("red", "blue", "magenta", "green2"),
       cex = 1.5, pch = c(2, 6, 16, 16), lty = c(1, 1, NA, NA), xpd = TRUE, horiz = TRUE, inset = c(0,0), 
       bty = "n") 
par(op)
dev.off()
```
...
#----------------------------
pdf("./Results/Cause4copula_results/compare_params_pGOF_num_keep_last_2500.pdf", width=12, height=6)
op<-par(mfrow=c(2,4),mar=c(3,3.5,3,3.5), mgp=c(1.5,0.5,0))

Plotter_Cause4copula_GOF(N=10000,fcode=3,method = "spearman",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=13,method="spearman",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=4,method = "spearman",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=14,method="spearman",num_keep_last=2500,BS=1000)

Plotter_Cause4copula_GOF(N=10000,fcode=3,method = "kendall",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=13,method="kendall",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=4,method = "kendall",num_keep_last=2500,BS=1000)
Plotter_Cause4copula_GOF(N=10000,fcode=14,method="kendall",num_keep_last=2500,BS=1000)
...
cause4copula_stat_C_spear chunk: `r Sys.time()`

```{r cause4copula_stat_C_spear, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#---------------------------------------
pdf("./Results/Cause4copula_results/Clayton_spearman.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=3,method="spearman",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_C_kend chunk: `r Sys.time()`

```{r cause4copula_stat_C_kend, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#---------------------------------------
...
cause4copula_stat_SC_spear chunk: `r Sys.time()`

```{r cause4copula_stat_SC_spear, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#---------------------------------------
pdf("./Results/Cause4copula_results/SClayton_spearman.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=13,method="spearman",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_SC_kend chunk: `r Sys.time()`

```{r cause4copula_stat_SC_kend, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#----------------------
pdf("./Results/Cause4copula_results/SClayton_kendall.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=13,method="kendall",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_G_spear chunk: `r Sys.time()`

```{r cause4copula_stat_G_spear, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#----------------------
pdf("./Results/Cause4copula_results/Gumbel_spearman.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=4,method="spearman",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_G_kend chunk: `r Sys.time()`

```{r cause4copula_stat_G_kend, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#----------------------
pdf("./Results/Cause4copula_results/Gumbel_kendall.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=4,method="kendall",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_SG_spear chunk: `r Sys.time()`

```{r cause4copula_stat_SG_spear, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#----------------------
pdf("./Results/Cause4copula_results/SGumbel_spearman.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=14,method="spearman",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```

cause4copula_stat_SG_kend chunk: `r Sys.time()`

```{r cause4copula_stat_SG_kend, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("Cause4copula.R")
#-----------------
set.seed(seed=101)
#----------------------
pdf("./Results/Cause4copula_results/SGumbel_kendall.pdf")
Plotter_Cause4copula_stat(N=10000,numsim=50,fcode=14,method="kendall",lb=0,ub=0.1,num_keep_last=500)
dev.off()
```


![Results of comparison between the parameters of environmental noise generated from a specific family of copula and estimated sample parameter (with some standard error) of the affected populations (Initially 10000 points are generated but we kept last **500** points for both noise and population data for comparison) \label{fig_Cause4copula_params_GOF}](./Results/Cause4copula_results/compare_params_pGOF_num_keep_last_500.pdf)

![Results of comparison between the parameters of environmental noise generated from a specific family of copula and estimated sample parameter (with some standard error) of the affected populations (Initially 10000 points are generated but we kept last **2500** points for both noise and population data for comparison) \label{fig_Cause4copula_params_GOF}](./Results/Cause4copula_results/compare_params_pGOF_num_keep_last_2500.pdf)


![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Clayton** copula and of the affected populations for 9 equidistant **Spearman's Rho** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_C_spear}](./Results/Cause4copula_results/Clayton_spearman.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Clayton** copula and of the affected populations for 9 equidistant **Kendall's Tau** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_C_kend}](./Results/Cause4copula_results/Clayton_kendall.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Survival Clayton** copula and of the affected populations for 9 equidistant **Spearman's Rho** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_SC_spear}](./Results/Cause4copula_results/SClayton_spearman.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Survival Clayton** copula and of the affected populations for 9 equidistant **Kendall's Tau** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_SC_kend}](./Results/Cause4copula_results/SClayton_kendall.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Gumbel** copula and of the affected populations for 9 equidistant **Spearman's Rho** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_G_spear}](./Results/Cause4copula_results/Gumbel_spearman.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Gumbel** copula and of the affected populations for 9 equidistant **Kendall's Tau** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_G_kend}](./Results/Cause4copula_results/Gumbel_kendall.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Survival Gumbel** copula and of the affected populations for 9 equidistant **Spearman's Rho** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_SG_spear}](./Results/Cause4copula_results/SGumbel_spearman.pdf)

![Results of comparison between 3 types of correlation (i.e. Spearman, Kendall, Pearson) and 6 non-parametric stats ($\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_u$ and $\Dsq_l$) of environmental noise generated from **Survival Gumbel** copula and of the affected populations for 9 equidistant **Kendall's Tau** values in a range (0.1 to 0.9) (Initially 10000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_SG_kend}](./Results/Cause4copula_results/SGumbel_kendall.pdf)
