...
All branches of ecology and related areas study relationships among environmental and biological variables. However, ubiquitously used approaches to studying such relationships, which are typically based on correlation and regression, are limited, providing only a small slice of the complex information content of the dependence. Well-developed statistical tools do exist that provide a demonstrably complete description of the dependence between two or more variables. The tools are based on the concept of the *copula*, and have been applied to great benefit in finance, neuroscience, and other fields, but not in ecology. We here examine the potential for copula approaches to improve our understanding across multiple disciplines of ecology, using environmental, species-trait, phenological, population, community, and ecosystem functioning data, as well as models that can be widely applied across ecology. We find that informative copula structure of dependence between variables is exceedingly common across all the disciplines we considered. An important feature in numerous datasets is asymmetric tail dependence, whereby two variables are more strongly related in their left compared to right tails, or vice versa. Copula structure and tail dependence can help reveal causal relationships between variables. Via models, we describe two mechanisms by which observed copula structures and tail dependence can arise in ecological data: a Moran-like effect, whereby dependence structures between environmental variables are inherited by ecological variables influenced by the environmental variables; and via asymmetric or nonlinear influences of environmental variables on ecological variables, such as under Liebig's law of the minimum. Via models and data, we describe important consequences of copula structure for applications and our understanding of ecology, including: impacts of copula structure on the stability through time of ecosystem service provisioning and spatially averaged population levels; and impacts on extinction risk. By documenting the importance of a complete statistical description of dependence, advancing conceptual frameworks, and demonstrating powerful methods, we aim to help elevate copula approaches to widespread use in studying ecological phenomena.
...
of ecology. Via basic mathematical models, we hypothesized two probable
mechanisms causing those non-N copula structures with asymmetric tail-dependence. One
mechanism was "Moran"-like effect where dependence structure between ecological
variables from two locations was shaped by the dependence structure of 
the environmental variables at those patches. Second mechanism, a mimic of 
Liebig's law of the minimum, explored
the asymmetric influence of environmental variables which 
could limit the growth of populations when below (or beyond)
a certain threshold. Using model simulations and real ecological data,
we concluded asymmetric tail dependence feature of 
non-N copula could affect (1) the skewness (henceforth, the stability)
of spatially averaged quantities over years, (2) the extinction risk 
and (3) the spatial Taylor's law slope of metapopulations. 
...
<!--***DAN: some notes by which to revise this para, Shya:
1) no need to restate methods at all, the methods are not among our main results in this paper
2) No real need to say much about why it is important to study what we stdied, that was done in the intro. Except insofar as saying what our main results are should make it clear it is an important thng to study, and see also further notes below.
3) Some additional points below
-->
<!--
Most studies in ecology have focused on measuring correlations between variables and thus ignoring the possibility to recognize importance of stronger association between extreme values. In the present study, we introduced the concept of asymmetric tail-dependence and non-Normal copulae in this field. -->
<!--***DAN: Too discursive. We do not need to repeat that we introduced this. Just summarize our main results in pothy language! And then maybe emphasize the importance, or make any additional points about importance that could not be made in the Intro. This para needs to be hard-hitting and brief, saying all the important thigs we accomplished in short form, all in one place. See below for some examples.-->

<!--We showed by examples of several real data sets (Table \ref{tab_bivar_summary} and Table \ref{tab_multivar_summary}) that non-Normal copulae with specific tail dependence are common in nature and could tell us about the nature of interaction between different ecological variables.-->

<!--***DAN: How about simply: "We showed that non-N copula structure and asymmetric tail dependence are exceedingly common in datasets that range across multiple sub-disciplines of ecology." See if you can write 1-3 pithy, impactful sentences like this one for Q2, and 1-3 for Q3 as well. Look at the first para of Discussion for some of my recent papers to get examples of the writing techniques for how we have done this in the past.-->

<!--We applied two methods. First, we did a model selection approach based on maximum likelihood which fit the real copula data with an optimal set of 16 copulae family. Copula having highest AIC-weight came as best-fit from this approach. Though we did goodness of fit test based on two statistics (Cramer-von Mises and Kolmogorov-Smirnov), we wanted to ensure the reliability of our model selection results. So, on a second note, we measured the tail dependence at both lower and upper end of a copula by three different nonparametric statistics ($\cor$, $\Ps$ and $\Dsq$) which we defined and we found they were consistent with the inferences we drew from model selection results. -->
<!--***DAN: No need at all to summarize methods again, except in cases where the methods are 
among the main products of the paper - not our case. So delete the above text. If you want to discuss our methods, do in later in the Discussion, not in this first paragraph, unless you may want to have a sentence saying something like "We synthesized and expanded upon methods for statistically assessing copula structure and tail dependence, and models for exploring the causes and consequences of copula strucuture in ecology and related fields."-->
...
<!--***DAN: I do not see how these examples go beyond the examples of the introduction. If they do, please clarify. If they don't, please remove.-->

<!--***DAN: Shya, the below is more summary of results. Pls summarize very briefly in the first paragraph of the Discusion ONLY. So most of the below won't be needed, but some could be incorporated into the first Discussion para above.-->
...
We thank the many contributors to the large datasets we have used; D. Stevens and P. Verrier for data extraction; and Jonathan Walter, Thomas Anderson, and Lei Zhao for helpful suggestions. We especially thank James Bell of the Rothamsted Insect Survey (RIS). 
