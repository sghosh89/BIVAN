...
\caption{Bivariate datasets showing diverse relationships between 
variables; some of these datasets nevertheless have the same Pearson (P)
or Spearman (S) correlation coefficients. Subscripts h and v 
...
(D) copulas with parameter 5 (see section on methods for Q1), together
...
Fig. \ref{fig_cop_pedag1}C-D (the points), which show the normalized ranks of 
...
data contain dependency information distinct from what can be modelled with
standard approaches. (Q2) If the answer to
Q1 is "yes," what are some possible causes/mechanisms of non-normal
copula structure and tail dependence? And (Q3) what are the 
...
but have been used only rarely so far in ecology (but see @Valpine2014, 
@Anderson2018 and @Popovic2018). 
We argue that benefits wider usage of complete 
...
function of the random variable $(U,V)=(F_X(X),F_Y(Y))$ 
(Appendix \ref{ContStrictMonoton}). And 
...
(Appendix \ref{ContStrictMonoton}).
...
distributions and by specifying the dependence structure. The theorem
also make it possible to study and model dependence independently 
...
modelling the marginal distributions and then separately modelling 
...
Good idea. Steps: split the current Fig 1 so the new Fig 1 has the first four panels,
but C-D should only have the points. Then make a Fig 2 as above, and edit the
text in the above para to use that figure. 
The rest of the panels of the current Fig 1 will make a new Fig 3 later.-->
...
2) What good are copulas - intuitive idea, basically, but also have to be linked to 
the formalities of the definition

What some of the different copula families look like
...
What tail dependence actually is
-->
...
We introduce the copula families and 
measures of tail dependence we used.
...
\noindent \textbf{Figure 1.} (A-D) Bivariate datasets showing diverse relationships between 
...
\noindent \textbf{Figure 2.} Summary and guide for analyses presented in the text. Middle boxes
correspond to ecological datasets for which Q1 was examined (in the 
second through
fourth sections of the text, see also Table \ref{tab_data_info}). Upper boxes 
correspond to potential causes of non-normal (non-N) copula structure that
...
sections of the text). Arrow labels (A-D for analyses pertaining 
to causes, Q2; and X-Z for analyses pertaining to consequences, Q3) 
link to locations in the text.

\hspace{1in}

\noindent \textbf{Figure 3.} Upper panels show raw data plots for (A) $\log_{10}$(soil C) and $\log_{10}$(soil N) data, (B) $\log_{10}$(basal metabolic rate (BMR)) vs. $\log_{10}$(body mass) 
for birds, (C) the same for mammals, and (D) above-ground plant biomass vs. Shannon's index (\textit{H}) from Cedar Creek. Bottom panels show the corresponding copula plots. See Data section for units used in upper panels.

\hspace{1in}

\noindent \textbf{Figure 4.} If environmental effects operate asymmetrically in their tails on
ecological variables, it can result in non-N copula structure and asymmetric 
tail dependence across space in the ecological variables. Shown are the last
...
\noindent \textbf{Figure 5.} Example results showing similarity of copula statistics for 
environmental-noise inputs and ecological-variable outputs of a dynamical model.
Asymmetric tail dependence was similar in both noise inputs and model outputs
for this model. The 
AR(1) model and a Clayton copula were used (see methods section for Q2).
...
Figs \ref{SM-fig_Cause4copula_SC_kend_ar1}-\ref{SM-fig_Cause4copula_SJ_kend_ar1} for AR(1) results with other copula families, and 
Figs \ref{SM-fig_Cause4copula_C_kend_sRicker_nsd_0.1}-\ref{SM-fig_Cause4copula_SJ_kend_sRicker_nsd_1} for
the nonlinear models described in the methods section for Q2.

\hspace{1in}

\noindent \textbf{Figure 6.} Skewness of spatially averaged green spruce aphid abundance $(A)$, leaf-curling plum 
aphid first flight dates $(B)$, \emph{Ceratium furca} abundance $(C)$, and methane-flux
...
\noindent \textbf{Figure 7.} Extinction risk for the metapopulation extension of the Lewontin-Cohen model, after 
$25$ time steps, was higher for left-tail-dependent environmental noise. Dispersal was local,
with dispersal rate $d$ (Appendix \ref{SM-ext_risk_model}). Simulations used $N$ patches
for $N=5$ (A) and $N=25$ (B).