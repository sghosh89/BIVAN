...
#---------------------------------------------------------------
#checkloc<-tempdir()
#if (!dir.exists(paste0(checkloc,"/.checkpoint/")))
#{
#  dir.create(paste0(checkloc,"/.checkpoint/"))
#  checkpoint("2018-03-29",checkpointLocation = checkloc)
#}
#---------------------------------------------------------------
...
## Abundance data for *Ceratium furca* in UK seas \label{Pns_data}
   ...
   * note, when we edit this, we cannot call it the North Seas since data come from around the UK not only the North Sea, unless Shya threw out all grid cells that were outside the North Sea
...
$\Dsq_u-\Dsq_u$ revealed the asymmetry of tail dependence known to be present in data simulated
...
statistics $\cor_l-\cor_u$, $\Ps_l-\Ps_u$, and $\Dsq_u-\Dsq_u$. This exercise 
...
```{r stat_testing_save_result, message=F, echo=F, cache=T, cache.extra=list(seed,mtime("TestStats_multivar_dataset.R"),mtime("CopupulaFunctions.R"),mtime("CopulaFunctions_flexible.R"))}
set.seed(seed)
...
```{r stat_testing_plot, results='hide', echo=F, cache=T, cache.extra=list(seed,callfn,mtime("StatTestNumericResults.RData"),mtime("TestStats_multivar_dataset.R"))}
...
![Results of tests of whether $\cor_l-\cor_u$, $\Ps_l-\Ps_u$, and $\Dsq_l-\Dsq_u$ reveal the know asymmetry of tail dependence of the Clayton and survival Clayton copulas, and the known symmetry of tail dependence for the normal and Frank copulas. See text for details. $n=35$. \label{fig_stat_testing35}](./Results/stat_results/stat_testing/res_pt35_rankT.pdf)
...
```{r stat_aphid_count, echo=F, cache=T, cache.extra=list(seed,data_aphid_count,mtime("NonParamStat.R"))}
set.seed(seed) 
...
<!--Shya, can you please organize these following plots into one plot with mutliple panels? Once that is done, please add some text describing these results.-->
\begin{figure}[!h]
\begin{center}
\textbf{\;\;\;\;\;\;\;\ (A) \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\ (B)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_Spearman_vs_D.pdf}
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_Kendall_vs_D.pdf}\\
\textbf{\;\;\;\;\;\;\;\ (C) \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\ (D)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_Corl-Coru_vs_D.pdf}
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_Pl-Pu_vs_D.pdf}\\
\textbf{\;\;\;\;\;\;\;\ (E)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_D2u-D2l_vs_D.pdf}
\caption{Non-parametric tail statistics results for green spruce aphid abundance data plotted against geographical distance $(D)$ between two sampling locations; $\textbf{(A)}$ Spearman correlation vs. $D$, $\textbf{(B)}$ Kendall correlation vs. $D$, $\textbf{(C)}$ $\cor_l-\cor_u$ vs. $D$, $\textbf{(D)}$ $\Ps_l-\Ps_u$ vs. $D$, $\textbf{(E)}$ $\Dsq_u-\Dsq_l$ vs. $D$ plotted for all (10 $\times$ 10 locations) taken pairwise. \label{fig_aphid_count}}
\end{center}
\end{figure}

We computed Spearman and Kendall correlation along with our three non-parametric statistics ($\cor_l-\cor_u$, $\Ps_l-\Ps_u$, $\Dsq_u-\Dsq_l$) as described in Sec. \ref{NPA} for  green spruce aphid abundance data taken at all $10$ $\times$ $10$ location pairs (code given in [`NonParamStat.R`](https://github.com/sghosh89/BIVAN/blob/master/NonParamStat.R)). We also constructed a $10$ $\times$ $10$ matrix using `gcdist` from `ncf` package where each element in that matrix represented the geographical distance between any two selected locations. 
Figure \ref{fig_aphid_count} presented plots for **(A)** Spearman, **(B)** Kendall, **(C)** $\cor_l-\cor_u$, **(D)** $\Ps_l-\Ps_u$ and **(E)** $\Dsq_u-\Dsq_l$ statistics against the geographical distance for all selected sampling location pairs. With increasing the distance *D*, the Spearman and Kendall correlation (hence the synchrony among time series data) between the location pairs decreased as expected. But one cannot conclude the tail dependence of that synchrony from Figure \ref{fig_aphid_count} **(A)** or **(B)**, whereas **(C - E)** clearly showed the lower tail dependence with most of the data points lied in the positive quadrant as described before in Sec. \ref{NPA}.
...
\textbf{\;\;\;\;\;\;\;\ (E)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_count/Sp_10_D2u-D2l_vs_D.pdf}
\caption{Results for Aphid-count}
\end{center}\label{fig_aphid_count}
\end{figure}
...
knitr::kable(RES_aphid_ff$LTdep_AICw-RES_aphid_ff$UTdep_AICw,
             digits = 5,
             format='pandoc',
             caption = "Model averaged lower tail dependence minus model averaged upper tail dependence statistic for each location pair for each location pair for first flight data for the leaf-curling plum aphid.\\label{tab_ff_LTmUT}",
             booktabs=TRUE
             )
```
...
<!--Do the analysis-->
```{r stat_aphid_ff, echo=F, cache=T, cache.extra=list(seed,data_aphid_ff,mtime("NonParamStat.R"))}
set.seed(seed)
...
\begin{center}
\textbf{\;\;\;\;\;\;\;\ (A) \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\ (B)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_ff/Sp_11_Spearman_vs_D.pdf}
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_ff/Sp_11_Kendall_vs_D.pdf}\\
\textbf{\;\;\;\;\;\;\;\ (C) \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\ (D)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_ff/Sp_11_Corl-Coru_vs_D.pdf}
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_ff/Sp_11_Pl-Pu_vs_D.pdf}\\
\textbf{\;\;\;\;\;\;\;\ (E)} \\
\includegraphics[width=2in]{./Results/stat_results/stat_aphid_ff/Sp_11_D2u-D2l_vs_D.pdf}
\caption{Non-parametric tail statistics results for leaf-curling plum aphid's first flight date data plotted against geographical distance $(D)$ between two sampling locations; $\textbf{(A)}$ Spearman correlation vs. $D$, $\textbf{(B)}$ Kendall correlation vs. $D$, $\textbf{(C)}$ $\cor_l-\cor_u$ vs. $D$, $\textbf{(D)}$ $\Ps_l-\Ps_u$ vs. $D$, $\textbf{(E)}$ $\Dsq_u-\Dsq_l$ vs. $D$ plotted for all (10 $\times$ 10 locations) taken pairwise. \label{fig_aphid_ff}}
\end{center}
\end{figure}

Figure \ref{fig_aphid_ff} represented plots for **(A)** Spearman correlation, **(B)** Kendall correlation, **(C)** $\cor_l-\cor_u$, **(D)** $\Ps_l-\Ps_u$ and **(E)** $\Dsq_u-\Dsq_l$ statistics, computed on leaf-curling plum aphid's first flight date data, against the geographical distance for all selected sampling location pairs. Likewise in Figure \ref{fig_aphid_count}, with increasing the distance *D*, the Spearman and Kendall correlation (hence the synchrony among time series data) between the location pairs decreased as expected. But here in contrast, Figure \ref{fig_aphid_ff}  **(C - E)** clearly showed the upper tail dependence with most of the data points lied in the negative quadrant as described before in Sec. \ref{NPA}.
...
As in previous sections (see also Methods) model selection among copulas was done separately for 
...
knitr::kable(RES_plankton_north_sea$LTdep_AICw,
             format="pandoc",
             digits=3,
             caption = "Plankton_north_sea : model averaged lower tail dependence statistic for each selective location pair\\label{tab_pns_LTdep_AICw}",
             booktabs=TRUE)
```
...
knitr::kable(RES_plankton_north_sea$UTdep_AICw,
             format="pandoc",
             digits=3,
             caption = "Plankton_north_sea : model averaged upper tail dependence statistic for each selective location pair\\label{tab_pns_UTdep_AICw}",
             booktabs=TRUE)
```
...
knitr::kable(RES_plankton_north_sea$LTdep_AICw-RES_plankton_north_sea$UTdep_AICw,
             format="pandoc",
             digits=3,
             caption = "Plankton_north_sea : model averaged lower tail - model averaged upper tail dependence statistic for each of the selective location pair\\label{tab_pns_LTmUT}",
             booktabs=TRUE)
```
...
```{r stat_plankton_north_sea, echo=F, cache=T, cache.extra=list(seed,data_plankton_north_sea,mtime("NonParamStat.R"))}
set.seed(seed)
...
s<-stat_plankton_north_sea$numericdf[,2:5]
rownames(s)<-c("spear","kend","cor~l~","cor~u~","P~l~","P~u~","$D^2$~l~","$D^2$~u~","cor~l~ - cor~u~","P~l~ - P~u~","$D^2$~u~ - $D^2$~l~")
knitr::kable(s,
             format = "pandoc",
...
![$cor_l$-$cor_u$ vs. Distance plot for Plankton_north_sea showing lower tail dependence\label{fig_pns_Corl-Coru}](./Results/stat_results/stat_plankton_north_sea/Sp_16_Corl-Coru_vs_D.pdf){ width=50%, height=50% }

![$P_l$-$P_u$ vs. Distance plot for Plankton_north_sea showing lower tail dependence\label{fig_pns_Pl-Pu}](./Results/stat_results/stat_plankton_north_sea/Sp_16_Pl-Pu_vs_D.pdf){ width=50%, height=50% }

![$D^2_u$-$D^2_l$ vs. Distance plot for Plankton_north_sea showing lower tail dependence\label{fig_pns_D2u-D2l}](./Results/stat_results/stat_plankton_north_sea/Sp_16_D2u-D2l_vs_D.pdf){ width=50%, height=50% }

Table \ref{tab_pns_resamp} shows the mean of each non-parametric statistic taken from all pairwise locations and then based on a resampling results how the resampled mean distributed from or around the real mean. Fourth column named $fracgt0$ in Table \ref{tab_pns_resamp} indicates the fraction of resampled mean greater then zero. $\cor_l-\cor_u$, $\Ps_l-\Ps_u$ and $\Dsq_u-\Dsq_l$ statistic with positive mean and almost $100$% $fracgt0$ proves that plankton species data for *Ceratium furca* has significant lower tail dependence.
...
### LC model :
```{r fig_risk_r0_numlocs5, echo=F, cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="LC model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0_numlocs5}", fig.height=3.5, fig.width=8}
set.seed(seed)
varying_d(numsims=10000,numsteps =25,numlocs=5,r=0,K=50,disp_everywhere=F)
```

```{r fig_risk_r0_numlocs25, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="LC model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0_numlocs25}", fig.height=3.5, fig.width=8}
set.seed(seed)
varying_d(numsims=10000,numsteps =25,numlocs=25,r=0,K=50,disp_everywhere=F)
```

  *  As expected, when extinction risk calculated after $25$ years, was plotted against each $d$ (=degree of dispersion) we varied, we found that extinction risk was higher for data came from left tail dependent environmental fluctuations than right tail dependent one. With increasing $numlocs$ from $5$ to $25$, extinction risk decreased [for a fixed $d$] ; [see Figure \ref{fig_risk_r0_numlocs5} - Figure \ref{fig_risk_r0_numlocs25}]. 
 ...
 *  For ricker model when extinction risk calculated after $25$ years, was plotted against each $d$ (=degree of dispersion) we varied, we found that extinction risk was higher for data came from left tail dependent environmental fluctuations than right tail dependent one upto $r=0.7$. For $r>0.7$, extinction risk is greater for data came from right tail dependent environmental fluctuations.
  ...
  * With increasing $K$ from $50$ to $200$, extinction risk in both case gets decreased as expected. [see Figure \ref{fig_risk_r0.05} - Figure \ref{fig_risk_r0.05_K200}]

 * With increasing $numlocs$ from $5$ to $25$, extinction risk decreased [for a fixed $d$] ; [see Figure \ref{fig_risk_r0.05} and Figure \ref{fig_risk_r0.05_numlocs25}]. 
 

```{r fig_risk_r0.05, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.05}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-5
K<-50
varying_d(numsims=10000,numsteps =25,numlocs=numlocs,r=r,K=K,disp_everywhere=F)
```

```{r fig_risk_r0.05_K200, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.05_K200}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-5
K<-200
...
```{r fig_risk_r0.05_numlocs25, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.05_numlocs25}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-25
K<-50
...
```{r fig_risk_r0.25, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.25}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.25
numlocs<-5
K<-50
...
```{r fig_risk_r0.5, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.5}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.5
numlocs<-5
K<-50
...
```{r fig_risk_r0.6, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.6}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.6
numlocs<-5
K<-50
...
```{r fig_risk_r0.7, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.7}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.7
numlocs<-5
K<-50
...
```{r fig_risk_r0.75, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.75}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.75
numlocs<-5
K<-50
...
```{r fig_risk_r0.9, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.9}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.9
numlocs<-5
K<-50
...
```{r fig_risk_r1.0, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.0}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.0
numlocs<-5
K<-50
...
```{r fig_risk_r1.25, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.25}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.25
numlocs<-5
K<-50
...
```{r fig_risk_r1.5, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.5}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.5
numlocs<-5
K<-50
...
```{r fig_risk_r1.75, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.75}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.75
numlocs<-5
K<-50
...
```{r fig_risk_r1.95, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.95}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.95
numlocs<-5
K<-50
...
# Explore equal_dispersal everywhere
...
### Ricker model :
  ...
  * With increasing $K$ from $50$ to $200$, extinction risk in both case gets decreased as expected. [see Figure \ref{fig_risk_r0.05_disp_everywhere} - Figure \ref{fig_risk_r0.05_K200_disp_everywhere}]

 * With increasing $numlocs$ from $5$ to $25$, extinction risk decreased [for a fixed $d$] ; [see Figure \ref{fig_risk_r0.05_disp_everywhere} and Figure \ref{fig_risk_r0.05_numlocs25_disp_everywhere}]. 

```{r fig_risk_r0.05_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.05_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-5
K<-50
varying_d(numsims=10000,numsteps =25,numlocs=numlocs,r=r,K=K,disp_everywhere=T)
```

```{r fig_risk_r0.05_K200_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal); With increasing K from 50 to 200, extinction risk decreases.\\label{fig_risk_r0.05_K200_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-5
K<-200
...
```{r fig_risk_r0.05_numlocs25_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal); With increasing numlocs from 5 to 25, extinction risk decreases.\\label{fig_risk_r0.05_numlocs25_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.05
numlocs<-25
K<-50
...
```{r fig_risk_r0.25_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.25_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.25
numlocs<-5
K<-50
...
```{r fig_risk_r0.5_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.5_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.5
numlocs<-5
K<-50
...
```{r fig_risk_r0.6_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.6_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.6
numlocs<-5
K<-50
...
```{r fig_risk_r0.7_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.7_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.7
numlocs<-5
K<-50
...
```{r fig_risk_r0.75_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.75_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.75
numlocs<-5
K<-50
...
```{r fig_risk_r0.9_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r0.9_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-0.9
numlocs<-5
K<-50
...
```{r fig_risk_r1.0_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.0_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.0
numlocs<-5
K<-50
...
```{r fig_risk_r1.25_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.25_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.25
numlocs<-5
K<-50
...
```{r fig_risk_r1.5_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.5_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.5
numlocs<-5
K<-50
...
```{r fig_risk_r1.75_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.75_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.75
numlocs<-5
K<-50
...
```{r fig_risk_r1.95_disp_everywhere, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R")), fig.cap="Ricker model_disp_everywhere : Plot Extinction risk vs. d(degree of dispersal).\\label{fig_risk_r1.95_disp_everywhere}", fig.height=3.5, fig.width=8}
set.seed(seed)
r<-1.95
numlocs<-5
K<-50
...
#because it comes from a multivariate normal
sig<-matrix(rho,n,n)
diag(sig)<-1
...
```{r tl_analysis, echo=F, cache=T, cache.extra=list(mtime("TaylorsLawTools.R"),seed,mtime("copsurrognd.R"))}
