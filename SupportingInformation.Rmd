...
the three non-parametric statistics $\cor_{0,0.2}-\cor{0.8,1}$, $\Ps_{0,0.2}-\Ps{0.8,1}$,
and $\Dsq_{0.8,1}-\Dsq_{0,0.2}$ for each copula. For each statistic and for each
dependence structure, we made a histogram (Fig. \ref{fig_cop_evo}). Values of these statistics less than 0
indicate lower-tail dependence in characters of extant species, whereas values greater
than 0 indicate upper-tail dependence. Asymmetry of tail dependence in evolutionary changes to characters
...
computed for each of 100 simulations of mammalian evolution for each of five types of tail dependence between 
...
Appendix \ref{evol_mech}. The number in the upper right of each panel indicates how many statistics out of $100$ were less than or equal to $0$. Values substantially lower (respectively, higher) than $50$ therefore indicate greater left (respectively, right) tail-dependence between characters.\label{fig_cop_evo}}
\end{center}
\end{figure}
