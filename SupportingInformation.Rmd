...
i<-1
  for(j in c(1:13)){
    ...
    #plot(v_phylo[,1],v_phylo[,2],col="green4")
    #mtext("(i,j)=(",i,", ",j,")")
...
dev.off()
#----------------------------------------------------------------------------------------
# Generating raw data plot btw AVG.humerus and AVG.radius from contrasts.csv
pdf("./Results/Anolis_results/contrasts_csv_results/rawdataplot_AVG_humerus_radius.pdf")
op<-par(mar=c(5.1, 4.1, 0.2, 2.1))
plot(phylo_dat$AVG.humerus,phylo_dat$AVG.radius,type="p",col="red",xlab="AVG.humerus",ylab="AVG.radius",
     cex.lab=1.5,cex.axis=1.5)
par(op)
dev.off()
#--------------------------------------------------------------------------------------------
# Generating copula plot btw AVG.humerus and AVG.radius from contrasts.csv
pdf("./Results/Anolis_results/contrasts_csv_results/copula_AVG_humerus_radius.pdf")
...
\textbf{ \hspace{5.5 cm} (A) \hspace{7 cm} (B)  } \\
\begin{center}
\includegraphics[width=3 in]{./Results/Anolis_results/contrasts_csv_results/rawdataplot_AVG_humerus_radius.pdf}
\includegraphics[width=3 in]{./Results/Anolis_results/contrasts_csv_results/copula_AVG_humerus_radius.pdf}
\caption{\textbf{(A)} Plot of the raw AVG.radius vs. AVG.humerus phylogenetic traits. \textbf{(B)} Copula plot of AVG.radius vs. AVG.humerus phylogenetic traits. Obtained by applying ranks separately to each variable, and then dividing the ranks by $n+1$ where $n=`r nrow(v_contrasts_67)`$ is the number of contrasts (see Sec. \ref{Model_selection}). \label{fig_raw_cop_AVG_humerus_radius}}
\end{center}
\end{figure}
...
knitr::kable(h,
             format='pandoc',
             caption = "Fitting info for contrasts between AVG.humerus and AVG.radius dataset. \\label{tab_contrasts_67fit}",
             booktabs=T)
```
...
```{r cause4copula, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("Cause4copula.R"),mtime("CopulaFunctions_flexible.R"))}
...
Plotter_Cause4copula_GOF(fcode=3,method = "spearman",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=13,method="spearman",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=4,method = "spearman",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=14,method="spearman",num_keep_last=500,BS=100)

Plotter_Cause4copula_GOF(fcode=3,method = "kendall",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=13,method="kendall",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=4,method = "kendall",num_keep_last=500,BS=100)
Plotter_Cause4copula_GOF(fcode=14,method="kendall",num_keep_last=500,BS=100)
...
![Results of comparison between the parameters of environmental noise generated from a specific family of copula and estimated sample parameter (with some standard error) of the affected populations (Initially 5000 points are generated but we kept last 500 points for both noise and population data for comparison) \label{fig_Cause4copula_params_GOF}](./Results/Cause4copula_results/compare_params_pGOF.pdf)
