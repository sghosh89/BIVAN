...
2) Go through the whole thing one more time
3) send it out to coauthors (done) and maybe get feedback
...
considered. Many datasets exhibited asymmetric tail associations, whereby two variables are more 
...
complex dependence structure of data, we have found tail association to be
a useful aspect of the full structure of dependence, so tail association 
...
introductions elsewhere. Instead our goal is to 
...
structure (see section on methods for Q1 below).
...
language. See Figs \ref{figped_N}-\ref{figped_BB1} and \ref{SM-figped_SC} and \ref{SM-figped_G}-\ref{SM-figped_SBB8} 
...
difference between these fits was not meaningfully large.
...
and comprehensive explorations of
...
disproportionately strongly in one of its tails, we explored whether the ecological
...
for ecological dynamics to influence how complex dependencies between
the $E_i(t)$ may arise: 
...
surrogate comparison methods (see section on methods for Q1) to these 
...
The nonlinear population model was a stochastic, multi-habitat-patch version of the Ricer model, 
...
the methods section for Q2 (i.e., Figs \\ref{fig_ms_asym_b_0.1} and 
...
(Fig. \ref{fig_Cause4copula_C_kend_ar1} for the Clayon copula, 
...
our correlation and tail-asymmetry statistics, generally 
...
See methods section for Q2 
for details.\label{fig_Cause4copula_C_kend_ar1}}
\end{center}
\end{figure}

For our evolutionary model, the hypothesis was 
correct that asymmetric tail association in evolutionary changes 
...
dependence structure we used on evolutionary changes, 
we had 817 bivariate characters for each of 500 simulations (see methods 
section for Q2). We 
...
this is a mechanism behind observed asymmetric tail associations
...
(E, J, O)). See the methods section for Q2 for details. The number above each panel indicates how many statistics out of $100$ were less than or equal to $0$. Values substantially lower (respectively, higher) than $50$ indicate greater left (respectively, right) tail-dependence between characters.\label{fig_cop_evo}}
\end{center}
\end{figure}
...
associations. As for Q2, models used are intended as initial explorations, only,
of whether copula structure may possibly have a meaningful influence on important ecological
...
multivariate normal distribtion), but the data were otherwise statistically
...
with $x_i(t)$ in the $t$th row and the $i$th column as a *population matrix*. 
We consider that 
...
3) the copula for the dependence between $m_{t,j_1}^{(\text{X},k)}$ and $m_{t,j_2}^{(\text{X},k)}$ was X.
...
X. Thus comparing how Taylor's law may manifest differently for $m^{(\text{X},k)}$
...
in different locations), $p$-values for the linearty
...
though results using Kendall-preserving surrogates were similar. See the methods
section for Q3 and Appendices \ref{SM-Consequence_TL_simulation} and 
...
can lead to "spiky" spatially averaged time series, with large outbreaks.
Thus tail associations could have implications for exploited and pest species.
Extinction risk and Taylor's law can also be altered by tail association patterns across space
...
complete copula characterization of dependence is employed. 
Copula approaches are
well developed [@nelsen2006_copula; @joe2014_dependence; @MaiScherer2017], statistically, 
...
asymmetric tail dependence should be special properties of datasets from temperate 
regions. The mechanisms we have proposed of non-normal copula structure and asymmetric tail 
dependence seem equally likely to apply anywhere. For instance, the Moran effect, which underpins 
...
another one of our proposed mechanisms (Fig. \ref{MasterFigureA}), are widely
demonstrated paradigms that apply to all biomes. 
...
may opperate commonly. It may be a dominant cause of 
...
may operate commonly: Moran effects are common [@Liebhold2004], and 
...
analyses in this study solely in order to have at least one
way to constrain the complexity of what is already a complex piece of work.
But the statistical theory on multi-dimensional copulas is well developed
...
bivariate copulas (e.g., the `VineCopula`
package for the R programming language). Such approaches may be an 
...
explore parameter space. We re-emphasize that fuller explorations in future work
of some of our models, and variant model, are likely to be informative. 
...
We also elaborated potential consequences of copula structure for ecological understanding. 
...
is strongly influenced by the temperature of the previous winter (@Sheppard2016,
...
and was processed in the same way as @Sheppard2016.
...
and nonparametric methods detailed in the section on methods for Q1. The column labeled p, CvM is the p-value 
...
mentioned in the Introduction, Background, and methods sections and 
...
goal) using normalized ranks, stating explicitly "statistical inference concerning
...
the maginal distributions of variables with information about the dependence
...
science [@Zale2013]. The CPUE measurements, which we denote $c_{1,t}$ and $c_{2,t}$ may be more
or less correlated through time than the true densities would be, if they were known,
...
between variables are influenced not only by the ordinal information in ranks but also 
by the cardinal information in the unranked data. Judgements of what statistics
...
in tail associations using cardinal data, a partial Pearson correlation can be developed
...
that may also have utility for some applications.
