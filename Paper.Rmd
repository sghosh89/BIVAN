...
features of ecological data, we explored via mathematical 
...
or flux of a biogeochemical variable. 

To explore the importance of nonlinear dynamics for our hypothersis, we also considered 
...
the weak-noise and strong-noise cases. The $\epsilon_i(t)$ were again standard
...
The second mechanism we will explore, which was also summarized in the 
Introduction,
...
local noise, independent across time and sampling locations. We used $b=0.1$ and
$a=0.2$, and $(\epsilon_1(t),\epsilon_2(t))$ were drawn, independently through
time, from a bivariate normal distribution with $\var(\epsilon_i)=1$ and 
$\cov(\epsilon_1,\epsilon_2)=0.8$. We used $g$ equal to $g_1$ or $g_2$ in two different simulations:
...
We simulated the model once using $g_1$ and once using $g_2$, for 5000 time
...
# Results for Q2: Moran-like effects and asymmetric dependencies \label{Results_Q2}
...
to model equilibrium. Results that hold for "weak noise" in this sense
...
at $K$ when $r<1$ (so-called under-compensating dynamics), but exhibits 
an oscillatory approach to a stable equilibrium at $K$ when $r>1$ (over-compensating
dynamics).
...
and weak-noise results, and, for a few simulations, tail dependence 
...
<!--***DAN: Right, Shya? Please replace X with the actual value(s)-->

Our model with environmental effects which differed in the
two tails produced simulated ecological variables with visually apparent asymmetry 
of tail dependence and non-N copula structure (Fig. \ref{fig_ms_asym_b_0.1}).
Model selection results (Table \ref{SM-tab_summary_stat_asym_sens}) and
our nonparametric statistics (Tables \ref{SM-tab_stat_npa_asym_1} -\ref{SM-tab_stat_npa_asym_2}) 
statistically confirmed the visual result.
We repeated our analyses with $b=-0.1, 0.9, -0.9$, with similar conclusions
in all cases.
<!--***DAN: Right Shya? I have written this, but I want you to show me these results
(without putting them in the paper) to make sure the statement is accurate.
But we should put the figs in the sup mat, not the stats results, I think, but we could put the figs. Or maybe we do want to put the stats. Let's look at them anyway. Maybe the sentence needs to
be rewritten if the results for other b are actually different.-->
...
\end{center}
\end{figure}
<!--***DAN: Shya, pls remove the left 4 panels, and also the headers of the right
two panels - we can put that information in the caption, as I have done-->
