...
The raw data looks like : `r fig_nums("soilCNraw",display="c")`.

The copula looks like : `r fig_nums("soilCNcop",display="c")`.
...
### Non-paramtric analysis of tail dependence
<!--Do the intensive part of the analysis-->
```{r stat_soilCN, echo=F, cache=T, cache.extra=list(seed,v,mtime("CopulaFunctions_flexible.R"),mtime("copsurrog2d.R"))}
set.seed(seed) 
resloc<-"./Results/stat_results/stat_soilCN/"

stat_soilCN<-stat_flexible_fn(m=v,lb=0,ub=0.1)
saveRDS(stat_soilCN,paste(resloc,file="stat_soilCN.RDS",sep=''))

stat<-unlist(stat_soilCN)
stat<-c(stat[1:4],unname(stat[3]-stat[4]),stat[5:6],unname(stat[5]-stat[6]),stat[7:8],unname(stat[8]-stat[7]))
names(stat)[c(5,8,11)]<-c("Corl-Coru","Pl-Pu","D2u-D2l")

#get surrogates and get stats for them, preserving kendall
cop<-normalCopula(.5,2)
numsurrog<-1000
surv<-copsurrog2d(v,cop,"kendall",numsurrog)
statsurr_k<-matrix(NA,length(stat),numsurrog)
rownames(statsurr_k)<-names(stat)
for (counter in 1:dim(surv)[3])
{
  h<-stat_flexible_fn(m=surv[,,counter],lb=0,ub=0.1)
  h<-unlist(h)
  h<-c(h[1:4],h[3]-h[4],h[5:6],h[5]-h[6],h[7:8],h[8]-h[7])
  statsurr_k[,counter]<-unname(h)
}

#same, preserving spearman
surv<-copsurrog2d(v,cop,"spearman",numsurrog)
statsurr_s<-matrix(NA,length(stat),numsurrog)
rownames(statsurr_s)<-names(stat)
for (counter in 1:dim(surv)[3])
{
  h<-stat_flexible_fn(m=surv[,,counter],lb=0,ub=0.1)
  h<-unlist(h)
  h<-c(h[1:4],h[3]-h[4],h[5:6],h[5]-h[6],h[7:8],h[8]-h[7])
  statsurr_s[,counter]<-unname(h)
}
```
...
stattab<-data.frame(statname=names(stat),stat=unname(stat),frac_surr_gt_k=NA,frac_surr_lt_k=NA,frac_surr_gt_s=NA,frac_surr_lt_s=NA)
for (counter in 1:length(stat))
{
  stattab[counter,3]<-sum(statsurr_k[counter,]>stat[counter])/numsurrog
  stattab[counter,4]<-sum(statsurr_k[counter,]<stat[counter])/numsurrog
  stattab[counter,5]<-sum(statsurr_s[counter,]>stat[counter])/numsurrog
  stattab[counter,6]<-sum(statsurr_s[counter,]<stat[counter])/numsurrog
}

#display
names(stattab)<-c("Statistic","Value on data","Frac_K_sur_gt","Frac_K_sur_lt","Frac_S_sur_gt","Frac_S_sur_lt")
table_nums("table_stat_soilCN",display=F)
knitr::kable(stattab,caption = "Results of non-parametric tests of tail dependence and asymmetry of tail dependence for the soil C and N dataset. Names and values on the actual C and N data are listed in the first two columns. The third (respectively, fourth) column is the fraction of Kendall-preserving surrogate datasets for which the given statistic is greater than (resp., less than) its value on the real data. The last two columns are analogous but for Spearman-preserving surrogates.")
```
...
```{r stat_aphid_count, echo=F, cache=T, cache.extra=list(seed,data_aphid_count,mtime("CopulaFunctions.R"),mtime("CopulaFunctions_flexible.R"),mtime("NonParamStat.R"))}
set.seed(seed) 
...
```{r stat_aphid_ff, echo=F, cache=T, cache.extra=list(seed,data_aphid_ff,mtime("CopulaFunctions.R"),mtime("CopulaFunctions_flexible.R"),mtime("NonParamStat.R"))}
...
### Non-paramtric analysis of tail dependence
```{r stat_plankton_north_sea, echo=F, cache=T, cache.extra=list(seed,data_plankton_north_sea,mtime("CopulaFunctions.R"),mtime("CopulaFunctions_flexible.R"),mtime("NonParamStat.R"))}
