...
Nonparametric statistics verified both that tail dependence was distinct from that 
of an N copula for the green spruce aphid abundance data, and that it was asymmetric.
Mean values across all pairs of locations of the statistics 
$\cor_l$, $\cor_u$, $\Ps_l$, $\Ps_u$, $\Dsq_l$, and $\Dsq_u$ were positive and
confidence intervals excluded $0$ (the value for an N copula), confirming tail 
dependence distinct from that of an N copula (Table \ref{SM-tab_count_resamp}). 
...
elaborated in detail above, the table also summarizes results of analyzing 
leaf-curling plum aphid first flight data, abundance data for the dinoflagellate
\textit{Ceratium furca}, and methan flux data (see Data).  


We did model selection and non-parametric analysis with all 20 aphid and 22 plankton species and found our two approaches were consistent to each other. Here, to present our results we chose one species from each category. 
...
aphids species for which we had data other than the green spruce and leaf-curling plum aphids,
...
dt <- tibble(
           ...
           "7. 2.5th and 97.5 quantiles of model-avg. lower-tail dependence",
           "8. 2.5th and 97.5th quantiles of model-avg. upper-tail dependence",
           "9. Percent pairs showing stronger lower tail dependence",
           "10. 95% CI of average P_l - P_u across all location pair")
           )
...
knitr::kable(tab_multivar_summary, 
             format="latex", 
             #short.caption = "Summary of results for multivariate data sets",
