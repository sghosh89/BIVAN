...
<!--***DAN: Maybe insert some text indicating these are first, exploratory models, and are not intended to comprehensively explore
a mechanism (that can be done in future work), but rather to get a sense
for whether observed copula structure may be partly due to the
mechanism, and whether the mechanism is worthy of more comprehensive
exploration in that connection in future work. One way these are noncomprehensive
explorations is that I chose particular limited sets of parameters.-->
...
nonparametric statistics (see section on methods for Q1) to these 
...
<!--***DAN: Didn't we use another value of r as well, and present results
on it? If so, probably should mention it here, in part so the exploration
seems a bit broader? Well, we mentioned that we did those and we verbally summarize
results but we did not show any. So maybe leave things as is in this respect.-->

For each of the model setups above, for each of several copula families, 
<!--***DAN: I currently use C, SC, G, SG, J, SJ. Do I really need all those? 
Surely it would be enough to just do C and SC and then state that the others 
give similar results? And if I move one of the result figure from this section
into the main text, that will mean only one such fig in the sup mat, at least
for AR(1)? I mean, right now we have 18 figure in sup mat from this analysis,
and only part of one of them in main text! See if the results are actually similar across
S/SC, G/SG, J/SJ, and if so consider keeping only the S/SC results in either the
main text or the sup mat, and just stating the others were similar. Then you would presumably
move the full C fig into the main text, replacing the current partial version.
So instead of having 18 figs in sup mat and half a fig in main text, you would
have 5 figs in sup mat and 1 in main text, and then you would just say "we did the
same thing for G, SG, J, SJ and got similar results."
So let's check, are results, broadly speaking, independent of which of the 6 copulas
I used?:
A) for AR(1) models, yes
B) for Ricker, weak noise, yes
C) for Ricker, strong noise, its more complicated. It's generally true that you get
more approximate agreement between noise and pop stats, with some rather glaring disagreement.
But which stats disagree the most depends on which cop we used!
So I guess we can still just display the C and SC results, as planned above, and explain
in text.
-->
...
hypothesis from the Introduction was correct that characteristics of 
...
<!--***DAN: move the following to the end of this part-->
...
<!--***DAN: move the above to the end of this part-->
...
<!--***DAN: Shyamolina, can you please combine Figs S24-S26 into one single figure
with 6 panels? Please also search for citations in the main text and sup mat that 
used to refer to the three figures and replace them with a citation to the
new, combined figure.
***Shya: Done.-->
...
<!--***DAN: Simplify as follows, maybe:
1) get rid of table S10 (you will have to change the methods so it no longer says we
do our model selection analysis here)
2) simplify table S11 so it only shows the cor results (also means a change to methods)
and then maybe bring the resulting table to main text?
-->

<!--***DAN: Shyamolina, can you please simplify Table S11 so it only shows the cor
results? That also means the third column will be unnecessary. Please do this as follows:
1) keep but comment out the code that produces the old table; 2) write new code (presumably
starting from a copy of the old code) that makes the new, simpler version of the table. 
That will make it very easy to produce the old version again if we ever want to.
***Shya: Done.-->
...
<!--***DAN: You are using language about confirming hypotheses, here and elsewhere,
but this may rub some people the wrong way because we have not confirmed anything
empirically, we are just doing models, and only a cursory exploration of each
model, at that! So change the language.-->
...
<!--****DAN: Don't firget to replace @tail dependence@ by @tail asociation@ as appropriate-->
across space in ecological variables.
<!--***DAN: move the below to end of this part-->
This result is represented in Fig. \ref{MasterFigure} as the dashed box around 
"Moran effects" and the arrows labelled "B".
<!--***DAN: move the above to end of this part-->
