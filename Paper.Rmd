...
phylogeny tips (Appendix \ref{SM-evol_mech} fro details). This topic is revisited in the Discussion.
...
mechanism (Fig. \ref{MasterFigure}, A) was via nonlinear environmental influences
on ecological variables. A second mechanism (Fig. \ref{MasterFigure}, B) produces tail dependence via
...
(Fig. \ref{MasterFigure}, X-Z). First (Fig. \ref{MasterFigure}, X), the skewness
...
Second (Fig. \ref{MasterFigure}, Y), metapopulation extinction risk can be accentuated or reduced by tail dependence
properties of environmental variables. We also showed effects of copula 
structure on Taylor's law (Fig. \ref{MasterFigure}, Z), which has been applied in many fields.
In our view, our results make our "overall hypothesis," framed in the Introduction,
...
example datasets and models. Further suggestions for future possible tests are below. 
...
straightforwardly for numerous datasets by examing copulas describing dependencies
...
Causal mechanism C, which we also did not test, could be tested straightforwardly
by analyzing copulas of abundances of competing species, across space or
through time.
...
made through time at different locations. Non-extreme values, not associated with 
...
to $Y_1$, $Y_2$, $Y_3$ or to any combination thereof. This constitutes 
...
fishes because such fishes may invest much or little in juvenile survival.
...
is strongly influenced by temperature the previous winter (@Sheppard2016,
supplementary fig. 6).
...
parts of causal hypothesis A due to much existing evidence, this result 
illustrates how parts one and two of the hypothesis can be tested using copula
techniques. The third part of causal hypothesis A, which our modelling results
supported, now suggests spatial dependence between green spruce aphid counts
should exhibit left-tail dependence - exactly what was observed 
(Table \ref{tab_multivar_summary}). And the consequences of that spatial tail 
dependence for skewness of spatially averaged aphid counts was described already 
(Fig. \ref{fig_skewness_spearman}A). Thus the asymmetric influence of winter 
temperature ultimately causes spatially averaged green spruce aphid abundances to have
lower skew than they would otherwise, because negative impacts of cold winter
are spatially synchronous but positive impacts of warmer winters and other factors
are less spatially or temporally consistent.
...
variability of total-community
...
may also be of interest because it can help characterize "spikiness". Future work on copula 
structure of interspecific relationships in communities and its implications for community 
stability seems likely to be an exciting avenue of research.
...
can influence extinction risk, substantial work remains to determine the magnitude 
of this effect in different scenarios. First, we used 
