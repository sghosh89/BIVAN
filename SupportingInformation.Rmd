...
copulas. S is Spearman correlation; p is the value of the parameter for the survival Clayton 
...
\caption[Example Gumbel copulas]{Log-transformed pdfs (A-E) and samples (F-J) from example Gumbel copulas. S is 
Spearman correlation; $p$ is the value of the parameter for the Gumbel family (it is a 
...
copulas. S is Spearman correlation; $p$ is the value of the parameter for the survival Gumbel 
family (it is a one-parameter family); and LT and UT are the measures of lower- and 
...
copulas. K is Spearman correlation; $p$ is the value of the parameter for the survival Joe  
family (it is a one-parameter family); and LT and UT are the measures of lower- and 
upper-tail dependence, respectively. The parameter range for the family is $p \in [1,\infty)$, upper-tail dependence is $0$ and lower-tail
...
\caption[Example Frank copulas]{Log-transformed pdfs (A-E) and samples (F-J) from example Frank copulas. S is Kendall correlation; $p$ is the value of the parameter for the Frank family (it is a 
one-parameter family); and LT and UT are the measures of lower- and upper-tail dependence, 
...
upper-tail dependence, and survival Clayton copulas, being rotations of C copulas by 
...
C copulas, negative for survival Clayton copulas (except for the case of $\rho=0$, a 
...
knitr::kable(h_ord,
             format='latex',linesep = "",row.names = F, digits = c(4,2,2,4,4),
...
\caption[Nonparametric tests against an N-copula null, bird body mass and BMR data]{Nonparametric tests for tail dependence and other deviations from N-copula structure, for bird body mass and BMR data. Same format as Fig. \ref{MT-fig_soilCN_nonparam}, see caption of that figure. \label{fig_bmr_birds_nonparam}}
\end{center}
\end{figure}
...
knitr::kable(tab_3biv_npa_summary, booktabs = T, escape=F,
      ...
      caption.short = "Nonparametric asymmetry tests against an Normal-copula null model with birds, mammals and Cedar Creek data",
      caption = "Nonparametric asymmetry tests against an Normal-copula null model with birds, mammals and Cedar creek data: for birds and mammals data, results should be interpreted as of soil C and N results (see Table \\ref{MT-tab_soilCN_asym} which shows stronger upper tail dependence). For Cedar Creek data, first slice ($FS$) and last slice ($LS$) stand for $(0,0.25)$ and $(0.75,1)$ range of lower and upper bounds. Though the non-parametric result does not indicate significantly stronger lower tail dependence for Cedar Creek data, the 
greater than (>) sign before the numbers do show the opposite direction in tail-dependence compared to other bivariate datasets.\\label{tab_3biv_npa_summary}")
```
...
(Fig. \ref{MT-fig_soilCN_nonparam}). Results show that tail dependence of data is stronger in 
...
on 1000 Kendall- or Spearman-preserving Normal surrogates, separately in two 
comparisons (Table \ref{MT-tab_soilCN_asym}). Results showed that
...
for 1000 Kendall-preserving surrogate datasets with Normal copulas. Due to relatively smaller sample size,
...
N-copula AICw: the AIC weight of the Normal copula family; Model-avg. LT: the lower-tail 
...
Clayton copula.]{Comparison of copula structure of noise inputs and model 
...
model, survival Clayton copula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
...
\caption[Comparison of copula structure of noise and model output, AR(1) model, 
Gumbelcopula.]{Comparison of copula structure of noise inputs and model outputs, AR(1) 
model, Gumbelcopula. Same format as Figs \ref{fig_Cause4copula_C_kend_ar1} and 
\ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, see captions of those figures for 
...
outputs, stochastic Ricker model, weak noise, Clayton copula. Same format as Figs 
...
outputs, stochastic Ricker model, strong noise, Clayton copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
outputs, stochastic Ricker model, weak noise, survival Clayton copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
...
outputs, stochastic Ricker model, strong noise, survival Clayton copula. Same format as Figs 
\ref{fig_Cause4copula_C_kend_ar1} and \ref{MT-fig_Cause4copula_scatter_C_kend_ar1}, 
