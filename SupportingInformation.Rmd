...
\caption{Analogous to figure \ref{MT-fig_extrisk_lcohen_local} in maintext but with global dispersal.\label{fig_extrisk_lcohen_global}}
\end{center}
\end{figure}
...
A $N \times N$ dispersal matrix ($D$) was generated assuming that the $N$ number of metapopulation patches
were arranged in a linear chain fashion. Say, population from $i^{th}$ patch could either disperse to its nearest
connecting patches only (we called it as local dispersal) or could disperse to all other patches 
equally (we called it as global dispersal) at an dispersal rate, $d$. In case of 
local dispersal matrix the diagonal entries $D_{ii}$ would be equal to $d$ and its immediate neighbors
$D_{ii\pm j ,\;\ j=1}$ would get an equal proportion from the rest of $(1-d)$. The other distant neighbors 
$D_{ii\pm j ,\; j>1}$ were set to zero to execute local dispersal. For global dispersal matrix, $D_{ii}$
would be $d$ and all the other entries were equal to $(1-d)/N$. Code to generate these dispersal matrices 
was implemented in [Dispersal_mat](https://github.com/sghosh89/BIVAN/blob/master/mydispersal.R) function.
