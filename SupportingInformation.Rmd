...
\caption[Testing non-parametric statistics for Ceder-creek data with $numbin=10$ for year 2000]{Nonparametric tests for tail dependence and other deviations from a normal copula for \textbf{birds' BMR data}. As described in \nameref{MT-Methods}, each statistic was computed on the data using the ranges $0-0.1$, $0.1-0.2$, ..., $0.9-1$ (red cross), and was also computed using the same ranges for 1000 normal-copula surrogate datasets (blue and green cross show 0.025 and 0.975 quantiles of these results).  When red cross was outside blue and green cross, text at the top of plots indicates how many surrogate values the data value was less than or greater than. \label{fig_bmr_birds_nonparam}}
\end{center}
\end{figure}
...
knitr::kable(tab_bmr_birds_asym,
             format="pandoc",
             caption = "**Birds' BMR data set results** for tests of whether asymmetry of tail dependence was significant compared to a normal-copula null hypothesis on year 2000 \\label{tab_bmr_birds_asym}",
             booktabs=T)
```
...
m3<-bivar_tab_summary_fn(r=BivMS_res_cc$`2000`,rs=stat_cc_ga$`2000`)
tab_bivar_summary<-cbind(m1,m2,m3)
colnames(tab_bivar_summary)<-c("SoilCN","Birds' BMR","Ceder-creek")
...
```{r skewness_aphidcount_allsp_plot, results='hide', echo=F, cache=T, cache.extra=list(seed,data_aphid_count,mtime("good_loclist.R"),mtime("skewness_multivar_dataset.R"),mtime("ncsurrog.R"),mtime("SkewnessAnd3CentMom.R"))}
...
```{r skewness_aphidff_allsp_plot, results='hide', echo=F, cache=T, cache.extra=list(seed,data_aphid_ff,mtime("good_loclist.R"),mtime("skewness_multivar_dataset.R"),mtime("ncsurrog.R"),mtime("SkewnessAnd3CentMom.R"))}
...
```{r skewness_plankton_allsp_plot, results='hide', echo=F, cache=T, cache.extra=list(seed,data_plankton_north_sea,mtime("good_loclist.R"),mtime("skewness_multivar_dataset.R"),mtime("ncsurrog.R"),mtime("SkewnessAnd3CentMom.R"))}
...
pdf("./Results/skewness_results/skewness_plankton_north_sea/plankton_allsp_skewness_kendall.pdf",height=20,width=20)
op<-par(mfrow=c(4,5),mar=c(4.2,5,1,1),mgp=c(3,1,0))
for(i in splist){
  #cat("sp=",i,"\n")
  loclist<-good_loclist(d_allsp=data_plankton_north_sea,sp=i,data_pt_thrs=45)
...
\caption{Skewness of spatially averaged $(A)$ green-spruce aphid's abundance, $(B)$ leaf-curling plum aphid's firstflight date, $(C)$ ceratium furca's abundance and $(D)$ methane-flux data, were indicated by red line in each plot and were compared with respect to skewness of spatially averaged 10000 Normal surrogates preserving Spearman correlation). \label{fig_skewness_spearman}}
\end{center}
\end{figure}
...
for (counter in 1:length(kend.surrog)){
 ...
 kend.stats.res[[counter]]<-matrix(NA,6,numdat) # to keep spatial TL stats
 spea.stats.res[[counter]]<-matrix(NA,6,numdat)
...
pdf("./Results/taylorslaw_results/SpatialTL_6stats_kendall.pdf",height=3,width=18)
op<-par(mfrow=c(1,6),mar=c(4.2,5,1,1),mgp=c(3,1,0))
...
boxplot(s.slope , col=rainbow(4) , xlab="Copula family", ylab="Spatial TL quadratic coefficient", cex.axis=1.5,cex.lab=1.5,xaxt="n")
axis(1, at = c(1:4), x2, cex.axis=1.5)
...
pdf("./Results/taylorslaw_results/SpatialTL_6stats_spearman.pdf",height=3,width=18)
op<-par(mfrow=c(1,6),mar=c(4.2,5,1,1),mgp=c(3,1,0))
x2<-c("C", "N", "F", "SC")
...
\vspace{-0.5 cm} 
\begin{center}
\includegraphics[width=16 cm]{./Results/taylorslaw_results/SpatialTL_CNFSC_kendall.pdf}\\
\end{center}
\textbf{ \hspace{2.8 cm} (E) \hspace{1.8 cm} (F) \hspace{1.8 cm} (G) \hspace{1.8 cm} (H) \hspace{1.6 cm} (I) \hspace{2 cm} (J)}\\
\vspace{-0.5 cm}
\begin{center}
\includegraphics[width=16 cm]{./Results/taylorslaw_results/SpatialTL_6stats_kendall.pdf}
\caption[Spatial Taylor's Law results]{Spatial Taylor's Law results: log(spatial variance) vs. log(spatial mean) for $ns=10$ number of (A) Clayton, (B) Normal, (C) Frank, (D) Survival Clayton surrogates; (E) p-value for test of linearity, (F) p-value for test of homoskedasticity, (G) root mean square error, (H) intercept, (I) slope, (J) quadratic coefficient of regression line for first panel plot. \label{fig_spTL}}
\end{center}
\end{figure}
