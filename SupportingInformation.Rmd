...
<!--\newcommand{\D2}{\operatorname{D}^2} gives error msg : insert $ -->
...
## North Sea plankton data \label{Pns_data}
...
<!--Shya, these citations render as (4), (5), whereas they should render as (4, 5), or better still, (Bjornstad & Falck, 2001; Walter et al, 2017) (the author-date callout style is better when space is not a limitation because it gives the reader more information without having to look in the bibliography). Can you fix this, please?--> 
...
## Results: aphid abundance data for the green spruce aphid, *Elatobium abietinum* \label{Ex_count}
...
### Non-paramtric analysis of tail dependence \label{Ex_count_NPA}
...
<!--Shya, I dont think we need this, so I have commented it out. If you disagree, let me know, otherwise
please go ahead and delete it and this comment.
![copula plot for aphid-count\label{fig_count_allcop}](./Results/stat_results/stat_aphid_count/Sp_10_AllCops.pdf)
-->

<!--Shya, can you please organize these following plots into one plot with mutliple panels?-->
...
![$cor_l$-$cor_u$ vs. Distance plot for aphid-count showing lower tail dependence\label{fig_count_Corl-Coru}](./Results/stat_results/stat_aphid_count/Sp_10_Corl-Coru_vs_D.pdf){ width=50%, height=50% }

![$P_l$-$P_u$ vs. Distance plot for aphid-count showing lower tail dependence\label{fig_count_Pl-Pu}](./Results/stat_results/stat_aphid_count/Sp_10_Pl-Pu_vs_D.pdf){ width=50%, height=50% }

![$D^2_u$-$D^2_l$ vs. Distance plot for aphid-count showing lower tail dependence\label{fig_count_D2u-D2l}](./Results/stat_results/stat_aphid_count/Sp_10_D2u-D2l_vs_D.pdf){ width=50%, height=50% }

## Results: aphid first flight data for the leaf-curling plum aphid, *Brachycaudus helichrysi* \label{Ex_ff}
...
knitr::kable(RES_aphid_ff$LTdep_AICw,
             format='pandoc',
             caption = "Aphid_firstflight : model averaged lower tail dependence statistic for each selective location pair\\label{tab_ff_LTdep_AICw}",
             booktabs=TRUE
             )
```
...
knitr::kable(RES_aphid_ff$UTdep_AICw,
             format='pandoc',
             caption = "Aphid_firstflight : model averaged upper tail dependence statistic for each selective location pair\\label{tab_ff_UTdep_AICw}",
             booktabs=TRUE
             )
```
...
knitr::kable(RES_aphid_ff$LTdep_AICw-RES_aphid_ff$UTdep_AICw,
             digits = 5,
             format='pandoc',
             caption = "Aphid_firstflight : model averaged lower tail dependence - model averaged upper tail dependence statistic for each selective location pair\\label{tab_ff_LTmUT}",
             booktabs=TRUE
             )
```
...
![$cor_l$-$cor_u$ vs. Distance plot for aphid-firstflight showing upper tail dependence\label{fig_ff_Corl-Coru}](./Results/stat_results/stat_aphid_ff/Sp_11_Corl-Coru_vs_D.pdf){ width=50%, height=50% }

![$P_l$-$P_u$ vs. Distance plot for aphid-firstflight showing upper tail dependence\label{fig_ff_Pl-Pu}](./Results/stat_results/stat_aphid_ff/Sp_11_Pl-Pu_vs_D.pdf){ width=50%, height=50% }

![$D^2_u$-$D^2_l$ vs. Distance plot for aphid-firstflight showing upper tail dependence\label{fig_ff_D2u-D2l}](./Results/stat_results/stat_aphid_ff/Sp_11_D2u-D2l_vs_D.pdf){ width=50%, height=50% }

Table \ref{tab_ff_resamp} shows the mean of each non-parametric statistic taken from all pairwise locations and then based on a resampling results how the resampled mean distributed from or around the real mean. Fourth column named $fracgt0$ in Table \ref{tab_ff_resamp} indicates the fraction of resampled mean greater then zero. $\cor_l-\cor_u$, $\Ps_l-\Ps_u$ and $\Dsq_u-\Dsq_l$ statistic with negative mean has all resampled mean less than $0$ for *Leaf-curling plum aphid*-firstflight data. This proves that there is a significant upper tail dependence in *Leaf-curling plum aphid*-firstflight data.

## North sea plankton data for 'Ceratium furca' \label{Ex_pns}
