...
corlmcoru_K<-corstats_K[,1]-corstats_K[,numsurrog]
...
corlmcoru_S<-corstats_S[,1]-corstats_S[,numsurrog]
...
PlmPu_K<-Pstats_K[,1]-Pstats_K[,numsurrog]
...
PlmPu_S<-Pstats_S[,1]-Pstats_S[,numsurrog]
...
D2lmD2u_d<-D2stats_d[1]-D2stats_d[length(D2stats_d)]
D2lmD2u_K<-D2stats_K[,1]-D2stats_K[,numsurrog]
D2lmD2u_frac_K<-sum(D2lmD2u_K<D2lmD2u_d)
...
D2lmD2u_S<-D2stats_S[,1]-D2stats_S[,numsurrog]
D2lmD2u_frac_S<-sum(D2lmD2u_S<D2lmD2u_d)
...
<!--Now save a second plot to the results folder, this one about asymmetry of tail dependence-->
```{r plot_soilCN_graphic_approach_2, echo=F}
#plotting layout, units inches
xht<-0.5
ywd<-0.5
titleht<-.25
...
panwd<-3
gap<-.05
totwd<-2*ywd+2*panwd+2*gap
totht<-3*xht+3*panht+3*gap+titleht
pdf(file="./Results/stat_results/stat_soilCN//CNdata_AsymmTailDep.pdf",width=totwd,height=totht)
...
par(fig=c(ywd/totwd,
          (ywd+panwd)/totwd,
          (3*xht+2*panht+2*gap)/totht,
          (3*xht+3*panht+2*gap)/totht),
    mai=c(0,0,0,0),mgp=c(3,.15,0),tcl=-.25)
hist(corlmcoru_K,50)
points(corlmcoru_d,0,col='red')
mtext(side=3,line=0,text="Kendall-preserving surrogates")
mtext(side=1,line=1,text="Partial correlation")
#text(xlimits[1],ylimits_cor[1],labels='A',cex=1.5,adj=c(.5,0))
fracplot<-function(fracs,ylims)
{
  for (counter in 1:length(fracs))
