...
generate and transmit nontrivial copula structure and tail dependence, with substantial 
...
copula structure? And (Q3) what are the consequences of non-normal copula
structure and tail dependence for our understanding of ecology and for 
applications? To address Q1 (the second through fourth sections of the paper), 
...
To address Q2 (the fifth and sixth sections of the paper) we use 
simple models. To address Q3 (the seventh and eigths sections) we use 
...
Q2 for these data. The values also differed substantially 
...
Fig. \ref{SM-fig_soilCN_nonparam}, Table \ref{SM-tab_soilCN_asym}). The 
...
we also considered 
nonlinear population models based on $P_i(t+1)=P_i(t)\exp \left(r\left(1-P_i(t)/K  \right)+\sigma\epsilon_i(t)\right)$, using $r=0.5$, $K=100$, and $\sigma=0.1$ or 
...
Our next mechanism is evolutionary, and pertains to bivariate 
...
in bivariate characters, and gives rise to tail dependence in the 
character values observed across extant species.
...
of a spatial-average time series is influenced by the tail dependence
between the local quantities that were averaged. Thus copula 
...
time series" and the solid arrows labelled "X". For datasets that
...
empirical evidence supporting the mechanism. The consequences of such tail 
...
to have lower skewness (i.e., less spikiness, greater stability through time) 
...
for birds, (C) the same for mammals, and (D) above-ground plant biomass vs. Shannon's index (\textit{H}) from Cedar Creek. Bottom panels show the corresponding copula plots. See Data for units used in upper panels.
