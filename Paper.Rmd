...
3) send it out to coauthors (done) and maybe get feedback, ESP FROM lAWRENCE
4)make sure to say something about tropical systems
...
on extinction risk, Taylor's law, the stability through time of ecosystem services. By documenting the 
...
<!--***DAN: cehcking the below with Shya-->
We did the same analyses with bird or mammal body mass versus BMR datasets filtered
to include only tropical species. Answers to Q1-Q3 were substantially the same.
...
is correlated across locatons ($\epsilon$) and by an entirely local factor ($\delta$). 
...
The nonlinear population model was $P_i(t+1)=P_i(t)\exp \left(r\left(1-P_i(t)/K  \right)+\sigma\epsilon_i(t)\right)$ for $i=1,2$, using $r=0.5$, $K=100$, $\epsilon_i(t)$
...
(i.e., for simulations that gave normal-copula dependence between populatons
...
in ecology, although these facets of ecological data are only ocassionally 
...
Approaches used and insights gained via our work should apply equally well to 
tropical and temperate ecosystem; we believe copulas are among the tools 
that all ecologists should consider for analysis of their data in the 21st century.
There is certainly no reason to expect that non-normal copula structure and 
asymmetric tail dependence are special properties of datasets from temperate 
regions; indeed both of these features were found 
in mammal and bird body mass versus BMR data filtered to tropical
species. The Moran-effect and Liebig's law potential mechanisms that we explored
for copula structure and tail associations (Fig. \ref{MasterFigure}) also seem as 
likely to hold in tropical as in temperate systems; these are general mechanisms.
Indeed, Moran effects are widespread, and Liebig's law is a dominant paradigm,
certainly operating in a wide variety of tropical and temperate ecosystems. 
...
mentioned in the Inrtoduction, Background, and methods sections and 
...
between variables. To provide an ecological example, suppose the true population densities $p_{1,t}$
...
science. The CPUE measurements, which we denote $c_{1,t}$ and $c_{2,t}$ may be more
...
between variables are influenced not only by the ordinal informatio in ranks but also 
...
in tail associations using cardinal data, a partial Pearson correlation can developed
straightforwardly by replacing $u$ and $v$ in equation \ref{eq:partialspearman}
by the $x$ and $y$, but still computing the sum in that equation over data points
...
that may also be useful for understanding ecological relationships.
...
We thank Joel E. Cohen for making us aware of copulas and their potential utility in the 
...
Anderson, Lei Zhao, and editors and annonymous reviewers for helpful suggestions. 
