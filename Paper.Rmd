...
of the spatial average of time series from different locations (e.g., local pest abundance
time series) is affected by the tail dependence of the relationships
...
simulation results in an Appendix showed that spatial Taylor's law can be strongly influenced by
tail dependence between local populations. In our view, our results show that, for many
ecological phenomena, a complete understanding is impossible without 
...
convenient computer implementations exist (e.g., the `copula` and 
`VineCopula` packages in R). Ecologists need only to become aware of these tools
and understand them conceptually to start applying them broadly to ecological
problems. We created a variety of interrelated surrogate (randomization) procedures
(Appendices \ref{SM-surrog_test}, \ref{SM-Surrog_multi}, \ref{SM-sect:copsurrognd}) 
that build upon existing methods and may be especially useful in ecological applications.

<!--***DAN: some mechanism paragraphs follow-->
...
<!--***DAN: Shya, please read or reread all the papers we have that say anything about 
copula structure of dependence between an environmental variable measured in different places, 
and then write this paragraph.-->
One may expect our Moran-like mechanism for non-N copula structure and tail dependence to 
operate commonly, because Moran effects are likely ubiquitous [@Liebhold2004], and 
non-N and tail-dependent copula structures are common in environmental variables.
<!--***DAN: Shya, this topic sentence may have to be modified if you find non-N and tail-dependent
copula structures in environmental variables measured in different places are not so common,
or have not been studied much. Butif you find some papers, and they show non-N structure, you 
can just summarize their results here.-->
<!--***DAN: Idea to work in if it fits: if large weather events (e.g., large rain events) 
are also widespread, that should lead to right-tail dependence in, e.g., rainfall.-->
...
There are also substantial reasons to expect that our second mechanism operates commonly. 
The second mechanism is the one that involves asymmetric or nonlinear influences of 
environmental variables. Liebig's law of the minimum and the idea of the "limiting 
nutrient" for organism or population growth are fundamental and dominant paradigms in ecology.
...
<!--***DAN: you may want to combine the above para with the previous one in the end, using an
intro something like "There are good reasons to suspect that the two mechanisms we suggested
operate extremely commonly in real ecosystems"-->

<!--4) We have only given two mechanisms, but there are likely to be others, and discoverig
additional mechanisms of copula structure is certain worthy of future work. Maybe I can think
of some ideas and make a paragraph.
5) If Van gives us some ideas, they may fit in the Discussion here.
...
<!--***DAN: some consequences paragraphs next-->
...
<!--A para that sets up Shya's extinction risk paper - asks about importance of this effect for real 
...
plays a role? Second, we considered metapopulation extinction risk, but the whole field 
of population viability analysis [@REF-Morris and Doak] stochastic matrix modelling 

<!--Maybe a para from Holder on his thoughts in the importance of copulas for
