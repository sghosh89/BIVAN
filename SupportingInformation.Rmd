...
- Given data $x_i(t)$ for $i=1,\ldots,N$ and $t=1,\ldots,T$, let $\tau_{ij}$ be the Kendall
correlation of $x_i(t)$ and $x_j(t)$.  
- Calculate the covariance matrix of a multivariate normal distribution with standard-normal
marginals and with pairwise Kendall correlations equal to the $\tau_{ij}$. This is possible
using the `iTau` function of the `VineCopula` package.  
...
Our code for this algorithm is `ncsurrog` in the `BIVAN` repository 
...
Because ranks are preserved in passing from $a_i(t)$ to $b_i(t)$, pairwise Kendall 
correlations for the $b_i(t)$ will be the same as those of the $a_i(t)$. 
Discrepancies will come only from the presence of NAs and from sampling variation, 
and should be minor if the 
